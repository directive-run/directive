...
**Charging for a throw, without pretending it is the same as spend.** Nothing at the runner-wrapper layer can tell a completion the provider billed from a connection that was refused: both arrive as an opaque `Error` from an opaque function. Charging the estimate is the safe direction, but it means a cap can fill with money that was never spent &ndash; five refused connections consumed $9 of a $10 hourly cap in testing, indistinguishable from real spend. Two changes:

- A `BudgetExceededError` from a nested `withBudget` is charged **nothing**. That guard raises it from its own pre-call check, before it invokes the runner it wraps, so the provider was provably never contacted. Chained guards no longer bill each other for calls none of them made.
- Every other throw is still charged the estimate, and the charge is now reported separately by a new **`getFailedCallSpend(window)`** on `BudgetRunner`, alongside `getSpent`. `getSpent(w) - getFailedCallSpend(w)` is spend attributable to calls that returned; a figure approaching `getSpent` means a cap is being consumed by failures rather than by work.

`createConstraintRouter` gets both, for the same reason it gets everything else here: a `facts.totalCost` that moves on a call the provider never saw makes a `facts.totalCost > N` failover fire on spend that never happened. Its accessor is `getFailedCallSpend()` &ndash; no window argument, since the router keeps a lifetime total. A blocked call still counts toward `facts.errorCount`, because the routing constraints should see that it failed.
...
3. **Check `getFailedCallSpend()` too.** It is the part of `getSpent()` charged for calls that threw. A figure close to `getSpent()` means a cap is filling with failures, not work.
