...
### 1. `playground_link` MCP tool — **SHIPPED v0.4.0 (2026-06-06, post-amendment)**

Returns a `directive.run/playground` URL (or `directive.run/run` when `mode: "instant"`). The page decompresses the payload from the URL hash and offers Open-in-StackBlitz, which boots a real running Directive project. Source travels in the URL fragment (never sent to the server) and is compressed with lz-string. 8 KB cap on raw input.

**Initial 0.3.0 ship (2026-06-04) was alpha-incomplete** — accepted only a single `source` string and dropped it as `src/main.ts`. When the AI piped a `generate_module` library export through, the StackBlitz project booted to a file that re-exported a binding and exited. The user road-tested it and surfaced the gap on 2026-06-05; the UX-lens AE called it "a 10-second boot to broken code" failure mode and the worst kind of trust-burn.

**0.4.0 closes the gap** with the UX lens's "paired files always" pattern:

- `@directive-run/scaffold@0.2.0` — `generateModule` / `generateOrchestrator` now return `{moduleSource, runnerSource, suggestedFilenames, runnable}`. The runner is a 15-25 LOC driver that does `createSystem(module).start()` → dispatch detected events → `await system.settle()` → log facts → destroy. Generated by regex-extracting the binding name + event keys from the module source.
- `playground_link` now accepts EITHER a single `source` string (kept for `get_example` / `fix_code` already-runnable output) OR a `files: [{path, source}]` array (used by `generate_module`'s paired output). The multi-file payload is JSON-encoded before lz-string compression.
- New `mode: "preview" | "instant"` param. `"preview"` (default) routes to `directive.run/playground` (code + Open-in-StackBlitz button — the only Directive-branded surface and the recommended default for first-time users). `"instant"` routes to `directive.run/run`, a thin auto-submit page that POSTs the StackBlitz form on load and lands in the editor in ~600ms.
- E2E gate in `packages/mcp/__tests__/multi-file-playground.test.ts` asserts that a `generate_module` → `playground_link` chain produces a URL whose decoded payload contains both `createSystem` AND `system.start()`. Catches the entire class of "runner forgot to ship" regressions before npm.

**v0.3.0 alpha exit criterion (now enforced):** the URL the MCP tool emits, when decoded, must contain both `createSystem` and `system.start()`.

**Phase 2 (follow-on PR, deferred):** `run_in_sandbox({source})` tool that actually executes the snippet in a Node worker (vendored `@directive-run/core`, 5s budget, 64MB cap) and returns the first facts transcript in chat. The Innovation lens's "Karpathy tweet asset" pick — "the first state-management MCP that returns observed behavior, not just code." Gated on a sandbox-design review (FS / network / import allowlist; `worker_threads` shares the parent process FS with the spawner, so just adding it to a tool isn't safe).
