...
The schema declares all types for your module. Only `facts` is required &ndash; other sections are optional.
...
Available builders:
- `t.string<T>()` &ndash; String (optionally narrowed to literal union)
- `t.number()` &ndash; Number with `.min()`, `.max()` chainable validators
- `t.boolean()` &ndash; Boolean
- `t.array<T>()` &ndash; Array with `.of()`, `.nonEmpty()`, `.minLength()`, `.maxLength()`
- `t.object<T>()` &ndash; Object with `.shape()`, `.nonNull()`, `.hasKeys()`
- `t.enum(...values)` &ndash; String enum from literal values
...
**Note:** Type assertion schemas produce console warnings for unknown keys. This is expected behavior &ndash; the types exist only at compile time.
...
// Validation works with t.*(), Zod, but NOT type assertions (no runtime info)
```
...
const schema = {
  ...
  derivations: {} as { doubled: number }, // Type assertions for type-only
  events: { increment: {} },
  requirements: {},
} satisfies ModuleSchema;
```
...
|----------|------|-------------|
| `when` | `(facts) => boolean \| Promise<boolean>` | Condition &ndash; returns true when the constraint is active |
...
Constraint `when()` functions are auto-tracked &ndash; Directive records which facts are read during evaluation. On subsequent reconciliation cycles, only constraints affected by changed facts are re-evaluated (incremental evaluation).
...
- If A's `when()` returns false (no requirement), B proceeds immediately &ndash; nothing to wait for
...
**Priority vs `after`:**
- `after` always takes precedence &ndash; a constraint with `after: ["A"]` will always wait for A, regardless of priority
...
Fact mutations inside `resolve` are automatically batched &ndash; all synchronous writes are coalesced into a single notification.

#### Dynamic Requirement Matching

The `requirement` field accepts a string or a function. The string form matches `req.type` directly. The function form lets you match dynamically &ndash; useful for wildcards, prefix matching, or matching on payload fields:
...
Backoff calculation:
- `"none"` &ndash; constant delay (`initialDelay` every time)
- `"linear"` &ndash; `initialDelay * attempt` (100ms, 200ms, 300ms...)
- `"exponential"` &ndash; `initialDelay * 2^(attempt-1)` (100ms, 200ms, 400ms...)

Retries are AbortSignal-aware &ndash; cancelling a resolver immediately interrupts retry sleep.
...
Directive uses a **flat merge** architecture where all modules share one facts store. This is intentional &ndash; constraints need a complete view of the world to decide what requirements are needed.
...
Directive centralizes your business rules, but computed state often needs checking in places where running the full runtime is impractical &ndash; API routes, edge functions, background jobs. **Distributable snapshots** solve this by producing serializable objects containing computed derivation values.
...
**Note:** `validateSnapshot` performs structural validation &ndash; it will throw if the snapshot is missing required `data` or `createdAt` properties, making it safe to use with untrusted input (e.g., parsed from Redis/JWT).
...
- **Reactive UI** &ndash; Use framework adapters (`@directive-run/react`, etc.) instead
- **Real-time updates** &ndash; Snapshots are point-in-time; use `subscribe()` for live data
- **Sensitive data** &ndash; Be mindful of what you include in JWTs (they're readable)
