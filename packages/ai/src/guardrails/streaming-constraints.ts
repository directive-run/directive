...
export interface StreamingConstraint {
  ...
  check: (
    ctx: StreamingConstraintContext,
  ) => Promise<StreamingConstraintResult> | StreamingConstraintResult;
...
export interface StreamingConstraintRunnerConfig {
  ...
  onViolation?: (
    constraint: StreamingConstraint,
    result: StreamingConstraintResult,
    ctx: StreamingConstraintContext,
  ) => void;
 ...
 * ```
 */
export function createStreamingConstraintRunner(
  config: StreamingConstraintRunnerConfig,
) {
  const {
    constraints,
      ...
      for (const constraint of sortedConstraints) {
        ...
        try {
          ...
          if (!result.passed) {
            ...
            if (
              doTerminate &&
              (result.action === "terminate" || result.action === undefined)
            ) {
...
export function withStreamingConstraints(
  ...
  return <T>(
    agent: Parameters<StreamRunner>[0],
    input: string,
    options?: Parameters<StreamRunner>[2],
  ...
  return {
    ...
    check(ctx) {
      ...
      if (!warned && warnAtPercent > 0) {
        ...
        const charPercent = maxCharacters
          ? (ctx.partialOutput.length / maxCharacters) * 100
          : 0;
        ...
        case "json": {
          ...
          if (!output.startsWith("{") && !output.startsWith("[")) {
            return {
              passed: false,
              reason:
                "Output does not appear to be JSON (missing opening brace/bracket)",
              severity: "warning",
              action: "warn",
            };
          }
