...
describe("createOpenAIEmbedder", () => {
  ...
  it("passes apiKey as Bearer token", async () => {
    const mockFetch = vi
      .fn()
      .mockResolvedValue(jsonResponse({ data: [{ embedding: [0.1] }] }));
  ...
  it("sends correct body with model and dimensions", async () => {
    const mockFetch = vi
      .fn()
      .mockResolvedValue(jsonResponse({ data: [{ embedding: [0.1, 0.2] }] }));
  ...
  it("returns the embedding array", async () => {
    ...
    const mockFetch = vi
      .fn()
      .mockResolvedValue(jsonResponse({ data: [{ embedding: expected }] }));
  ...
  it("throws on non-OK response with truncated error body", async () => {
    const mockFetch = vi
      .fn()
      .mockResolvedValue(textResponse("Unauthorized: invalid API key", 401));
