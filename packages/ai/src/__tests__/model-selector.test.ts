...
import { withModelSelection, byInputLength, byAgentName, byPattern } from "../model-selector.js";
...
describe("withModelSelection", () => {
	...
	it("falls through to agent's original model when no rule matches", async () => {
		const inner = makeRunner();
		const runner = withModelSelection(inner, [
			byInputLength(5, "mini"),
		]);

		const result = await runner({ name: "test", model: "gpt-4" }, "this is a long input");
	...
	it("preserves agent properties other than model", async () => {
		...
		const runner = withModelSelection(inner, [
			byAgentName("my-agent", "mini"),
		]);

		await runner({ name: "my-agent", instructions: "Be helpful.", model: "big" }, "hello");
	...
	it("passes options through to inner runner", async () => {
		const inner = vi.fn(async (_agent: AgentLike, _input: string, options: unknown) => {
