...
describe("withFallback", () => {
	...
	it("throws AllProvidersFailedError when all fail", async () => {
		...
		await expect(runner(mockAgent(), "hello")).rejects.toThrow(AllProvidersFailedError);
	});
...
describe("AllProvidersFailedError", () => {
	...
	it("errors array is frozen (immutable)", async () => {
		const runner = withFallback([failingRunner("error-1"), failingRunner("error-2")]);
	...
	it("cause is set to the last error", async () => {
		const runner = withFallback([failingRunner("first"), failingRunner("last")]);
