...
describe("extractJsonFromOutput", () => {
  it("parses clean JSON object", () => {
    expect(extractJsonFromOutput('{"name": "Alice"}')).toEqual({
      name: "Alice",
    });
  });
  ...
  it("extracts JSON from surrounding text", () => {
    expect(
      extractJsonFromOutput(
        'Here is the result: {"score": 0.95} Hope that helps!',
      ),
    ).toEqual({ score: 0.95 });
  });
  ...
  it("handles nested objects", () => {
    expect(extractJsonFromOutput('{"a": {"b": {"c": 1}}}')).toEqual({
      a: { b: { c: 1 } },
    });
  });

  it("handles strings with braces", () => {
    expect(extractJsonFromOutput('{"msg": "hello {world}"}')).toEqual({
      msg: "hello {world}",
    });
  });
  ...
  it("handles escaped quotes in strings", () => {
    expect(extractJsonFromOutput('{"text": "he said \\"hello\\""}')).toEqual({
      text: 'he said "hello"',
    });
  });
...
describe("withStructuredOutput", () => {
  ...
  it("throws StructuredOutputError after all retries exhausted", async () => {
    ...
    await expect(runner(mockAgent(), "Do something")).rejects.toThrow(
      StructuredOutputError,
    );
...
describe("withStructuredOutput config validation", () => {
  it("throws on negative maxRetries", () => {
    ...
    expect(() =>
      withStructuredOutput(inner, { schema, maxRetries: -1 }),
  ...
  it("throws on NaN maxRetries", () => {
    ...
    expect(() =>
      withStructuredOutput(inner, { schema, maxRetries: Number.NaN }),
    ).toThrow("maxRetries must be a non-negative finite number");
  });
...
describe("extractJsonFromOutput length guard", () => {
  it("throws on output exceeding 1MB", () => {
    ...
    expect(() => extractJsonFromOutput(oversized)).toThrow(
      "Output too large for JSON extraction",
    );
  });
