...
describe("withBudget", () => {
	...
	it("blocks calls exceeding per-call budget", async () => {
		...
		await expect(runner(mockAgent(), longInput)).rejects.toThrow(BudgetExceededError);
	...
	it("tracks rolling hourly budget", async () => {
		...
		const runner = withBudget(inner, {
			budgets: [
				{ window: "hour", maxCost: 0.05, pricing: PRICING },
			],
		});
		...
		await expect(runner(mockAgent(), "hello")).rejects.toThrow(BudgetExceededError);
	});
...
describe("withBudget config validation", () => {
	it("throws on non-positive charsPerToken", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { charsPerToken: 0 })).toThrow("charsPerToken must be a positive finite number");
	});

	it("throws on negative charsPerToken", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { charsPerToken: -1 })).toThrow("charsPerToken must be a positive finite number");
	});

	it("throws on NaN charsPerToken", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { charsPerToken: NaN })).toThrow("charsPerToken must be a positive finite number");
	});

	it("throws on negative maxCostPerCall", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { maxCostPerCall: -1, pricing: PRICING })).toThrow("maxCostPerCall must be a non-negative finite number");
	});

	it("throws on Infinity maxCostPerCall", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { maxCostPerCall: Infinity, pricing: PRICING })).toThrow("maxCostPerCall must be a non-negative finite number");
	});

	it("throws on negative estimatedOutputMultiplier", () => {
		const inner = makeRunner();
		expect(() => withBudget(inner, { estimatedOutputMultiplier: -0.5 })).toThrow("estimatedOutputMultiplier must be a non-negative finite number");
	});
	...
	it("warns when maxCostPerCall set without pricing", () => {
		...
		expect(spy).toHaveBeenCalledWith(expect.stringContaining("maxCostPerCall has no effect without pricing"));
		spy.mockRestore();
	});
});
