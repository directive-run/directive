...
import { withRetry, parseHttpStatus, parseRetryAfter, RetryExhaustedError } from "../retry.js";
...
describe("parseHttpStatus", () => {
	...
	it("extracts 503 from error message", () => {
		expect(parseHttpStatus(new Error("request failed: 503 Service Unavailable"))).toBe(503);
	});
	...
	it("extracts status from [Directive] error format", () => {
		expect(parseHttpStatus(new Error("[Directive] AgentRunner request failed: 401 Unauthorized"))).toBe(401);
	});
});
...
describe("parseRetryAfter", () => {
	it("extracts Retry-After in seconds", () => {
		expect(parseRetryAfter(new Error("429 rate limited, Retry-After: 5"))).toBe(5000);
	});
...
describe("withRetry", () => {
	...
	it("throws RetryExhaustedError when all retries fail", async () => {
		...
		await expect(runner(mockAgent(), "hello")).rejects.toThrow(RetryExhaustedError);
...
describe("withRetry config validation", () => {
	it("throws on negative maxRetries", () => {
		const inner = makeRunner([successResult("ok")]);
		expect(() => withRetry(inner, { maxRetries: -1 })).toThrow("maxRetries must be a non-negative finite number");
	});

	it("throws on NaN maxRetries", () => {
		const inner = makeRunner([successResult("ok")]);
		expect(() => withRetry(inner, { maxRetries: NaN })).toThrow("maxRetries must be a non-negative finite number");
	});

	it("throws on Infinity maxRetries", () => {
		const inner = makeRunner([successResult("ok")]);
		expect(() => withRetry(inner, { maxRetries: Infinity })).toThrow("maxRetries must be a non-negative finite number");
	});

	it("throws on negative baseDelayMs", () => {
		const inner = makeRunner([successResult("ok")]);
		expect(() => withRetry(inner, { baseDelayMs: -100 })).toThrow("baseDelayMs must be a non-negative finite number");
	});

	it("throws on negative maxDelayMs", () => {
		const inner = makeRunner([successResult("ok")]);
		expect(() => withRetry(inner, { maxDelayMs: -1 })).toThrow("maxDelayMs must be a non-negative finite number");
	});
...
describe("withRetry AbortSignal", () => {
	...
	it("does not retry when signal is already aborted", async () => {
		...
		await expect(runner(mockAgent(), "hello", { signal: controller.signal })).rejects.toThrow(RetryExhaustedError);
		expect(inner).toHaveBeenCalledTimes(1);
	});
});
