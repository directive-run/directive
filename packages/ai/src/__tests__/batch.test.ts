...
describe("createBatchQueue", () => {
	...
	it("flushes when batch is full", async () => {
		const inner = makeRunner();
		const queue = createBatchQueue(inner, { maxBatchSize: 2, maxWaitMs: 60000 });
	...
	it("manual flush processes pending calls", async () => {
		...
		const inner = makeRunner();
		const queue = createBatchQueue(inner, { maxBatchSize: 100, maxWaitMs: 60000 });
	...
	it("respects concurrency limit", async () => {
		...
		const queue = createBatchQueue(inner, { maxBatchSize: 10, maxWaitMs: 10, concurrency: 2 });
	...
	it("rejects submissions after dispose", async () => {
		vi.useRealTimers();
		const inner = makeRunner();
		const queue = createBatchQueue(inner, { maxBatchSize: 100, maxWaitMs: 100 });
		await queue.dispose();

		await expect(queue.submit(mockAgent(), "hello")).rejects.toThrow("disposed");
	});
...
describe("createBatchQueue config validation", () => {
	it("throws on zero maxBatchSize", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { maxBatchSize: 0 })).toThrow("maxBatchSize must be a positive finite number");
	});

	it("throws on negative maxBatchSize", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { maxBatchSize: -1 })).toThrow("maxBatchSize must be a positive finite number");
	});

	it("throws on NaN maxBatchSize", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { maxBatchSize: NaN })).toThrow("maxBatchSize must be a positive finite number");
	});

	it("throws on negative maxWaitMs", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { maxWaitMs: -1 })).toThrow("maxWaitMs must be a non-negative finite number");
	});

	it("throws on zero concurrency", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { concurrency: 0 })).toThrow("concurrency must be a positive finite number");
	});

	it("throws on NaN concurrency", () => {
		const inner = makeRunner();
		expect(() => createBatchQueue(inner, { concurrency: NaN })).toThrow("concurrency must be a positive finite number");
	});
