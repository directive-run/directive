		...
		[id]: {
			...
			resolve: async (_req: Requirement, ctx: ResolverContext<S>) => {
				...
				try {
					...
					setFact(facts, dataKey, transformedData as FactValue<S, typeof dataKey>);
				} catch (error) {
					if (errorKey) {
					...
					if (loadingKey) {
						setFact(facts, loadingKey, false as FactValue<S, typeof loadingKey>);
					}
				}
			},
		},
...
export function createPollingConstraint<S extends Schema>(
	...
	const { id, intervalMs, lastPollKey, when, action, clock = Date.now } = options;
...
export function createDebouncedConstraint<S extends Schema>(
	...
	const { id, delayMs, watchKey, firstChangeKey, processedValueKey, when, action, clock = Date.now } = options;
	...
	const constraints: ConstraintsDef<S> = {
		[id]: {
			when: (facts) => {
				...
				if (firstChange == null) {
					setFact(facts, firstChangeKey, clock() as FactValue<S, typeof firstChangeKey>);
	...
	const resolvers: ResolversDef<S> = {
		[id]: {
			...
			resolve: async (_req: Requirement, ctx: ResolverContext<S>) => {
				...
				setFact(facts, processedValueKey, currentValue as FactValue<S, typeof processedValueKey>);
...
export function createThrottleConstraint<S extends Schema>(
	...
	const { id, intervalMs, lastExecutionKey, when, action, clock = Date.now } = options;
	...
	const constraints: ConstraintsDef<S> = {
		[id]: {
			when: (facts) => {
				...
				const lastExecution = getFact(facts, lastExecutionKey) as number | undefined;
		...
		[id]: {
			...
			resolve: async (_req: Requirement, ctx: ResolverContext<S>) => {
				...
				if (attemptKey) {
					...
					setFact(facts, attemptKey, (current + 1) as FactValue<S, typeof attemptKey>);
				}
...
export interface OptimisticUpdateOptions<S extends Schema, TPayload = unknown, TResult = unknown> {
	/** Unique ID for this constraint */
	id: string;
	...
	perform: (payload: TPayload, facts: Facts<S>, signal: AbortSignal) => Promise<TResult>;
 ...
 * ```
 */
export function createOptimisticUpdate<S extends Schema, TPayload = unknown, TResult = unknown>(
		...
		[id]: {
			...
			resolve: async (_req: Requirement, ctx: ResolverContext<S>) => {
				...
				try {
					...
					// Rollback on error
					rollback(facts, payload, error instanceof Error ? error : new Error(String(error)));
				} finally {
					if (inflightKey) {
						setFact(facts, inflightKey, false as FactValue<S, typeof inflightKey>);
					}
				}
			},
		},
		...
		[id]: {
			...
			resolve: async (req: Requirement, ctx: ResolverContext<S>) => {
				...
				try {
					...
					if (transformResult) {
						output = transformResult(toolReq.tool, output) as Awaited<ReturnType<typeof tool.execute>>;
					}
					...
					if (errorKey) {
						setFact(
							facts,
							errorKey,
							(error instanceof Error ? error.message : String(error)) as FactValue<S, typeof errorKey>,
						);
					}
					setFact(facts, resultKey, {
						requestId: toolReq.requestId,
