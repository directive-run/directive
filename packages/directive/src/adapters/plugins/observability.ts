 ...
 * @example
 * ```typescript
 * import { createObservability, createAgentMetrics } from 'directive/openai-agents';
 *
...
export function createObservability(config: ObservabilityConfig = {}): ObservabilityInstance {
  ...
  function recordMetric(dataPoint: MetricDataPoint): void {
    ...
    const key = `${dataPoint.name}:${JSON.stringify(dataPoint.labels)}`;
    ...
    getDashboard(): DashboardData {
      ...
      return {
        ...
        summary: {
          ...
          p99Latency: latencyMetric?.p99 ?? 0,
