...
function dt() {
  return (globalThis as Record<string, unknown>).__DIRECTIVE__ as NonNullable<
...
describe("devtoolsPlugin", () => {
  ...
  describe("event tracing", () => {
    ...
    it("records fact.set events", async () => {
      ...
      const factEvent = events.find(
        (e) =>
          e.type === "fact.set" &&
          (e.data as Record<string, unknown>).key === "count",
      );
    ...
    it("respects maxEvents cap", () => {
      ...
      const system = createSystem({
        module: mod,
        plugins: [
          devtoolsPlugin({ name: "capped", trace: true, maxEvents: 5 }),
        ],
      });
      system.start();
  ...
  describe("exportSession / importSession", () => {
    ...
    it("importSession rejects payload without events array", () => {
      ...
      expect(
        dt().importSession(JSON.stringify({ version: 1 }), "bad-import2"),
      ).toBe(false);
    ...
    it("importSession returns false for unknown target system", () => {
      ...
      const payload = JSON.stringify({
        events: [{ timestamp: 1, type: "x", data: {} }],
      });
  ...
  describe("multi-module", () => {
    it("traces events across multiple modules", async () => {
      ...
      const factEvents = events.filter(
        (e) => e.type === "fact.set" || e.type === "facts.batch",
      );
  ...
  describe("circular buffer behavior", () => {
    it("oldest events are dropped when maxEvents is exceeded", () => {
      ...
      const system = createSystem({
        module: mod,
        plugins: [
          devtoolsPlugin({ name: "overflow", trace: true, maxEvents: 3 }),
        ],
      });
      system.start();
  ...
  describe("maxEvents validation", () => {
    it("treats zero maxEvents as default 1000", () => {
      ...
      const system = createSystem({
        module: mod,
        plugins: [
          devtoolsPlugin({ name: "zero-max", trace: true, maxEvents: 0 }),
        ],
      });
      system.start();
    ...
    it("treats negative maxEvents as default 1000", () => {
      ...
      const system = createSystem({
        module: mod,
        plugins: [
          devtoolsPlugin({ name: "neg-max", trace: true, maxEvents: -5 }),
        ],
      });
      system.start();
  ...
  describe("non-writable global", () => {
    it("window.__DIRECTIVE__ cannot be overwritten by assignment", () => {
      ...
      try {
        (globalThis as Record<string, unknown>).__DIRECTIVE__ = {
          hacked: true,
        };
      } catch {
  ...
  describe("timeline tracking", () => {
    it("tracks resolver execution in trace events", async () => {
      ...
      const resolverComplete = events.find(
        (e) => e.type === "resolver.complete",
      );
