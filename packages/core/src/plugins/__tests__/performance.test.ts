...
describe("performancePlugin", () => {
  ...
  describe("resolver metrics", () => {
    ...
    it("tracks completions and duration on onResolverComplete", () => {
      const plugin = createPlugin();
      plugin.onResolverComplete!("r1", mockReq("TEST"), 42);
    ...
    it("tracks maxDurationMs across completions", () => {
      const plugin = createPlugin();
      plugin.onResolverComplete!("r1", mockReq("A"), 10);
      plugin.onResolverComplete!("r1", mockReq("A"), 90);
    ...
    it("tracks lastCompletedAt as a timestamp", () => {
      const plugin = createPlugin();
      const before = Date.now();
      plugin.onResolverComplete!("r1", mockReq("A"), 5);
      const after = Date.now();
    ...
    it("increments errors on onResolverError", () => {
      const plugin = createPlugin();
      plugin.onResolverError!("r1", mockReq("A"), new Error("boom"));
    ...
    it("increments retries on onResolverRetry", () => {
      const plugin = createPlugin();
      plugin.onResolverRetry!("r1", mockReq("A"), 1);
      plugin.onResolverRetry!("r1", mockReq("A"), 2);
  ...
  describe("reconcile metrics", () => {
    it("tracks runs and duration via onReconcileStart/onReconcileEnd", () => {
      ...
      plugin.onReconcileEnd!({} as never);
  ...
  describe("slow resolver callback", () => {
    it("fires onSlowResolver when duration exceeds default threshold (1000ms)", () => {
      ...
      plugin.onResolverComplete!("r-slow", mockReq("A"), 1500);
    ...
    it("does not fire when duration is below threshold", () => {
      ...
      plugin.onResolverComplete!("r-fast", mockReq("A"), 500);
  ...
  describe("independent tracking by ID", () => {
    it("tracks multiple resolvers independently", () => {
      ...
      plugin.onResolverStart!("auth", mockReq("AUTH"));
      plugin.onResolverStart!("auth", mockReq("AUTH"));
      plugin.onResolverStart!("data", mockReq("FETCH"));

      plugin.onResolverComplete!("auth", mockReq("AUTH"), 100);
      plugin.onResolverComplete!("data", mockReq("FETCH"), 200);
      plugin.onResolverError!("data", mockReq("FETCH"), new Error("timeout"));
