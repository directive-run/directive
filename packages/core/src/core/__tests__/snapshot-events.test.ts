...
describe("snapshotEvents", () => {
	...
	it("multi-module: each module controls its own events", async () => {
		const moduleA = createModule("modA", {
			...
			events: {
				...
				noopA: (facts) => { facts.x = (facts.x as number); },
			},
			// Only setX snapshots
			snapshotEvents: ["setX"],
		});
		...
		const moduleB = createModule("modB", {
			...
			events: {
				...
				noopB: (facts) => { facts.y = (facts.y as number); },
			},
	...
	describe("debug.snapshotModules", () => {
		...
		it("dev warning for unknown module name", () => {
			...
			expect(warnSpy).toHaveBeenCalledWith(
				expect.stringContaining('debug.snapshotModules entry "nonExistent" doesn\'t match any module'),
			);
		...
		it("dev warning for single-module system", () => {
			...
			expect(warnSpy).toHaveBeenCalledWith(
				expect.stringContaining("debug.snapshotModules has no effect in single-module mode"),
			);
