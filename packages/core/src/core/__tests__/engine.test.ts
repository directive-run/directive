...
describe("effects and the reconcile boundary", () => {
  /**
   * A fact written while an async effect is awaiting must still reach the
   * effects that depend on it.
   *
   * The effects phase is awaited, and anything a resolver or another effect
   * writes while it is open lands in the pending set behind it. That set used
   * to be cleared wholesale afterwards, so the constraints saw those keys and
   * the effects never did — an effect declared on the exact key that changed
   * simply did not run, and whether it happened depended on which microtask
   * the write landed in.
   */
  it("delivers a fact written during the effects phase to its effect", async () => {
    const sawB: number[] = [];
    ...
    expect(sawB.length).toBeGreaterThan(before);
    expect(sawB.at(-1)).toBe(30);
