...
function createFailingModule(opts?: {
	...
	return {
		module,
		setResolverFail: (v: boolean) => { resolverShouldFail = v; },
		setConstraintFail: (v: boolean) => { constraintShouldFail = v; },
		setEffectFail: (v: boolean) => { effectShouldFail = v; },
		setDerivationFail: (v: boolean) => { derivationShouldFail = v; },
	};
}
...
describe("error boundary: retry-later manager", () => {
	...
	it("respects maxRetries limit", () => {
		...
		const strategy = boundary.handleError("resolver", "testResolver", new Error("fail"), {});
...
describe("error boundary: getStrategy callback integration", () => {
	it("callback returning a strategy string is used", () => {
		...
		const strategy = boundary.handleError("resolver", "test", new Error("fail"));
