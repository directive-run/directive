...
import { isAgentComplete, type DebugEvent } from "../lib/types";
...
function buildCostRows(events: DebugEvent[]): AgentCostRow[] {
  const agentMap = new Map<string, { tokens: number; runs: number; durationMs: number }>();
  ...
  for (const event of events) {
    ...
    } else if (event.type === "resolver_complete" && typeof event.totalTokens === "number") {
      agentId = typeof event.agentId === "string" ? event.agentId
        : typeof event.requirement === "string" ? event.requirement
        : "resolver";
      tokens = event.totalTokens;
      duration = typeof event.durationMs === "number" ? event.durationMs : 0;
    }
  ...
  const grandTotal = Array.from(agentMap.values()).reduce((sum, v) => sum + v.tokens, 0);

  const rows: AgentCostRow[] = Array.from(agentMap.entries()).map(([agentId, data]) => ({
    agentId,
    runs: data.runs,
    ...
    <div className="flex h-full flex-col overflow-auto">
      ...
      <div className="flex items-center gap-6 border-b border-zinc-800 px-6 py-3">
        <div>
          ...
          <div className="text-xl font-bold text-zinc-100">{totals.totalTokens.toLocaleString()}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wider text-zinc-500">Est. Cost</div>
          <div className="text-xl font-bold text-emerald-400">${totals.totalCost.toFixed(2)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wider text-zinc-500">Agents</div>
          <div className="text-xl font-bold text-zinc-100">{totals.totalAgents}</div>
        </div>
      </div>
      ...
      <div className="flex-1 p-6">
        ...
        <div className="mb-2 flex h-8 w-full overflow-hidden rounded">
          {rows.map((row, i) => (
            <div
              ...
              style={{ width: `${row.pctOfTotal}%`, backgroundColor: getAgentColor(i, rows.length) }}
        ...
        <div className="mb-6 flex flex-wrap gap-3">
          {rows.map((row, i) => (
            <div key={row.agentId} className="flex items-center gap-1.5 text-xs text-zinc-400">
              <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: getAgentColor(i, rows.length) }} />
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-900 text-xs uppercase tracking-wider text-zinc-500">
                ...
                <th className="px-4 py-2 text-right font-medium">Total Duration</th>
                ...
                >
                  <td className="px-4 py-2 text-zinc-200">
                    <span className="flex items-center gap-2">
                      <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: getAgentColor(i, rows.length) }} />
                      {row.agentId}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-right text-zinc-400">{row.runs}</td>
