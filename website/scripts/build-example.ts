...
function buildExample(name: string): void {
  ...
  execFileSync('pnpm', ['--filter', `@directive-run/example-${name}`, 'build'], {
    cwd: ROOT,
    stdio: 'inherit',
  })
...
function findBuildableExamples(): string[] {
  return fs
    ...
    .filter((entry) => {
      ...
      return fs.existsSync(path.join(EXAMPLES_DIR, entry.name, 'vite.config.ts'))
    })
...
if (buildAll) {
  ...
  console.log('  pnpm build:examples         Build all examples with vite.config.ts')
  process.exit(1)
}
