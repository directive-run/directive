...
function processInterface(
  ...
  for (const member of decl.getMembers()) {
    ...
    methods.push({
      name: memberName,
      signature:
        memberSig.length > 500 ? memberSig.slice(0, 500) + "..." : memberSig,
...
function processDeclaration(
  ...
  switch (kind) {
    ...
    case SyntaxKind.ClassDeclaration:
      return processClass(decl as ClassDeclaration, name, moduleName, basePath);
    default:
      return null;
  }
}
...
function generateMarkdown(entries: ApiDocEntry[]): string {
  ...
  for (const mod of sortedModules) {
    ...
    lines.push(
      `## ${mod === "directive" ? "Core (`directive`)" : `AI Adapter (\`${mod}\`)`}`,
    );
    lines.push("");
