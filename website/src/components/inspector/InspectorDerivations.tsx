...
export function InspectorDerivations() {
  ...
  if (!connected || derivationCount === 0) {
    return (
      <div className="py-6 text-center text-xs text-zinc-400 dark:text-zinc-500">
        {connected ? "No derivations in system" : "Connecting..."}
      </div>
    );
  }
...
function DerivationRow({
  ...
  const handleCopy = useCallback(() => {
    copyToClipboard(derivKey, value);
    setCopied(true);
  ...
  return (
    ...
    >
      <span className="w-32 shrink-0 truncate text-violet-600 dark:text-violet-400" title={derivKey}>
        {derivKey}
      </span>
      ...
      <button
        onClick={handleCopy}
        aria-label={copied ? `Copied ${derivKey}` : `Copy ${derivKey}`}
