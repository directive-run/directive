...
const ANIMATION_STEPS = [
  'input',
  'arrow-fan-out',
  'guardrails', 'approval', 'budget',
  'arrow-converge',
  'agent',
  'arrow-fan-out-2',
  'outputGuardrails', 'memory',
  'arrow-converge-2',
  'response',
] as const
...
export const OrchestratorDiagram = memo(function OrchestratorDiagram() {
  ...
  const nodes = useMemo<Node[]>(() => [
    ...
    // Row 2: Three parallel gate nodes
    {
      ...
      data: {
        ...
        status: isGroupActive(['guardrails', 'approval', 'budget']) ? 'active' : 'idle',
        colorScheme: 'red',
      } satisfies StepNodeData,
    },
    {
    ...
    // Row 4: Two parallel output nodes
    {
      ...
      data: {
        ...
        status: isGroupActive(['outputGuardrails', 'memory']) ? 'active' : 'idle',
        colorScheme: 'red',
      } satisfies StepNodeData,
    },
    {
  ...
  ], [isActive, isGroupActive, centerX, row2Left, row2Center, row2Right, row4Left, row4Right])
  ...
  const edges = useMemo<Edge[]>(() => [
    // Fan-out: Input → 3 gates
    {
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-fan-out'), colorScheme: 'primary', smooth: true },
    },
    {
      id: 'input->approval',
      source: 'input',
    ...
    // Converge: 3 gates → Agent
    {
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-converge'), colorScheme: 'red', smooth: true },
    },
    {
      id: 'approval->agent',
      source: 'approval',
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-converge'), colorScheme: 'amber', smooth: true },
    },
    {
      id: 'budget->agent',
      source: 'budget',
    ...
    // Fan-out 2: Agent → Output + Memory
    {
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-fan-out-2'), colorScheme: 'emerald', smooth: true },
    },
    {
      id: 'agent->memory',
      source: 'agent',
    ...
    // Converge 2: Output + Memory → Response
    {
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-converge-2'), colorScheme: 'red', smooth: true },
    },
    {
      id: 'memory->response',
      source: 'memory',
      ...
      targetHandle: 'top',
      type: 'labeled',
      data: { active: isActive('arrow-converge-2'), colorScheme: 'violet', smooth: true },
    },
  ], [isActive])
