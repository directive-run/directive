    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h4
          ...
          className="mt-3 mb-1 text-xs font-bold"
        >
    ...
    if (line.match(/^[-*]\s/)) {
      ...
      elements.push(
        <ul
          ...
          className="my-1 list-disc pl-4 text-sm"
        >
          {items}
        </ul>,
      );
    ...
    if (line.match(/^\d+\.\s/)) {
      ...
      elements.push(
        <ol
          ...
          className="my-1 list-decimal pl-4 text-sm"
        >
          {items}
        </ol>,
      );
                ...
                {tableRows.slice(1).map((row, ri) => (
                  <tr key={ri}>
                    {row.map((cell, ci) => (
                      <td
                        key={ci}
                        className="border-b border-slate-200 px-2 py-1 dark:border-slate-700"
                      >
                        {renderInlineMarkdown(cell)}
                      </td>
                    ))}
