...
import {
  DevToolsLabelContext,
  DevToolsSystemContext,
} from "./DevToolsSystemContext";
...
export function DevToolsProvider(props: DevToolsProviderProps) {
  ...
  const snapshotUrl =
    effectiveMode !== "system" ? props.snapshotUrl : undefined;

  const urls = useMemo(
    () => ({
      streamUrl: streamUrl ?? "/api/devtools/stream",
      snapshotUrl: snapshotUrl ?? "/api/devtools/snapshot",
      resetUrl: (streamUrl ?? "/api/devtools/stream").replace(
        /\/stream$/,
        "/reset",
      ),
    }),
  ...
  const effectiveRuntimeName =
    runtimeSystemName !== undefined ? runtimeSystemName : detectedRuntime;
