...
export function GuardrailsView() {
  ...
  const { guardrailEvents, guardrails, totalChecks, totalBlocked, passRate } = useMemo(() => {
    ...
    const guardrailMap = new Map<string, { total: number; passed: number; blocked: number; totalMs: number; type: string | null; lastReason: string | null }>()
    for (const e of gEvents) {
      const name = e.guardrailName ?? 'unknown'
      const prev = guardrailMap.get(name) ?? { total: 0, passed: 0, blocked: 0, totalMs: 0, type: null, lastReason: null }
      ...
      guardrailMap.set(name, {
        ...
        lastReason: (!didPass && e.reason) ? (e.reason as string) : prev.lastReason,
      })
    }

    const gList = Array.from(guardrailMap.entries()).map(([name, data]) => ({ name, ...data }))
    ...
    return { guardrailEvents: gEvents, guardrails: gList, totalChecks: tChecks, totalBlocked: tBlocked, passRate: rate }
  }, [events])
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-6 text-xs">
        <div>
          ...
          <div className="text-lg font-semibold text-zinc-900 dark:text-white">{totalChecks}</div>
        </div>
        <div>
          <span className="text-zinc-500 dark:text-zinc-400">Passed</span>
          <div className="text-lg font-semibold text-emerald-600 dark:text-emerald-400">{totalChecks - totalBlocked}</div>
        </div>
        <div>
          <span className="text-zinc-500 dark:text-zinc-400">Blocked</span>
          <div className={`text-lg font-semibold ${totalBlocked > 0 ? 'text-red-600 dark:text-red-400' : 'text-zinc-900 dark:text-white'}`}>
            {totalBlocked}
          </div>
        </div>
        <div>
          <span className="text-zinc-500 dark:text-zinc-400">Pass rate</span>
          <div className={`text-lg font-semibold ${passRate === 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
            {passRate.toFixed(0)}%
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {guardrails.map((g) => {
          ...
          const rate = g.total > 0 ? ((g.passed / g.total) * 100).toFixed(0) : '100'
          ...
          return (
            ...
            >
              ...
              >
                {/* Status dot */}
                <span className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${g.blocked > 0 ? 'bg-red-500' : 'bg-emerald-500'}`} />

                {/* Name + type badge */}
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-medium text-zinc-900 dark:text-white">{g.name}</span>
                  ...
                  {info && (
                    <div className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">{info.description}</div>
                  )}
                </div>

                {/* Stats summary */}
                <div className="flex shrink-0 items-center gap-3 text-[11px]">
                  <span className="text-zinc-500 dark:text-zinc-400">{g.total} checks</span>
                  <span className="font-medium text-emerald-600 dark:text-emerald-400">{rate}%</span>
                  <span className="text-zinc-400 dark:text-zinc-500">{avgMs}ms</span>
              ...
              {isExpanded && (
                <div className="border-t border-zinc-200 px-3 py-2 dark:border-zinc-700">
                  <div className="grid grid-cols-4 gap-3 text-xs">
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">Checks</span>
                      <div className="font-semibold text-zinc-900 dark:text-white">{g.total}</div>
                    </div>
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">Passed</span>
                      <div className="font-semibold text-emerald-600 dark:text-emerald-400">{g.passed}</div>
                    </div>
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">Blocked</span>
                      <div className={`font-semibold ${g.blocked > 0 ? 'text-red-600 dark:text-red-400' : 'text-zinc-900 dark:text-white'}`}>{g.blocked}</div>
                    </div>
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">Avg latency</span>
                      <div className="font-semibold text-zinc-900 dark:text-white">{avgMs}ms</div>
                    </div>
                  </div>
                      ...
                      {[...guardrailEvents]
                        ...
                        .map((e) => (
                          <div key={e.id} className="flex items-center gap-2 font-mono text-[10px]">
                            <span className={`inline-block h-1.5 w-1.5 shrink-0 rounded-full ${e.passed !== false ? 'bg-emerald-500' : 'bg-red-500'}`} />
                            <span className="text-zinc-400 dark:text-zinc-500">{e.durationMs ?? 0}ms</span>
                            {e.passed === false && e.reason && (
                              <span className="truncate text-red-500 dark:text-red-400">{e.reason}</span>
                            )}
                            {e.passed !== false && (
        ...
        <div className="min-h-0 flex-1 space-y-1 overflow-y-auto">
          {[...guardrailEvents].reverse().slice(0, 20).map((e) => (
            <div key={e.id} className="flex items-center gap-2 font-mono text-[11px]">
              ...
              <span className="text-zinc-500 dark:text-zinc-400">{e.guardrailName ?? 'unknown'}</span>
              {e.durationMs != null && (
                <span className="text-zinc-400 dark:text-zinc-500">{e.durationMs}ms</span>
              )}
              {e.passed === false && e.reason && (
                <span className="truncate text-red-500 dark:text-red-400">{e.reason}</span>
              )}
