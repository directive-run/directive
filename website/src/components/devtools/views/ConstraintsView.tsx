...
function LatestRunSummary({ run }: { run: RunChangelogEntry }) {
  ...
  const statusColor = run.status === 'settled'
    ? 'text-emerald-600 dark:text-emerald-400'
    : 'text-amber-600 dark:text-amber-400'

  const chainParts: string[] = []
  if (run.factChanges.length > 0) chainParts.push(`${run.factChanges.length} fact${run.factChanges.length !== 1 ? 's' : ''}`)
  if (run.constraintsHit.length > 0) chainParts.push(`${run.constraintsHit.length} constraint${run.constraintsHit.length !== 1 ? 's' : ''}`)
  if (run.resolversStarted.length > 0) chainParts.push(`${run.resolversStarted.length} resolver${run.resolversStarted.length !== 1 ? 's' : ''}`)
    ...
    <div className="rounded border border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800/50">
      ...
      >
        ...
        {!run.causalChain && chainParts.length > 0 && (
          <span className="truncate text-zinc-500 dark:text-zinc-400">{chainParts.join(' → ')}</span>
        )}
        ...
        <span className="ml-auto text-zinc-400 dark:text-zinc-500">{expanded ? '▲' : '▼'}</span>
      </button>
      ...
      {expanded && (
        <div className="space-y-3 border-t border-zinc-200 px-3 py-2 dark:border-zinc-700">
          ...
          {run.factChanges.length > 0 && (
            <RunSection title="Fact Changes" count={run.factChanges.length} color="sky">
              {run.factChanges.map((fc, i) => (
                <div key={i} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="text-sky-600 dark:text-sky-400">{fc.key}</span>
                  <span className="text-zinc-400">:</span>
                  <span className="text-red-500 line-through">{formatValue(fc.oldValue)}</span>
                  <span className="text-zinc-400">→</span>
                  <span className="text-emerald-600 dark:text-emerald-400">{formatValue(fc.newValue)}</span>
                </div>
              ))}
            </RunSection>
          )}
          ...
          {run.derivationsRecomputed.length > 0 && (
            <RunSection title="Derivations" count={run.derivationsRecomputed.length} color="purple">
              {run.derivationsRecomputed.map((d, i) => (
                <div key={i} className="font-mono text-[10px]">
                  <div className="flex items-center gap-2">
                    <span className="text-purple-600 dark:text-purple-400">{d.id}</span>
                    <span className="text-zinc-400">deps: [{d.deps.join(', ')}]</span>
                  </div>
          ...
          {run.constraintsHit.length > 0 && (
            <RunSection title="Constraints Hit" count={run.constraintsHit.length} color="emerald">
              {run.constraintsHit.map((c, i) => (
                <div key={i} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="text-emerald-600 dark:text-emerald-400">{c.id}</span>
                  <span className="rounded bg-zinc-200 px-1 py-px text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400">
                    p{c.priority}
                  </span>
                  <span className="text-zinc-400">deps: [{c.deps.join(', ')}]</span>
                </div>
              ))}
            </RunSection>
          )}
          ...
          {(run.requirementsAdded.length > 0 || run.requirementsRemoved.length > 0) && (
            <RunSection title="Requirements" count={run.requirementsAdded.length + run.requirementsRemoved.length} color="amber">
              {run.requirementsAdded.map((r, i) => (
                <div key={`a-${i}`} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="rounded bg-emerald-100 px-1 py-px text-[9px] text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">+</span>
              ...
              {run.requirementsRemoved.map((r, i) => (
                <div key={`r-${i}`} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="rounded bg-red-100 px-1 py-px text-[9px] text-red-700 dark:bg-red-900/30 dark:text-red-400">−</span>
          ...
          {/* Resolvers */}
          {(run.resolversStarted.length > 0 || run.resolversCompleted.length > 0 || run.resolversErrored.length > 0) && (
            <RunSection title="Resolvers" count={run.resolversStarted.length} color="indigo">
              {run.resolversCompleted.map((r, i) => (
                <div key={`c-${i}`} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="text-emerald-500">✓</span>
                  <span className="text-indigo-600 dark:text-indigo-400">{r.resolver}</span>
                  <span className="text-zinc-400">{r.duration.toFixed(0)}ms</span>
                </div>
              ))}
              ...
              {run.resolversStarted
                .filter((s) => !run.resolversCompleted.some((c) => c.resolver === s.resolver) && !run.resolversErrored.some((e) => e.resolver === s.resolver))
                .map((r, i) => (
                  <div key={`s-${i}`} className="flex items-center gap-2 font-mono text-[10px]">
          ...
          {run.effectsRun.length > 0 && (
            <RunSection title="Effects" count={run.effectsRun.length} color="fuchsia">
              {run.effectsRun.map((e, i) => (
                <div key={i} className="flex items-center gap-2 font-mono text-[10px]">
                  <span className="text-fuchsia-600 dark:text-fuchsia-400">{e.id}</span>
                  <span className="text-zinc-400">triggered by [{e.triggeredBy.join(', ')}]</span>
                </div>
              ))}
          ...
          {run.anomalies && run.anomalies.length > 0 && (
            <div className="rounded border border-amber-200 bg-amber-50 px-2 py-1.5 dark:border-amber-800/30 dark:bg-amber-900/10">
              <div className="text-[10px] font-semibold text-amber-700 dark:text-amber-400">Anomalies</div>
              {run.anomalies.map((a, i) => (
                <div key={i} className="font-mono text-[10px] text-amber-600 dark:text-amber-400">{a}</div>
              ))}
            </div>
          )}
        </div>
...
function RunSection({ title, count, color, children }: { title: string; count: number; color: string; children: React.ReactNode }) {
...
function ConstraintRow({
  ...
  const rowBg = isDisabled
    ...
    : c.active ? 'bg-emerald-50 dark:bg-emerald-900/10' : 'opacity-60'
      ...
      <div className="flex items-center gap-3 px-2 py-1.5">
        ...
        <span className={
          isDisabled
            ...
            : c.active ? 'text-emerald-700 dark:text-emerald-400' : 'text-zinc-500 dark:text-zinc-500'
        }>
          {c.id}
        </span>
        {isDisabled && (
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            aria-label={isDisabled ? `Enable constraint ${c.id}` : `Disable constraint ${c.id}`}
          ...
          {deps && (
            <button
              ...
              aria-label={isExpanded ? `Hide deps for ${c.id}` : `Show deps for ${c.id}`}
...
function RequirementRow({
  ...
  const resolverFromHistory = useMemo(() => {
    ...
    const started = latestRun.resolversStarted.find((s) => s.requirementId === r.id)
    if (started) {
      return started.resolver
    }

    // Fallback: heuristic match on type
    return Object.keys(resolverStats).find((key) => {
  ...
  const resolverError = useMemo(() => {
    ...
    const errored = latestRun.resolversErrored.find((e) => e.resolver === resolverFromHistory)
  ...
  return (
    <div className={`rounded font-mono text-xs ${bgClass}`}>
      <div className="flex items-center gap-3 px-2 py-1.5">
        ...
        <span className={`rounded px-1 py-px text-[9px] font-semibold uppercase leading-none ${
          isInflight
              ...
              <>
                ...
                {resolverStats[resolverFromHistory] && (
                  <span className="text-zinc-400">
                    ({resolverStats[resolverFromHistory].count} runs, {resolverStats[resolverFromHistory].errors} errors)
                  </span>
                )}
              </>
            )}
  ...
  return (
    <div className="rounded bg-indigo-50 font-mono text-xs dark:bg-indigo-900/10">
      <div className="flex items-center gap-3 px-2 py-1.5">
        ...
        {successRate != null && (
          <span className={`text-[10px] ${successRate === 100 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
      ...
      {isExpanded && latestError && (
        <div className="border-t border-indigo-200 px-3 py-1.5 dark:border-indigo-800/30">
          <div className="text-[10px] text-red-600 dark:text-red-400">{latestError}</div>
        </div>
      )}
    </div>
  )
...
export function PipelineView() {
  ...
  const latestRun = useSelector(system, (s) => s.derive.runtime.latestRun) as RunChangelogEntry | null
  const runCount = useSelector(system, (s) => s.derive.runtime.runCount) as number
  ...
  const handleToggleDisable = useCallback((c: Constraint) => {
    try {
  ...
  const filteredRequirements = useMemo(() =>
    filter ? allRequirements.filter((r) => r.type.toLowerCase().includes(lowerFilter) || r.fromConstraint.toLowerCase().includes(lowerFilter)) : allRequirements,
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      {!runHistoryEnabled && (
        <div className="rounded border border-dashed border-zinc-200 px-3 py-2 text-center font-mono text-[10px] text-zinc-400 dark:border-zinc-700 dark:text-zinc-500">
          Enable <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-800">runHistory: true</code> in debug config for pipeline timeline
        </div>
      )}
