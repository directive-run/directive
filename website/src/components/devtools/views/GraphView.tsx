...
import { formatCost, getDefaultPricing } from "../constants";
...
function TaskNode({ data, selected }: NodeProps) {
  ...
  const color =
    status === "running" ? "#8b5cf6" : (STATUS_COLORS[status] ?? "#8b5cf6");
...
function enrichFromAgentEvents(
  ...
  for (const e of events) {
    ...
    const agent =
      e.agentId ??
      ((e as Record<string, unknown>).taskId as string | undefined);
    if (!agent) {
      continue;
    }
    ...
    if (e.type === "agent_start") {
      ...
      matched.status = "completed";
      matched.isTask = true;
      matched.lastDurationMs =
        ((e as Record<string, unknown>).durationMs as number) ?? 0;
      ...
      matched.isTask = true;
      matched.lastError =
        ((e as Record<string, unknown>).error as string) ?? "Unknown error";
      matched.lastGuardrailName =
        ((e as Record<string, unknown>).guardrailName as string) ?? null;
      matched.lastGuardrailType =
        ((e as Record<string, unknown>).guardrailType as string) ?? null;
    } else if (e.type === "task_progress") {
      matched.isTask = true;
      matched.progress =
        ((e as Record<string, unknown>).percent as number) ?? null;
    }
  }
}
...
function buildDagGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
...
function buildParallelGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
  ...
  nodes.set(
    "__input",
    emptyNode("__input", { label: "Input", isVirtual: true }),
  );
  for (const id of agentIds) {
    nodes.set(
      id,
      emptyNode(id, { deps: ["__input"], isTask: expected?.taskIds.has(id) }),
    );
  }
  nodes.set(
    "__merge",
    emptyNode("__merge", { label: "Merge", isVirtual: true, deps: agentIds }),
  );
...
function buildSequentialGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
  const expected = extractExpectedHandlers(events);
  const agentIds = expected
    ? expected.handlers
    : extractAgentIdsOrdered(events);
...
function truncateLabel(prefix: string, text: string, maxLen = 30): string {
  ...
  const clean =
    lastSpace > available / 2 ? truncated.slice(0, lastSpace) : truncated;
...
function buildSupervisorGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
  ...
  const completions = events.filter(
    (e) =>
      (e.type === "agent_complete" || e.type === "agent_error") && e.agentId,
  );
...
function buildRaceGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
  ...
  nodes.set(
    "__output",
    emptyNode("__output", { label: "Output", isVirtual: true, deps: agentIds }),
  );
...
function buildReflectGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
...
function buildDebateGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
  const nodes = new Map<string, GraphNodeState>();
  ...
  const lastNodeId =
    prevJudgeNodeId ??
    (roundDebaters.length > 0 ? roundDebaters[roundDebaters.length - 1] : null);
  nodes.set(
    "__output",
    emptyNode("__output", {
      label: "Output",
...
function buildGoalGraph(events: DebugEvent[]): {
  nodes: Map<string, GraphNodeState>;
  edges: GraphEdgeState[];
} {
...
function extractExpectedHandlers(
  events: DebugEvent[],
): { handlers: string[]; taskIds: Set<string> } | null {
  const patternStart = events.find((e) => e.type === "pattern_start");
  if (!patternStart) {
    return null;
  }

  const handlers = (patternStart as Record<string, unknown>).handlers as
    | string[]
    | undefined;
  const taskIds = (patternStart as Record<string, unknown>).taskIds as
    | string[]
    | undefined;
...
function extractAgentIds(events: DebugEvent[]): string[] {
  ...
  for (const e of events) {
    const id =
      e.agentId ??
      (e.type === "task_start"
        ? ((e as Record<string, unknown>).taskId as string)
        : null);
    if (
      (e.type === "agent_start" || e.type === "task_start") &&
      id &&
      !seen.has(id)
    ) {
      seen.add(id);
...
function clusterEventsByTimestamp(
  events: DebugEvent[],
  thresholdMs = 50,
): PlaybackStep[] {
...
function buildStep(events: DebugEvent[]): PlaybackStep {
  ...
  for (const e of events) {
    const id =
      e.agentId ??
      ((e as Record<string, unknown>).taskId as string) ??
      ((e as Record<string, unknown>).nodeId as string);
    if (!id) {
      continue;
    }
    const action =
...
function replayPerRoundEnrichment(
  ...
  if (full.patternType === "supervisor") {
    const completions = agentEvents.filter(
      (e) => e.type === "agent_complete" || e.type === "agent_error",
    );
...
function buildGraphUpToStep(
  ...
  const isPerRoundPattern =
    full.patternType === "supervisor" ||
    full.patternType === "reflect" ||
    full.patternType === "debate";
  ...
  if (isPerRoundPattern) {
    ...
    if (outputNode) {
      const hasPatternComplete = partialEvents.some(
        (e) => e.type === "pattern_complete",
      );
...
function layoutGraph(
  ...
  const flowEdges: Edge[] = edges.map((edge, idx) => {
    ...
    return {
      ...
      style: {
        stroke:
          STATUS_COLORS[targetNode?.status ?? "pending"] ??
      ...
      labelStyle: edge.label
        ? { fill: "#a1a1aa", fontSize: 11, fontFamily: "inherit" }
        : undefined,
...
function CopyableBlock({
  ...
  content,
  maxHeight = "200px",
}: { label: string; content: string; maxHeight?: string }) {
  ...
  return (
    <div className="mt-3 border-t border-zinc-700 pt-3">
      ...
      <pre
        className="overflow-auto whitespace-pre-wrap rounded bg-zinc-800 px-2 py-1.5 text-[11px] leading-relaxed text-zinc-300"
        style={{ maxHeight }}
      >
        {content}
      </pre>
      ...
      <Panel position="top-left" className="!m-2 !z-30">
        <div className="flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-900/95 px-2 py-1.5 shadow-lg backdrop-blur-sm">
          ...
          <button
            ...
            title={
              showPending ? "Hide pending workers" : "Show pending workers"
            }
          >
            Pending
          </button>
...
export function GraphView() {
  ...
  const playbackSteps = useMemo(
    () => clusterEventsByTimestamp(activeEvents),
    [activeEvents],
  );
  ...
  const selected =
    hasGraph && selectedNode ? graphData.nodes.get(selectedNode) : null;
    ...
    <div className="flex w-80 flex-col gap-1 rounded-lg border border-zinc-700 bg-zinc-900/95 px-2 py-1.5 shadow-lg backdrop-blur-sm">
      ...
      {runs.length > 1 && (
        <div className="flex items-center justify-center gap-1.5">
          <button
            onClick={() => {
              playback.stop();
              setSelectedRun(Math.max(0, activeRunIndex - 1));
            }}
          ...
          <button
            onClick={() => {
              playback.stop();
              setSelectedRun(Math.min(runs.length - 1, activeRunIndex + 1));
            }}
      ...
      {showPlayback && (
        <PlaybackControls
          playback={playback}
          stepLabel={
            playback.step !== null && playbackSteps[playback.step]
              ? playbackSteps[playback.step].label
              : null
          }
        />
      )}
  ...
  if (!hasGraph) {
    return (
      <div
        className="-mx-4 -mt-4 -mb-4 flex flex-col"
        style={{ height: "calc(100% + 2rem)" }}
      >
        {controlPanel && (
          <div className="flex justify-end p-2">{controlPanel}</div>
        )}
  ...
  return (
    ...
    >
      <div className="flex-1">
        ...
        >
          ...
          <DrawingOverlay
            ...
            patternType={graphData?.patternType}
          />
      ...
      {selected && (
        <div
          className={`${panelExpanded ? "w-1/2" : "w-64"} shrink-0 overflow-auto border-l border-zinc-700 bg-zinc-900 p-3 transition-[width] duration-200`}
        >
          ...
          <div className="mt-3 space-y-2 text-[11px]">
            ...
            {!selected.isVirtual && (
              <>
                <DetailRow
                  label="Type"
                  value={selected.isTask ? "Task" : "Agent"}
                />
                ...
                {selected.isTask &&
                  ...
                  selected.progress >= 0 && (
                    <DetailRow
                      label="Progress"
                      value={`${Math.round(selected.progress)}%`}
                    />
                  )}
                ...
                {selected.lastDurationMs > 0 && (
                  <DetailRow
                    label="Last Duration"
                    value={`${selected.lastDurationMs}ms`}
                  />
                )}
                {!selected.isTask && selected.tokens > 0 && (
                  <DetailRow
                    label="Total Tokens"
                    value={selected.tokens.toLocaleString()}
                  />
                )}
                {!selected.isTask &&
                ...
                {!selected.isTask && selected.tokens > 0 && (
                  <DetailRow
                    ...
                    value={(() => {
                      ...
                      const cost =
                        (selected.inputTokens * pricing.input +
                          selected.outputTokens * pricing.output) /
                        1_000_000;
                ...
                {selected.lastError && (
                  <div className="flex justify-between">
                    ...
                    <span
                      className="min-w-0 truncate text-red-400"
                      title={selected.lastError}
                    >
                      {selected.lastError}
                    </span>
                  </div>
                ...
                {selected.lastGuardrailName && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-amber-400">
                      {selected.lastGuardrailName} ({selected.lastGuardrailType}
                      )
                    </span>
                  </div>
                )}
          ...
          {/* Agent input */}
          {selected.lastInput && (
            <CopyableBlock label="Input" content={selected.lastInput} />
          )}
