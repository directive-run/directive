...
export function CostBudgetView() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div
        className="mb-4 flex gap-1 rounded bg-zinc-100 p-0.5 dark:bg-zinc-800"
        role="radiogroup"
...
function CostSection({
  ...
  const {
    ...
    const agentList = Array.from(agentMap.entries()).map(([agent, data]) => ({
      agent,
      ...data,
    }));
  ...
  const estimateAgentCost = (a: {
    ...
    return (
      (a.inputTokens / 1_000_000) * p.input +
      (a.outputTokens / 1_000_000) * p.output
    );
  };
  ...
  const barSummary = agents
    .map(
      (a) =>
        `${a.agent}: ${totalTokens > 0 ? ((a.tokens / totalTokens) * 100).toFixed(0) : 0}%`,
    )
    .join(", ");
        ...
        <div>
          ...
          <div className="text-lg font-semibold text-zinc-900 dark:text-white">
            {completeEvents.length > 0
              ? Math.round(totalTokens / completeEvents.length).toLocaleString()
              : 0}
          </div>
        </div>
        {modelIds.length > 1 && (
          ...
          {agents.map((a) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-1.5 text-right">
                  {totalTokens > 0
                    ? ((a.tokens / totalTokens) * 100).toFixed(0)
                    : 0}
                  %
                </td>
              </tr>
          ...
          <tbody>
            {modelIds.map((mid) => {
              ...
              return (
                ...
                >
                  <td
                    className="py-1.5 font-mono text-zinc-700 dark:text-zinc-300"
                    title={mid}
                  >
                    {known?.label ?? mid}
                  </td>
                  <td className="py-1.5 text-right">
                    <input
                      ...
                      value={p.input}
                      onChange={(e) =>
                        updateModelPrice(mid, "input", Number(e.target.value))
                      }
                  ...
                  <td className="py-1.5 text-right">
                    <input
                      ...
                      value={p.output}
                      onChange={(e) =>
                        updateModelPrice(mid, "output", Number(e.target.value))
                      }
...
function BudgetSection({ getPricing }: BudgetSectionProps) {
  ...
  const budgets = data?.config.budgets ?? [
    { window: "hour", maxCost: 5.0 },
    { window: "day", maxCost: 50.0 },
  ];
  ...
  const hourlyLimit = budgets.find((b) => b.window === "hour")?.maxCost ?? 5.0;
  const dailyLimit = budgets.find((b) => b.window === "day")?.maxCost ?? 50.0;

  const { filteredEvents, filteredTotalCost, filteredTotalTokens } =
    useMemo(() => {
      ...
      for (const e of filtered) {
        fCost +=
          ((e.inputTokens ?? 0) / 1_000_000) * pricing.input +
          ((e.outputTokens ?? 0) / 1_000_000) * pricing.output;
      ...
      if (sortBy === "cost") {
        sorted.sort((a, b) => {
          const costA =
            (a.inputTokens ?? 0) * pricing.input +
            (a.outputTokens ?? 0) * pricing.output;
          const costB =
            (b.inputTokens ?? 0) * pricing.input +
            (b.outputTokens ?? 0) * pricing.output;
      ...
      return {
        filteredEvents: sorted.slice(0, 30),
        filteredTotalCost: fCost,
        filteredTotalTokens: fTokens,
      };
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-6 text-xs">
        ...
        <div>
          ...
          <div
            className={`text-lg font-semibold ${hourlyCost > hourlyLimit * 0.8 ? "text-amber-600 dark:text-amber-400" : "text-zinc-900 dark:text-white"}`}
          >
        ...
        <div>
          ...
          <div
            className={`text-lg font-semibold ${dailyCost > dailyLimit * 0.8 ? "text-amber-600 dark:text-amber-400" : "text-zinc-900 dark:text-white"}`}
          >
          ...
          <div key={b.window}>
            <div className="mb-1 flex items-center justify-between text-xs">
              <span className="font-medium capitalize text-zinc-700 dark:text-zinc-300">
                {b.window === "day" ? "Daily" : `${b.window}ly`} budget
              </span>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-zinc-400 dark:text-zinc-500">
                      {e.totalTokens?.toLocaleString()} tokens
                    </span>
                  </div>
          ...
          {filteredEvents.length > 0 && (
            <div className="flex items-center justify-between border-t border-zinc-200 pt-1.5 font-mono text-[11px] dark:border-zinc-700">
              <div className="flex items-center gap-2">
                ...
                <span className="text-zinc-400 dark:text-zinc-500">
                  {filteredTotalTokens.toLocaleString()} tokens
                </span>
              </div>
