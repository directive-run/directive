...
> = {
  ...
  "claude-haiku-4-5-20251001": { input: 0.8, output: 4.0, label: "Haiku 4.5" },
  "claude-sonnet-4-5": { input: 3.0, output: 15.0, label: "Sonnet 4.5" },
  "claude-sonnet-4-5-20250514": {
    input: 3.0,
    output: 15.0,
    label: "Sonnet 4.5",
  },
  "claude-opus-4": { input: 15.0, output: 75.0, label: "Opus 4" },
  "claude-opus-4-20250514": { input: 15.0, output: 75.0, label: "Opus 4" },
...
export const GUARDRAIL_INFO: Record<
  string,
  { description: string; type: "input" | "output"; icon: string }
> = {
  "rate-limit": {
    description:
      "Limits requests per minute to prevent abuse and control costs.",
    type: "input",
    icon: "⏱",
  },
  "prompt-injection": {
    description:
      "Detects prompt injection attempts that try to override system instructions.",
    type: "input",
    icon: "🛡",
  },
  "pii-detection": {
    description:
      "Scans for personally identifiable information and redacts it before processing.",
    type: "input",
    icon: "🔒",
  },
  length: {
    description:
      "Ensures responses stay within the configured character limit.",
    type: "output",
    icon: "📏",
  },
};
...
export function getDefaultPricing(modelId: string | null): {
  input: number;
  output: number;
} {
  if (!modelId) {
