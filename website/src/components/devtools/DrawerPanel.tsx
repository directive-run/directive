...
export function DrawerPanel({ children }: DrawerPanelProps) {
  ...
  const position = useSelector(system, (s) => s.facts.shell.drawerPosition) as
    | "bottom"
    | "right";
  ...
  const effectiveHeight = isMobile
    ? Math.min(
        height,
        typeof window !== "undefined" ? window.innerHeight * 0.7 : height,
      )
    : height;
        ...
        <div className="flex items-center gap-1">
          ...
          {timeTravelEnabled && (
            <>
              {snapshotCount > 0 && (
                <span
                  className="mr-0.5 font-mono text-[10px] text-zinc-400 dark:text-zinc-500"
                  role="status"
                  aria-label={`Snapshot ${snapshotIndex + 1} of ${snapshotCount}`}
              ...
              >
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M11.354 3.646a.5.5 0 0 1 0 .708L7.707 8l3.647 3.646a.5.5 0 0 1-.708.708l-4-4a.5.5 0 0 1 0-.708l4-4a.5.5 0 0 1 .708 0z"
                    clipRule="evenodd"
                  />
                </svg>
              </button>
              ...
              >
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M4.646 3.646a.5.5 0 0 1 .708 0l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L8.293 8 4.646 4.354a.5.5 0 0 1 0-.708z"
                    clipRule="evenodd"
                  />
                </svg>
              </button>
...
function ResizeHandle({ position }: { position: "bottom" | "right" }) {
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (isBottom) {
        system.events.shell.setDrawerSize({
          height: Math.round(newSize),
          width: system.facts.shell.drawerWidth,
        });
      } else {
        system.events.shell.setDrawerSize({
          height: system.facts.shell.drawerHeight,
    ...
    (e: React.KeyboardEvent) => {
      ...
      if (isBottom && (e.key === "ArrowUp" || e.key === "ArrowDown")) {
        ...
        const newHeight = Math.max(
          200,
          Math.min(currentHeight + delta, window.innerHeight * 0.8),
        );
        system.events.shell.setDrawerSize({
        ...
        const newWidth = Math.max(
          320,
          Math.min(currentWidth + delta, window.innerWidth * 0.8),
        );
        system.events.shell.setDrawerSize({
  ...
  return (
    <div
      ...
      aria-valuenow={
        isBottom
          ? system.facts.shell.drawerHeight
          : system.facts.shell.drawerWidth
      }
      tabIndex={0}
