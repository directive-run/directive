...
export function DevToolsContent({ mode = "standalone" }: DevToolsContentProps) {
  ...
  const exhaustedRetries = useSelector(
    system,
    (s) => s.derive.connection.exhaustedRetries,
  );
    ...
    () =>
      events.some(
        (e: DebugEvent) =>
          ...
          (e as Record<string, unknown>).patternType === "goal",
      ),
    [events],
  );

  // Read system trace event count for Timeline visibility
  const traceEventCount = useSelector(
    system,
    (s) => s.derive.runtime.traceEventCount ?? 0,
  );
  ...
  useEffect(() => {
    if (
      ...
      !visibleViews.includes(view as (typeof visibleViews)[number])
    ) {
  ...
  const factBreakpointHitCount = useSelector(
    system,
    (s) => s.derive.runtime.factBreakpointHitCount ?? 0,
  );
  const eventBreakpointHitCount = useSelector(
    system,
    (s) => s.derive.runtime.eventBreakpointHitCount ?? 0,
  );
  ...
  const tabBadges: Partial<Record<string, number>> = useMemo(() => {
    ...
    eventCount,
    traceEventCount,
    factBreakpointHitCount,
    eventBreakpointHitCount,
  ]);
  ...
  const handleTabKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLButtonElement>) => {
      ...
      if (e.key === "ArrowRight" || e.key === "ArrowDown") {
        ...
        e.preventDefault();
        const prev =
          visibleViews[(idx - 1 + visibleViews.length) % visibleViews.length];
        system.events.shell.setView({ view: prev });
        const container = e.currentTarget.parentElement;
        const buttons =
          container?.querySelectorAll<HTMLButtonElement>('[role="tab"]');
        buttons?.[
          (idx - 1 + visibleViews.length) % visibleViews.length
        ]?.focus();
      }
    },
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > MAX_IMPORT_SIZE) {
        console.warn(
          `[DevTools] Import file too large (${(file.size / 1024 / 1024).toFixed(1)} MB, max 10 MB)`,
        );
      ...
      reader.onload = () => {
        try {
          ...
          if (!valid) {
            console.warn(
              "[DevTools] Import contains invalid events (each must have type:string and timestamp:number)",
            );
      ...
      className={`flex flex-col overflow-hidden ${
        isDrawer
          ...
          : `border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900 ${
              isFullscreen ? "fixed inset-0 rounded-none" : "h-full rounded-lg"
            }`
      }`}
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 bg-zinc-50 px-4 py-2 dark:border-zinc-700 dark:bg-zinc-800/50">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleClear}
              aria-label={
                confirmClear ? "Confirm clear all events" : "Clear all events"
              }
...
function DevToolsTabBar({
  ...
  const system = useDevToolsSystem();
  const isSystemView = (v: string) =>
    (SYSTEM_VIEWS as readonly string[]).includes(v);
    ...
    <div className="relative border-b border-zinc-200 dark:border-zinc-700">
      ...
      >
        {visibleViews.map((v, i) => {
          ...
          const needsDivider =
            prevView && isSystemView(prevView) && !isSystemView(v);
          ...
          return (
            <div key={v} className="flex shrink-0 items-stretch">
              ...
              <button
                ...
                onClick={() =>
                  system.events.shell.setView({
                    view: v as (typeof VIEWS)[number],
                  })
                }
              >
                {v}
...
function DevToolsFooter() {
  const system = useDevToolsSystem();
  const eventCount = useSelector(system, (s) => s.derive.connection.eventCount);
  const totalTokens = useSelector(
    system,
    (s) => s.derive.connection.totalTokens,
  );
  ...
  const isPaused = useSelector(system, (s) => s.facts.connection.isPaused);
  const pausedOnEvent = useSelector(
    system,
  ...
  const runtimeSystemName = useSelector(
    system,
