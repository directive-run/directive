...
export const ROTATION_POOL_IDS = [
  "default",
  "A",
  "B",
  "C",
...
export const ALL_COLOR_IDS: string[] = [
  DEFAULT_COLOR_PRESET.id,
  ...COLOR_PRESETS.map((p) => p.id),
];
export const ALL_TYPO_IDS: number[] = [
  DEFAULT_TYPO_PRESET.id,
  ...TYPO_PRESETS.map((p) => p.id),
];
...
export function getRandomPreset(): { colorId: string; typoId: number } {
  ...
  const colorId =
    ROTATION_POOL_IDS[Math.floor(Math.random() * ROTATION_POOL_IDS.length)];
...
export function applyTypoPreset(preset: TypoPreset) {
  ...
  root.setProperty(
    "--brand-font-display",
    preset.display.cssVar
      ? `var(${preset.display.cssVar})`
      : preset.display.family,
  );
  root.setProperty(
    "--brand-font-body",
    preset.body.cssVar ? `var(${preset.body.cssVar})` : preset.body.family,
  );
  root.setProperty(
    "--brand-font-code",
    preset.code.cssVar ? `var(${preset.code.cssVar})` : preset.code.family,
  );
}
