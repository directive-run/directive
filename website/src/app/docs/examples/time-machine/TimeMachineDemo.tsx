    ...
    <div className="space-y-8">
      <section>
        ...
        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
          ...
          Export to save state as JSON, or use Changesets to group
          multiple strokes into a single undo step.
        </p>
      </section>
      ...
      <section>
        ...
        <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
          ...
          <ol className="list-inside list-decimal space-y-2 pl-1">
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Undo / Redo</strong>{' '}
              &ndash; Navigate through snapshot history with{' '}
              <code>goBack()</code> and <code>goForward()</code>
            </li>
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Export / Import</strong>{' '}
              &ndash; Serialize all snapshots to JSON and restore them later
            </li>
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Replay</strong>{' '}
              &ndash; Animate through the entire history to see strokes
              appear progressively
            </li>
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Changesets</strong>{' '}
              &ndash; Group multiple mutations into a single atomic undo
              step using <code>beginChangeset()</code> /{' '}
              <code>endChangeset()</code>
            </li>
          </ol>
        </div>
      </section>
