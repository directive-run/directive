    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
          Click &ldquo;Start Chain&rdquo; to begin. Adjust failure rate
          sliders to see error propagation and retry behavior. Each step
          only runs after its predecessor succeeds.
        </p>
      </section>
      ...
      <section>
        ...
        <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
          <p>
            Three modules form an async chain using <code>after</code>{' '}
            ordering: auth validates the session, permissions loads after
            auth succeeds, and dashboard loads after permissions.
          </p>
          <ol className="list-inside list-decimal space-y-2 pl-1">
            <li>
              <strong className="text-slate-900 dark:text-slate-200">after ordering</strong>{' '}
            ...
            <li>
              <strong className="text-slate-900 dark:text-slate-200">crossModuleDeps</strong>{' '}
              &ndash; each step reads facts from its predecessor to check
              success (<code>auth.isValid</code>, <code>permissions.role</code>)
            </li>
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Error propagation</strong>{' '}
              &ndash; if auth fails, permissions never evaluates (its{' '}
              <code>after</code> dependency is in rejected state), and
              dashboard is doubly blocked
            </li>
            <li>
              <strong className="text-slate-900 dark:text-slate-200">Retry</strong>{' '}
              &ndash; auth uses <code>retry: &#123; attempts: 2, backoff:
              &apos;exponential&apos; &#125;</code>. Restarting auth
              automatically resumes the chain from where it left off
            </li>
          </ol>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
          <p>
            ...
            with configurable failure rates, retry with exponential backoff,
            and visual chain status.
          </p>
          <p>
            <strong className="text-slate-900 dark:text-slate-200">How:</strong>{' '}
            Each module&rsquo;s constraint uses <code>after</code> to depend
            on the previous step&rsquo;s constraint, plus{' '}
            <code>crossModuleDeps</code> to read success state. The logging
            and devtools plugins trace the full chain execution.
          </p>
          <p>
            <strong className="text-slate-900 dark:text-slate-200">Why it works:</strong>{' '}
            <code>after</code> provides hard ordering guarantees without
            manual promise chaining. Error propagation is automatic &ndash;
            downstream steps simply never evaluate when upstream fails.
            Retrying a single step resumes the entire chain.
          </p>
        </div>
      </section>
