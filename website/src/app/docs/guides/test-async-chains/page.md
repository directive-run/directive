...
describe('auth flow', () => {
  it('logs in, fetches user, then sets authenticated', async () => {
    ...
    // Trigger login
    system.events.auth.requestLogin({ email: 'test@example.com', password: 'pass' });
  ...
  it('handles login failure', async () => {
    ...
    await system.start();
    system.events.auth.requestLogin({ email: 'bad@example.com', password: 'wrong' });
    await system.settle();
...
- [Loading & Error States](/docs/guides/loading-states) – what you're testing
