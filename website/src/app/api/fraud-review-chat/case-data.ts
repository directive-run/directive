  ...
  "acct-2002": [
    {
      ...
      risk: 70,
      detail: "Recent credential reset, new device",
    },
    {
      source: "geo-risk",
      risk: 85,
      detail: "Transactions from 4 countries in 24h",
    },
    ...
    events: [
      {
        ...
        memo: "laptop purchase ref SSN [REDACTED]",
        amount: 2499.99,
...
export function analyzeWithFormula(fraudCase: FraudCase): {
  ...
  const riskScore = Math.min(
    100,
    Math.round(avgSignalRisk * 0.5 + amountFactor + eventFactor),
  );
...
export function buildCaseSummary(scenarioKey: string): string {
  ...
  for (const acctId of accountIds) {
    const acctSignals =
      scenario.enrichment[acctId] ?? getMockEnrichment(acctId);
  ...
  // Format transactions
  const txnLines = scenario.events
    .map((e, i) => {
  ...
  const signalLines = signals
    .map((s) => `  - ${s.source}: risk ${s.risk} — ${s.detail}`)
    .join("\n");
