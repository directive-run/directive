...
const g = globalThis as typeof globalThis & { [GLOBAL_KEY]?: FraudReviewOrchestratorInstance }
...
export function getFraudReviewOrchestrator(): FraudReviewOrchestratorInstance | null {
  ...
  const audit = createAuditTrail({ maxEntries: 5000, sessionId: 'fraud-review-demo' })
...
{ "action": "complete", "report": "<your compiled findings summary>" }`,
      ...
      'transaction-analyst': {
        agent: {
          ...
          model: 'claude-haiku-4-5-20251001',
          instructions: 'You are a transaction pattern analyst. Analyze timing, amounts, merchant categories, velocity patterns, and spending anomalies. Look for unusual clustering, escalation patterns, structuring, and merchant category shifts. Respond in 2-3 sentences with specific observations.',
        },
      ...
      'geo-analyst': {
        agent: {
          name: 'geo-analyst',
          model: 'claude-haiku-4-5-20251001',
          instructions: 'You are a geographic risk analyst. Analyze location patterns, impossible travel scenarios, high-risk jurisdictions, and geographic consistency. Evaluate whether the physical movement of transactions is plausible. Respond in 2-3 sentences with specific geographic findings.',
        },
      ...
      'identity-analyst': {
        agent: {
          ...
          model: 'claude-haiku-4-5-20251001',
          instructions: 'You are an identity and PII analyst. Analyze PII exposure in transaction data, account verification signals, identity consistency across transactions, and name/account matching. Flag any personally identifiable information found in transaction memos. Respond in 2-3 sentences with specific identity findings.',
        },
    ...
    resolvers: {
      pauseForReview: {
        ...
        resolve: () => {
          console.log('[fraud-review-orchestrator] Token overload — pausing for review (demo no-op)')
        },
      },
      resetPipeline: {
        requirement: (req): req is { type: 'RESET_PIPELINE' } => req.type === 'RESET_PIPELINE',
        resolve: () => {
          console.log('[fraud-review-orchestrator] All agents errored — resetting pipeline (demo no-op)')
        },
      },
    },
