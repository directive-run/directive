...
import { getDataPipelineOrchestrator, getDataPipelineInputGuardrails } from './orchestrator-singleton'
...
export async function POST(request: Request) {
      ...
      async start(controller) {
        ...
        try {
          const result = await runner({
            ...
            instructions: 'You are a data analysis assistant. Classify the topic, analyze the data, and produce a structured report. Respond in 3-5 paragraphs with clear headings.',
          }, message)
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const result = await orchestrator.runPattern<string>('process', message)
