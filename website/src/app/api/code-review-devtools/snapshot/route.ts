...
export async function GET(request: Request) {
  ...
  if (!timeline || !instance) {
    return Response.json(
      { error: 'No active code review orchestrator. Submit code for review first.' },
      { status: 503 },
    )
  }
  ...
  return Response.json({
    ...
    supervisor: {
      ...
      workers: ['security-reviewer', 'style-reviewer', 'lint-check', 'dependency-audit', 'merge-decision'],
    ...
    config: {
      ...
      budgets: [{ window: 'hour', maxCost: 5.00 }],
