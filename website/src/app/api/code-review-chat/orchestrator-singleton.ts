...
const g = globalThis as typeof globalThis & { [GLOBAL_KEY]?: CodeReviewInstance }
...
const lintCheckTask: TaskRegistration = {
  run: async (input, _signal, context) => {
    ...
    const totalIssues = namingIssues.length + complexityIssues.length + errorIssues.length + importIssues.length
  ...
  label: 'Lint Check',
  description: 'Deterministic static analysis — naming, complexity, error handling, imports',
  maxConcurrent: 2,
}
...
const dependencyAuditTask: TaskRegistration = {
  run: async (input, _signal, context) => {
    ...
    const riskScore = Math.min(100, riskFactors.length * 30 + findings.length * 10)
  ...
  description: 'Scans for vulnerable patterns, insecure dependencies, license issues',
...
const mergeDecisionTask: TaskRegistration = {
  run: async (input, _signal, context) => {
    ...
    let lintScore = 50
    try {
      const lintOutput = lintState?.lastOutput ? JSON.parse(lintState.lastOutput) : null
    ...
    let depRiskScore = 0
    try {
      const depOutput = depState?.lastOutput ? JSON.parse(depState.lastOutput) : null
  ...
  description: 'Aggregates all review scores and computes final approve/reject verdict',
}
...
const inputGuardrails: NamedGuardrail<InputGuardrailData>[] = [
  {
    ...
    fn: (() => {
      ...
      return () => {
        ...
        while (startIdx < timestamps.length && timestamps[startIdx]! < windowStart) {
          startIdx++
        }
...
export function getCodeReviewOrchestrator(): CodeReviewInstance | null {
  ...
  runner = withBudget(runner, {
    budgets: [{ window: 'hour' as const, maxCost: 5.00, pricing: haikuPricing }],
  })
  ...
  const audit = createAuditTrail({ maxEntries: 5000, sessionId: 'code-review-demo' })
...
{ "action": "complete" }`,
      ...
      'style-reviewer': {
        agent: {
          name: 'style-reviewer',
          model: 'claude-haiku-4-5-20251001',
          instructions: 'You are a code style and readability reviewer. Evaluate naming conventions, function length, code organization, documentation quality, TypeScript best practices, and adherence to clean code principles. Respond in 2-3 sentences with specific observations and improvement suggestions.',
        },
    ...
    patterns: {
      codeReview: supervisor<string>(
        'supervisor',
        ['security-reviewer', 'style-reviewer', 'lint-check', 'dependency-audit', 'merge-decision'],
        {
          maxRounds: 8,
