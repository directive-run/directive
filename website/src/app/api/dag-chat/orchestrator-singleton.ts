...
const g = globalThis as typeof globalThis & { [GLOBAL_KEY]?: DagOrchestratorInstance }
  ...
  const orchestrator = createMultiAgentOrchestrator({
    ...
    patterns: {
      research: dag<string>(
        ...
        (context: DagExecutionContext) => String(context.outputs['reviewer'] ?? ''),
      ),
    },
    ...
    resolvers: {
      pauseForReview: {
        ...
        resolve: () => {
          console.log('[dag-orchestrator] Token overload — pausing for review (demo no-op)')
        },
      },
      resetPipeline: {
        requirement: (req): req is { type: 'RESET_PIPELINE' } => req.type === 'RESET_PIPELINE',
        resolve: () => {
          console.log('[dag-orchestrator] All agents errored — resetting pipeline (demo no-op)')
        },
      },
    },
