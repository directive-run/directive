...
import { getPitchDeckOrchestrator, getPitchDeckInputGuardrails } from './orchestrator-singleton'
...
export async function POST(request: Request) {
      ...
      async start(controller) {
        ...
        try {
          const result = await runner({
            ...
            instructions: 'You are a startup pitch deck evaluator. Analyze the startup idea and provide: 1) Market analysis (TAM/SAM/SOM, competition), 2) Financial projections (revenue, unit economics), 3) Investor narrative (problem, solution, why now), 4) Overall score 1-10 with justification. Respond in 3-5 paragraphs.',
          }, message)
          ...
          send({
            type: 'error',
            message: err instanceof Error ? err.message : 'Pitch evaluation failed',
          })
        } finally {
          try {
            controller.close()
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const result = await orchestrator.runPattern<string>('pitchDeck', message)
