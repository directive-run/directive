...
function Ce(e = [], r, l, s, i, o) {
  return {
    ...
    _refinements: o,
    validate(c) {
      return Ce([...e, c], r, l, s, i, o);
    },
  };
}
function ee(e, r, l, s, i, o) {
  return {
    ...Ce(e, r, l, s, i, o),
    default(c) {
      return ee(e, r, c, s, i, o);
    },
    transform(c) {
      return ee(
        [],
    ...
    brand() {
      return ee(e, `Branded<${r}>`, l, s, i, o);
    },
    describe(c) {
      return ee(e, r, l, s, c, o);
    },
    refine(c, d) {
      const h = [...(o ?? []), { predicate: c, message: d }];
      return ee([...e, c], r, l, s, i, h);
    },
    nullable() {
      return ee(
        [(c) => c === null || e.every((d) => d(c))],
        `${r} | null`,
        l,
        s,
        i,
...
var ie = {
  ...
  array() {
    const e = (r, l, s, i, o) => {
      ...
      return {
        ...
        nonEmpty() {
          return e([...r, (h) => h.length > 0], l, s, i, d);
        },
        maxLength(h) {
          return e([...r, (p) => p.length <= h], l, s, i, d);
        },
        minLength(h) {
          return e([...r, (p) => p.length >= h], l, s, i, d);
        },
  ...
  object() {
    const e = (r, l, s) => ({
      ...
      hasKeys(...i) {
        return e([...r, (o) => i.every((c) => c in o)], l, s);
      },
      default(i) {
        return e(r, i, s);
      },
...
function vt(e) {
  ...
  function m(f, b) {
    ...
    if (A(w)) {
      ...
      if (!P.success) {
        const v = b === null ? "null" : Array.isArray(b) ? "array" : typeof b,
          ...
          n = N(w);
        throw new Error(
          `[Directive] Validation failed for "${f}": expected ${n}, got ${v} ${C}. ${t}`,
        );
      }
      return;
    }
    ...
    for (let P = 0; P < F.length; P++) {
      ...
      if (typeof v == "function" && !v(b)) {
        ...
        throw new Error(
          `[Directive] Validation failed for "${f}": expected ${K}, got ${C} ${t}${a}${n}`,
        );
      }
    }
  }
  ...
  function E(f, b, w) {
    ...
    try {
      ...
      while (O.length > 0) {
        ...
        const F = [...O];
        O.length = 0;
        for (const K of F) l?.(K.key, K.value, K.prev), S(K.key);
        y();
      }
    } finally {
      D = !1;
      ...
      if ((s && B.length > 0 && s([...B]), j.size > 0)) {
        ...
        try {
          ...
          while (O.length > 0) {
            ...
            const b = [...O];
            O.length = 0;
            for (const w of b) l?.(w.key, w.value, w.prev), S(w.key);
            y();
          }
        } finally {
          D = !1;
  ...
  return (
    (z.registerKeys = (f) => {
      for (const b of Object.keys(f)) ve.has(b) || ((r[b] = f[b]), h.add(b));
    }),
    z
  );
}
...
function ye(e, r = 50) {
  ...
  function i(o, c) {
    ...
    if ((s.add(d), Array.isArray(d))) {
      for (const h of d) if (!i(h, c + 1)) return s.delete(d), !1;
      return s.delete(d), !0;
    }
    for (const h of Object.keys(d))
      if (l.has(h) || !i(d[h], c + 1)) return s.delete(d), !1;
    return s.delete(d), !0;
  }
  return i(e, 0);
}
...
function $t(e, r, l) {
  return { requirement: e, id: St(e, l), fromConstraint: r };
}
var qe = class ot {
    map = new Map();
    add(r) {
...
function Ct(e) {
    ...
    for (const [t, n] of Object.entries(r))
      if (n.after)
        for (const a of n.after)
          r[a] && (N.has(a) || N.set(a, new Set()), N.get(a).add(t));
  }
  function y() {
    const t = new Set(),
      n = new Set(),
    ...
    function g(x, I) {
      ...
      I.pop(), n.delete(x), t.add(x), a.push(x);
    }
    for (const x of Object.keys(r)) g(x, []);
    (T = a), (m = new Map(T.map((x, I) => [x, I])));
  }
  y(), S();
  function E(t, n) {
  ...
  function b(t) {
    ...
    try {
      let g;
      if (n.deps) (g = n.when(l)), j.set(t, new Set(n.deps));
      else {
        const x = Ee(() => n.when(l));
        (g = x.value), j.set(t, x.deps);
      }
      return g instanceof Promise
        ? (p.add(t),
          ...
          g
            .then(
              (x) => ((a.lastResult = x), (a.isEvaluating = !1), o?.(t, x), x),
            )
            .catch(
              (x) => (
  ...
  async function w(t) {
    ...
    try {
      const x = n.when(l),
        I = await xe(x, g, `Constraint "${t}" timed out after ${g}ms`);
  ...
  function K(t, n) {
    ...
    const a = u.get(t) ?? new Set();
    for (const g of n)
      a.add(g), $.has(g) || $.set(g, new Set()), $.get(g).add(t);
    u.set(t, a);
  }
  let P = null;
  function v() {
  ...
  function C(t) {
    ...
    for (const a of n.after)
      if (r[a] && !h.has(a) && !A.has(a) && !k.has(a)) return !1;
    return !0;
  }
  return {
    async evaluate(t) {
      const n = new qe();
      A.clear();
      let a = v().filter((_) => !h.has(_)),
        g;
      if (!O || !t || t.size === 0) (g = a), (O = !0);
      else {
        const _ = new Set();
        for (const U of t) {
          const Y = $.get(U);
        ...
        for (const U of B) h.has(U) || _.add(U);
        B.clear(), (g = [..._]);
        for (const U of a)
          if (!_.has(U)) {
            const Y = D.get(U);
      ...
      function x(_, U) {
        ...
        const Y = j.get(_);
        if (!U) {
          Y !== void 0 && f(_, Y), A.add(_), D.set(_, []);
          return;
        }
        A.delete(_);
        let te, Z;
      ...
      async function I(_) {
        ...
        if (X.length > 0) {
          ...
          for (const { id: J, active: G } of H) x(J, G);
        }
        if (Z.length > 0) {
          const H = await Promise.all(
      ...
      while (L.length > 0 && W > 0) {
        const _ = L.length;
        if (((L = await I(L)), L.length === _)) break;
        W--;
      }
      return n.all();
    },
    ...
    disable(t) {
      h.add(t), (P = null), D.delete(t);
      const n = u.get(t);
      if (n) {
        for (const a of n) {
          const g = $.get(a);
          g && (g.delete(t), g.size === 0 && $.delete(a));
        }
        u.delete(t);
      }
      j.delete(t);
    ...
    markResolved(t) {
      ...
      const a = N.get(t);
      if (a) for (const g of a) B.add(g);
    },
    isResolved(t) {
      return k.has(t);
    },
    registerDefinitions(t) {
      for (const [n, a] of Object.entries(t)) (r[n] = a), R(n), B.add(n);
      (P = null), y(), S();
    },
  };
}
function kt(e) {
  ...
  function N(y) {
    ...
    E.isComputing = !0;
    try {
      const { value: z, deps: f } = Ee(() => R(l, O));
      return (
        (E.cachedValue = z), (E.isStale = !1), T(y, f), s?.(y, z, [...f]), z
      );
    } catch (z) {
      throw (o?.(y, z), z);
    } finally {
  ...
  function S(y, E = new Set()) {
    if (E.has(y)) return;
    E.add(y);
    const R = c.get(y);
    if (!R || R.isStale) return;
    (R.isStale = !0), i?.(y), B.add(y);
    const z = p.get(y);
    if (z) for (const f of z) S(f, E);
  }
  return (
    (O = new Proxy(
      {},
    ...
    )),
    {
      get(y) {
        const E = A(y);
        return E.isStale && N(y), E.cachedValue;
      },
      isStale(y) {
        return c.get(y)?.isStale ?? !0;
      },
      invalidate(y) {
        const E = h.get(y);
        if (E) {
      ...
      registerDefinitions(y) {
        for (const [E, R] of Object.entries(y)) (r[E] = R), k(E);
      },
    }
  );
}
function Rt(e) {
  ...
  for (const k of Object.keys(r)) p(k);
  return {
    async runEffects(k) {
      const A = [];
      for (const N of Object.keys(r)) B(N, k) && A.push(N);
      await Promise.all(A.map(O)), (d = $());
    },
    async runAll() {
    ...
    cleanupAll() {
      h = !0;
      for (const k of c.values()) j(k);
    },
    registerDefinitions(k) {
      for (const [A, N] of Object.entries(k)) (r[A] = N), p(A);
    },
  };
}
function At(e = {}) {
  const {
    ...
    o = new Map();
  function c(d) {
    const h = r * Math.pow(s, d - 1);
    return Math.min(h, i);
  }
  return {
    scheduleRetry(d, h, p, u, $) {
      if (u > l) return null;
      const B = c(u),
        j = {
          source: d,
    ...
    processDueRetries() {
      const d = Date.now(),
        h = [];
      for (const [p, u] of o) u.nextRetryTime <= d && (h.push(u), o.delete(p));
      return h;
    },
    cancelRetry(d) {
      o.delete(d);
...
function jt(e = {}) {
  ...
  return {
    handleError(u, $, B, j) {
      ...
      let O = p(u, $, B instanceof Error ? B : new Error(String(B)));
      if (O === "retry-later") {
        const k = (d.get($) ?? 0) + 1;
        d.set($, k),
          c.scheduleRetry(u, $, j, k) ||
            ((O = "skip"), d.delete($), typeof process < "u");
      }
      try {
        s?.(D, O);
      } catch (k) {
...
function Dt() {
  ...
  return {
    ...
    emitFactSet(s, i, o) {
      for (const c of e) r(() => c.onFactSet?.(s, i, o));
    },
    emitFactDelete(s, i) {
    ...
    emitDerivationCompute(s, i, o) {
      for (const c of e) r(() => c.onDerivationCompute?.(s, i, o));
    },
    ...
    emitResolverComplete(s, i, o) {
      for (const c of e) r(() => c.onResolverComplete?.(s, i, o));
    },
    emitResolverError(s, i, o) {
      for (const c of e) r(() => c.onResolverError?.(s, i, o));
    },
    emitResolverRetry(s, i, o) {
      for (const c of e) r(() => c.onResolverRetry?.(s, i, o));
    },
    ...
    for (let P = 1; P <= F.attempts; P++) {
      ...
      try {
        ...
        if (M.resolve) {
          let n;
          s.batch(() => {
            n = M.resolve(b.requirement, C);
          });
          const a = M.timeout;
          a && a > 0
            ? await xe(n, a, `Resolver "${f}" timed out after ${a}ms`)
            : await n;
        }
    ...
    for (let t = 1; t <= M.attempts; t++) {
      ...
      try {
        ...
        if (w.resolveBatchWithResults) {
          ...
          if (
            (s.batch(() => {
              x = w.resolveBatchWithResults(a, n);
            }),
            C && C > 0
              ? (g = await xe(
                  x,
          ...
          if (!L || b.some((W, _) => g[_]?.success)) return;
        } else {
          let g;
          s.batch(() => {
...
function Mt(e) {
  ...
  function N(T) {
    ...
    s.batch(() => {
      for (const [m, S] of Object.entries(T)) {
  ...
  return {
    ...
    takeSnapshot(T) {
      ...
      const m = { id: u++, timestamp: Date.now(), facts: A(), trigger: T };
      for (
    ...
    goBack(T = 1) {
      ...
      let m = p,
        S = p,
        y = j.find((R) => p > R.startIndex && p <= R.endIndex);
      if (y) S = y.startIndex;
      else if (j.find((R) => p === R.startIndex)) {
        const R = j.find((z) => z.endIndex < p && p - z.endIndex <= T);
        S = R ? R.startIndex : Math.max(0, p - T);
      } else S = Math.max(0, p - T);
      if (m === S) return;
      p = S;
      const E = h[p];
      E && (this.restore(E), o?.(m, S));
    },
    goForward(T = 1) {
      if (!c || h.length === 0) return;
      let m = p,
        S = p,
        y = j.find((R) => p >= R.startIndex && p < R.endIndex);
      if ((y ? (S = y.endIndex) : (S = Math.min(h.length - 1, p + T)), m === S))
        return;
      p = S;
      const E = h[p];
      E && (this.restore(E), o?.(m, S));
    ...
    endChangeset() {
      !c ||
        D === null ||
        (p > O && j.push({ label: D, startIndex: O, endIndex: p }),
        (D = null),
        (O = -1));
    },
    pause() {
...
function lt(e) {
  ...
  let $ = jt({
    ...
    { store: O, facts: k } = bt({
      ...
      onChange: (t, n, a) => {
        u.emitFactSet(t, n, a),
          B(t),
          !D?.isRestoring && (h === 0 && (p = !0), w.changedKeys.add(t), M());
      },
      onBatch: (t) => {
        u.emitFactsBatch(t);
        const n = [];
  ...
  (B = (t) => A.invalidate(t)), (j = (t) => A.invalidateMany(t));
  const N = Rt({
      definitions: i,
      facts: k,
      store: O,
      ...
      onError: (t, n) => {
        $.handleError("effect", t, n), u.emitEffectError(t, n);
      },
    }),
    T = Ct({
      definitions: o,
      ...
      onError: (t, n) => {
        $.handleError("constraint", t, n), u.emitConstraintError(t, n);
      },
    }),
    m = It({
      definitions: c,
      ...
      onError: (t, n, a) => {
        $.handleError("resolver", t, a, n), u.emitResolverError(t, n, a);
      },
    ...
    if (!w.isReconciling) {
      ...
      try {
        w.changedKeys.size > 0 &&
          ...
          (p = !1));
        const t = k.$snapshot();
        u.emitReconcileStart(t), await N.runEffects(w.changedKeys);
        const n = new Set(w.changedKeys);
        w.changedKeys.clear();
        const a = await T.evaluate(n),
          g = new qe();
        for (const _ of a) g.add(_), u.emitRequirementCreated(_);
        const { added: x, removed: I } = g.diff(w.previousRequirements);
        ...
        const L = m.getInflightInfo(),
          W = {
            ...
            canceled: I.map((_) => ({
              id: _.id,
              resolverId: L.find((U) => U.id === _.id)?.resolverId ?? "unknown",
            })),
          };
  ...
  const K = new Proxy(
    ...
    v = {
      ...
      subscribe(t, n) {
        ...
        const x = [];
        return (
          a.length > 0 && x.push(A.subscribe(a, n)),
          g.length > 0 && x.push(O.subscribe(g, n)),
          () => {
            for (const I of x) I();
          }
        );
      ...
      explain(t) {
        ...
        const a = T.getState(n.fromConstraint),
          ...
          x = {},
          I = O.toObject();
        for (const [U, Y] of Object.entries(I)) x[U] = Y;
        const L = [
...
`)
      ...
      async settle(t = 5e3) {
        ...
        for (;;) {
          ...
          if (Date.now() - n > t) {
            ...
            const x = w.previousRequirements.all();
            throw (
              (x.length > 0 &&
                g.push(
                  `${x.length} unmet requirements: ${x.map((I) => I.requirement.type).join(", ")}`,
                ),
              new Error(
      ...
      getDistributableSnapshot(t = {}) {
        let {
          ...
          _ = Object.keys(s),
          U;
        if ((n ? (U = n.filter((Z) => _.includes(Z))) : (U = _), a)) {
          const Z = new Set(a);
      ...
      watchDistributableSnapshot(t, n) {
        let { includeDerivations: a, excludeDerivations: g } = t,
          x = Object.keys(s),
          I;
        if ((a ? (I = a.filter((W) => x.includes(W))) : (I = x), g)) {
          const W = new Set(g);
  ...
  function C(t) {
    ...
    Object.assign(r, t.schema),
      ...
      t.constraints &&
        (Object.assign(o, t.constraints), T.registerDefinitions(t.constraints)),
      t.resolvers &&
...
function Bt(e) {
  ...
  if (Array.isArray(o)) {
    const m = o,
      S = Object.keys(r).filter((y) => !m.includes(y));
    if (S.length > 0)
      throw new Error(
        `[Directive] initOrder is missing modules: ${S.join(", ")}. All modules must be included in the explicit order.`,
      );
    i = m;
  } else o === "declaration" ? (i = Object.keys(r)) : (i = Tt(r));
  let c = e.debug,
    d = e.errorBoundary;
  e.zeroConfig &&
  ...
  for (const m of Object.keys(r)) {
    if (m.includes(V))
      throw new Error(
        `[Directive] Module name "${m}" contains the reserved separator "${V}". Module names cannot contain "${V}".`,
      );
    const S = r[m];
    if (S) {
      for (const y of Object.keys(S.schema.facts))
        if (y.includes(V))
          throw new Error(
            `[Directive] Schema key "${y}" in module "${m}" contains the reserved separator "${V}". Schema keys cannot contain "${V}".`,
          );
    }
  }
  const h = [];
  for (const m of i) {
    const S = r[m];
    if (!S) continue;
    const y = S.crossModuleDeps && Object.keys(S.crossModuleDeps).length > 0,
      E = y ? Object.keys(S.crossModuleDeps) : [],
      R = {};
    for (const [v, C] of Object.entries(S.schema.facts)) R[`${m}${V}${v}`] = C;
    const z = {};
    if (S.schema.derivations)
      for (const [v, C] of Object.entries(S.schema.derivations))
        z[`${m}${V}${v}`] = C;
    const f = {};
    if (S.schema.events)
      for (const [v, C] of Object.entries(S.schema.events))
        f[`${m}${V}${v}`] = C;
    const b = S.init
        ? (v) => {
            const C = se(v, m);
      ...
      w = {};
    if (S.derive)
      for (const [v, C] of Object.entries(S.derive))
        w[`${m}${V}${v}`] = (t, n) => {
          const a = y ? ae(t, m, E) : se(t, m),
            g = Te(n, m);
          return C(a, g);
        };
    const M = {};
    if (S.events)
      for (const [v, C] of Object.entries(S.events))
        M[`${m}${V}${v}`] = (t, n) => {
          const a = se(t, m);
          C(a, n);
        };
    const F = {};
    if (S.constraints)
      for (const [v, C] of Object.entries(S.constraints)) {
        const t = C;
        F[`${m}${V}${v}`] = {
          ...t,
          deps: t.deps?.map((n) => `${m}${V}${n}`),
          when: (n) => {
            const a = y ? ae(n, m, E) : se(n, m);
            return t.when(a);
          },
          require:
    ...
    const K = {};
    if (S.resolvers)
      for (const [v, C] of Object.entries(S.resolvers)) {
        const t = C;
        K[`${m}${V}${v}`] = {
          ...t,
          resolve: async (n, a) => {
            const g = Ae(a.facts, r, () => Object.keys(r));
            await t.resolve(n, { facts: g[m], signal: a.signal });
          },
        };
      }
    const P = {};
    if (S.effects)
      for (const [v, C] of Object.entries(S.effects)) {
        const t = C;
        P[`${m}${V}${v}`] = {
          ...t,
          run: (n, a) => {
            const g = y ? ae(n, m, E) : se(n, m),
              x = a ? (y ? ae(a, m, E) : se(a, m)) : void 0;
            return t.run(g, x);
          },
          deps: t.deps?.map((n) => `${m}${V}${n}`),
        };
    ...
    h.push({
      ...
      hooks: S.hooks,
      snapshotEvents:
        s && !s.has(m) ? [] : S.snapshotEvents?.map((v) => `${m}${V}${v}`),
    });
  }
  let p = null,
    u = null;
  function $(m) {
    for (const [S, y] of Object.entries(m))
      if (!re.has(S) && l.has(S)) {
        if (y && typeof y == "object" && !ye(y))
          throw new Error(
            `[Directive] initialFacts/hydrate for namespace "${S}" contains potentially dangerous keys (__proto__, constructor, or prototype). This may indicate a prototype pollution attack.`,
          );
        for (const [E, R] of Object.entries(y))
          re.has(E) || (u.facts[`${S}${V}${E}`] = R);
      }
  }
  u = lt({
    modules: h.map((m) => ({
  ...
  for (const m of Object.keys(r)) {
    ...
    if (!S) continue;
    const y = [];
    for (const E of Object.keys(S.schema.facts)) y.push(`${m}${V}${E}`);
    if (S.schema.derivations)
      for (const E of Object.keys(S.schema.derivations)) y.push(`${m}${V}${E}`);
    B.set(m, y);
  }
  const j = { names: null };
  function D() {
  ...
  return {
    ...
    start() {
      if ((u.start(), T && T > 0)) {
        const m = Object.keys(h[0]?.events ?? {}).find((S) =>
          S.endsWith(`${V}tick`),
        );
        m &&
          (N = setInterval(() => {
    ...
    stop() {
      N && (clearInterval(N), (N = null)), u.stop();
    },
    destroy() {
      this.stop(), u.destroy();
    },
    ...
    subscribe(m, S) {
      ...
      return u.subscribe(y, S);
    },
    subscribeModule(m, S) {
      const y = B.get(m);
      return !y || y.length === 0 ? () => {} : u.subscribe(y, S);
    },
    watch(m, S, y) {
      return u.watch(ce(m), S, y);
    },
    when(m, S) {
      return u.when(() => m(O), S);
    },
    ...
    getDistributableSnapshot(m) {
      const S = {
        ...
        y = u.getDistributableSnapshot(S),
        E = {};
      for (const [R, z] of Object.entries(y.data)) {
        const f = R.indexOf(V);
        if (f > 0) {
          const b = R.slice(0, f),
            w = R.slice(f + V.length);
          E[b] || (E[b] = {}), (E[b][w] = z);
        } else E._root || (E._root = {}), (E._root[R] = z);
      }
      return { ...y, data: E };
    },
    watchDistributableSnapshot(m, S) {
      const y = {
        ...m,
        includeDerivations: m?.includeDerivations?.map(ce),
        excludeDerivations: m?.excludeDerivations?.map(ce),
    ...
    registerModule(m, S) {
      ...
      if (m.includes(V))
        throw new Error(
          `[Directive] Module name "${m}" contains the reserved separator "${V}".`,
        );
      if (re.has(m))
        throw new Error(
          `[Directive] Module name "${m}" is a blocked property.`,
        );
      for (const v of Object.keys(S.schema.facts))
        if (v.includes(V))
          throw new Error(
            `[Directive] Schema key "${v}" in module "${m}" contains the reserved separator "${V}".`,
          );
      const y = S,
      ...
      if (y.effects)
        for (const [v, C] of Object.entries(y.effects)) {
          ...
          K[`${m}${V}${v}`] = {
            ...t,
            run: (n, a) => {
              const g = E ? ae(n, m, R) : se(n, m),
                x = a ? (E ? ae(a, m, R) : se(a, m)) : void 0;
              return t.run(g, x);
            },
            deps: t.deps?.map((n) => `${m}${V}${n}`),
          };
        }
      l.add(m), (r[m] = y), (j.names = null);
      const P = [];
      for (const v of Object.keys(y.schema.facts)) P.push(`${m}${V}${v}`);
      ...
      B.set(m, P),
        u.registerModule({
          ...
          hooks: y.hooks,
          snapshotEvents:
            s && !s.has(m) ? [] : y.snapshotEvents?.map((v) => `${m}${V}${v}`),
        });
    },
  };
}
...
function se(e, r) {
  ...
  const s = new Proxy(
    {},
    {
      ...
      has(i, o) {
        return typeof o == "symbol" || re.has(o) ? !1 : `${r}${V}${o}` in e;
      },
      deleteProperty(i, o) {
        return typeof o == "symbol" || re.has(o)
          ? !1
...
function Te(e, r) {
  ...
  const s = new Proxy(
    ...
    {
      get(i, o) {
        if (typeof o != "symbol" && !re.has(o)) return e[`${r}${V}${o}`];
      },
      has(i, o) {
        return typeof o == "symbol" || re.has(o) ? !1 : `${r}${V}${o}` in e;
      },
...
function Pt(e, r, l) {
  let s = Xe.get(e);
  return (
    s || ((s = new Map()), Xe.set(e, s)),
    new Proxy(
      {},
      {
        get(i, o) {
          if (typeof o == "symbol" || re.has(o) || !Object.hasOwn(r, o)) return;
          const c = s.get(o);
          if (c) return c;
          const d = new Proxy(
            {},
        ...
        has(i, o) {
          return typeof o == "symbol" || re.has(o) ? !1 : Object.hasOwn(r, o);
        },
        ownKeys() {
          return l();
        },
...
function Lt(e) {
  ...
  if (e.tickMs !== void 0 && e.tickMs <= 0)
    throw new Error("[Directive] tickMs must be a positive number");
  if (e.initialFacts && !ye(e.initialFacts))
    throw new Error(
    ...
    onAfterModuleInit: () => {
      if (e.initialFacts)
        for (const [p, u] of Object.entries(e.initialFacts))
          re.has(p) || (o.facts[p] = u);
      if (i) {
        for (const [p, u] of Object.entries(i)) re.has(p) || (o.facts[p] = u);
        i = null;
      }
    },
  });
...
function Vt() {
  return { entries: new at(Kt), inflight: new Map() };
}
function Ht() {
  return {
...
function Xt(e, r, l, s) {
  ...
  (c.textContent = `[data-directive-devtools] summary:focus-visible{outline:2px solid ${q.accent};outline-offset:2px;border-radius:2px}[data-directive-devtools] button:focus-visible{outline:2px solid ${q.accent};outline-offset:2px}`),
  ...
  function N(H, J) {
    ...
    return (
      ue.appendChild(Le),
      G.appendChild(ue),
      { details: G, tbody: Le, countSpan: fe }
    );
  }
  function T(H, J) {
    const G = document.createElement("details");
  ...
  const S = N("Derivations", !1);
  h.appendChild(S.details);
  const y = T("Inflight", q.yellow);
  h.appendChild(y.details);
  const E = T("Unmet", q.red);
  ...
  d.addEventListener("click", _), $.addEventListener("click", U);
  function Y(H) {
    H.key === "Escape" && L && U();
  }
  ...
  function X() {
    ...
    W.clear(), d.remove(), h.remove(), c.remove();
  }
  return {
    refs: {
      container: h,
      ...
      derivBody: S.tbody,
      derivCount: S.countSpan,
      derivSection: S.details,
      inflightList: y.list,
...
function $e(e, r, l, s, i, o) {
  let c = ct(s),
    d = e.get(l);
  if (d) {
    const h = d.cells;
    if (h[1] && ((h[1].textContent = c), i && o)) {
      const p = h[1];
      p.style.background = "rgba(139, 154, 255, 0.25)";
      const u = setTimeout(() => {
        (p.style.background = ""), o.delete(u);
      }, 300);
      o.add(u);
    }
  } else {
...
function tt(e, r, l, s) {
  ...
  const o = new Set(i);
  for (const [c, d] of r) o.has(c) || (d.remove(), r.delete(c));
  for (const c of i) {
    let d;
    try {
      d = ct(l.read(c));
...
function Zt(e, r, l, s) {
  ...
  let c = new Date(),
    d = `${String(c.getHours()).padStart(2, "0")}:${String(c.getMinutes()).padStart(2, "0")}:${String(c.getSeconds()).padStart(2, "0")}.${String(c.getMilliseconds()).padStart(3, "0")}`,
    h;
  try {
  ...
  (u.style.color = q.accent), (u.textContent = ` ${r} `);
...
function er(e, r) {
  ...
  if (r.resolverStats.size > 0) {
    ...
    const o = [...r.resolverStats.entries()].sort(
      (c, d) => d[1].totalMs - c[1].totalMs,
    );
    for (const [c, d] of o) {
      const h = d.count > 0 ? (d.totalMs / d.count).toFixed(1) : "0",
        p = document.createElement("div");
      (p.style.paddingLeft = "8px"),
        (p.textContent = `${c}: ${d.count}x, avg ${h}ms${d.errors > 0 ? `, ${d.errors} err` : ""}`),
...
var De = new WeakMap();
function rr(e, r, l, s, i, o) {
  return [
    e.join(","),
    r.join(","),
    l.map((c) => `${c.id}:${c.active}`).join(","),
    [...s.entries()].map(([c, d]) => `${c}:${d.status}:${d.type}`).join(","),
    i.join(","),
    o.join(","),
  ].join("|");
}
function nr(e, r, l, s, i) {
  for (const o of l) {
    const c = e.nodes.get(`0:${o}`);
    if (!c) continue;
    const d = r.recentlyChangedFacts.has(o);
    c.rect.setAttribute("fill", d ? q.text + "33" : "none"),
      c.rect.setAttribute("stroke-width", d ? "2" : "1");
  }
  for (const o of s) {
    const c = e.nodes.get(`1:${o}`);
    if (!c) continue;
    const d = r.recentlyComputedDerivations.has(o);
    c.rect.setAttribute("fill", d ? q.accent + "33" : "none"),
      c.rect.setAttribute("stroke-width", d ? "2" : "1");
  }
  for (const o of i) {
    const c = e.nodes.get(`2:${o}`);
    if (!c) continue;
    const d = r.recentlyActiveConstraints.has(o),
      h = c.rect.getAttribute("stroke") ?? q.muted;
    c.rect.setAttribute("fill", d ? h + "33" : "none"),
      c.rect.setAttribute("stroke-width", d ? "2" : "1");
  }
}
function nt(e, r, l) {
  ...
  if (i.length === 0 && o.length === 0 && c.length === 0 && p.length === 0) {
    De.delete(e.flowSvg),
  ...
  const N = A(i.map((v) => ({ id: v, label: pe(v, Q.labelMaxChars) }))),
    T = A(o.map((v) => ({ id: v, label: pe(v, Q.labelMaxChars) }))),
    m = A(
      c.map((v) => ({
        id: v.id,
    ...
    y = A(p.map((v) => ({ id: v, label: pe(v, Q.labelMaxChars) }))),
    E = Math.max(N.length, T.length, m.length, S.length, y.length, 1),
    R = Q.startY + 12 + E * (Q.nodeH + Q.nodeGap) + 8;
  e.flowSvg.replaceChildren(),
    e.flowSvg.setAttribute("viewBox", `0 0 ${k} ${R}`),
    e.flowSvg.setAttribute(
      "aria-label",
      `Dependency graph: ${i.length} facts, ${o.length} derivations, ${c.length} constraints, ${u.size} requirements, ${p.length} resolvers`,
    );
  ...
  function b(v, C, t, n, a, g, x, I) {
    ...
    return (
      ...
      f.nodes.set(`${v}:${n}`, { g: L, rect: W, text: _ }),
      { midX: C + Q.nodeW / 2, midY: t }
    );
  }
  function w(v, C, t, n, a, g) {
    const x = document.createElementNS("http://www.w3.org/2000/svg", "line");
    x.setAttribute("x1", String(v)),
      x.setAttribute("y1", String(C)),
  ...
  const M = new Map(),
    ...
    P = new Map();
  for (const v of N) {
    const C = l.recentlyChangedFacts.has(v.id),
      t = b(0, O[0], v.y, v.id, v.label, q.text, !1, C);
    M.set(v.id, t);
  }
  for (const v of T) {
    const C = l.recentlyComputedDerivations.has(v.id),
      t = b(1, O[1], v.y, v.id, v.label, q.accent, !1, C);
    F.set(v.id, t);
  }
  for (const v of m) {
    const C = l.recentlyActiveConstraints.has(v.id),
      t = b(
        2,
        O[2],
        v.y,
  ...
  for (const v of S) {
    const C = v.status === "unmet" ? q.red : q.yellow,
      t = b(3, O[3], v.y, v.id, pe(v.type, Q.labelMaxChars), C, !1, !1);
    P.set(v.id, t);
  }
  for (const v of y) {
  ...
  for (const v of T) {
    const C = l.derivationDeps.get(v.id),
      t = F.get(v.id);
    if (C && t)
      for (const n of C) {
        const a = M.get(n);
  ...
  for (const v of S) {
    const C = K.get(v.fromConstraint),
      t = P.get(v.id);
    C &&
      t &&
...
function sr(e, r) {
  ...
  for (const j of r.inflight.values()) j < s && (s = j), o > i && (i = o);
  const c = i - s || 1,
    d = ke - ge - 10,
    h = [],
    p = new Set();
  ...
  for (let j = 0; j < u.length; j++) {
    ...
    N.setAttribute("x", String(ge - 4)),
      N.setAttribute("y", String(O + he / 2 + 3)),
      N.setAttribute("fill", q.muted),
      N.setAttribute("font-size", "7"),
      N.setAttribute("font-family", q.font),
      N.setAttribute("text-anchor", "end"),
      (N.textContent = pe(D, 12)),
      e.timelineSvg.appendChild(N);
    const T = l.filter((S) => S.resolver === D);
    for (const S of T) {
      const y = ge + ((S.startMs - s) / c) * d,
        E = Math.max(((S.endMs - S.startMs) / c) * d, Qe),
        ...
        subscribe(l, s) {
          ...
          if (!i) {
            let o = !1,
              ...
              d = setTimeout(() => clearInterval(c), 1e4);
            return () => {
              clearInterval(c), clearTimeout(d);
...
function lr(e = {}) {
  ...
  let u = (n, a) => {
    ...
    k = Yt(),
    A = Vt(),
    N = i && typeof window < "u" && typeof document < "u" && Be(),
    T = null,
    m = 0,
    S = 1,
    y = 2,
  ...
  function v() {
    ...
    const n = $.refs,
      ...
      g = m;
    if (((m = 0), g & S)) {
      for (const x of F) Qt(B, x);
      F.clear();
      for (const [x, { value: I, flash: L }] of M)
        $e(B, n.factsBody, x, I, L, $.flashTimers);
      M.clear(), (n.factsCount.textContent = String(B.size));
    }
    if ((g & y && tt(n, j, a, $.flashTimers), g & R))
      if (K) je(n, K.inflight.length, K.unmet.length);
      else {
        const x = Se(a);
        x && je(n, x.inflight.length, x.unmet.length);
      }
    if (g & E)
      if (K) Oe(n, K.inflight, K.unmet);
      else {
        const x = Se(a);
        x && Oe(n, x.inflight, x.unmet);
      }
    g & z && er(n, D),
      g & f && nt(n, a, O),
      g & b && rt(n, a),
  ...
  function C(n, a) {
    $ && l && Zt($.refs, n, a, p.events.size);
  }
  function t(n, a) {
    k.isRecording &&
      k.recordedEvents.length < Ut &&
      k.recordedEvents.push({ timestamp: Date.now(), type: n, data: Nt(a) });
  }
  return {
    name: "devtools",
    onInit: (n) => {
      if (
        ...
        try {
          ...
          for (const [L, W] of Object.entries(I)) $e(B, g.factsBody, L, W, !1);
          g.factsCount.textContent = String(Object.keys(I).length);
        } catch {}
        tt(g, j, a);
        const x = Se(a);
        x &&
          (je(g, x.inflight.length, x.unmet.length),
          Oe(g, x.inflight, x.unmet)),
          rt(g, a),
          tr(g, a),
          nt(g, a, O),
    ...
    onFactDelete: (n, a) => {
      u("fact.delete", { key: n, prev: a }),
        ...
        $ && (F.add(n), M.delete(n), P(S), C("fact.delete", { key: n }));
    },
    onFactsBatch: (n) => {
      if (
        ...
        for (const a of n)
          a.type === "delete"
            ...
            : (O.recentlyChangedFacts.add(a.key),
              ...
              F.delete(a.key));
        P(S), C("facts.batch", { count: n.length });
      }
    },
    ...
    onResolverError: (n, a, g) => {
      ...
      const x = D.resolverStats.get(n) ?? { count: 0, totalMs: 0, errors: 0 };
      if ((x.errors++, D.resolverStats.set(n, x), D.resolverStats.size > et)) {
        const L = D.resolverStats.keys().next().value;
        L !== void 0 && D.resolverStats.delete(L);
      }
      const I = A.inflight.get(n);
...
function ar(e, r, l) {
  const s = ut(`${e}:${r}`),
    i = l.reduce((c, d) => c + d.weight, 0);
  let o = s % i;
  for (const c of l) if (((o -= c.weight), o < 0)) return c.id;
...
function de(e, r) {
  ...
  let l = "",
    s = r,
    i = "register";
  r.startsWith("Registered")
    ? ((l = "registered"),
      (s = r.replace("Registered experiment: ", "")),
      (i = "register"))
    : r.startsWith("Assigned") || r.includes("→")
      ? ((l = "assigned"), (s = r), (i = "assign"))
      : r.includes("Exposure tracked")
        ? ((l = "exposure"),
          (s = r.replace("Exposure tracked: ", "")),
          (i = "exposure"))
        : r.includes("Manual assignment")
          ? ((l = "manual"),
            (s = r.replace("Manual assignment: ", "")),
            (i = "assign"))
          : r.includes("Paused") || r.includes("Resumed") || r.includes("Reset")
            ? ((l = r.toLowerCase().split(" ")[0]), (s = r), (i = "control"))
            : ((l = e), (s = r), (i = "register")),
    Re.unshift({ time: Date.now(), event: l, detail: s, type: i });
}
const cr = {
    facts: {
      experiments: ie.object(),
      assignments: ie.object(),
      exposures: ie.object(),
      userId: ie.string(),
      paused: ie.boolean(),
    },
    derivations: {
      activeExperiments: ie.object(),
      assignedCount: ie.number(),
      exposedCount: ie.number(),
    },
    events: {
      registerExperiment: {
        id: ie.string(),
      ...
      assignVariant: { experimentId: ie.string(), variantId: ie.string() },
      recordExposure: { experimentId: ie.string() },
      pauseAll: {},
      resumeAll: {},
      reset: {},
    },
    requirements: {
      ASSIGN_VARIANT: { experimentId: ie.string() },
      TRACK_EXPOSURE: { experimentId: ie.string(), variantId: ie.string() },
    },
  },
  ur = wt("ab-testing", {
    schema: cr,
    init: (e) => {
      (e.experiments = []),
        (e.assignments = {}),
        (e.exposures = {}),
        (e.userId = `user-${ut(String(Date.now())).toString(36)}`),
        (e.paused = !1);
    },
    derive: {
      activeExperiments: (e) =>
        e.experiments.filter((r) => r.active && !e.paused),
      assignedCount: (e) => Object.keys(e.assignments).length,
      exposedCount: (e) => Object.keys(e.exposures).length,
    },
    events: {
      registerExperiment: (e, { id: r, name: l, variants: s }) => {
        const i = e.experiments;
        i.find((o) => o.id === r) ||
          (e.experiments = [...i, { id: r, name: l, variants: s, active: !0 }]);
      },
      assignVariant: (e, { experimentId: r, variantId: l }) => {
        e.assignments = { ...e.assignments, [r]: l };
      },
      recordExposure: (e, { experimentId: r }) => {
        e.exposures = { ...e.exposures, [r]: Date.now() };
      },
      pauseAll: (e) => {
        e.paused = !0;
      },
      ...
      reset: (e) => {
        (e.assignments = {}), (e.exposures = {}), (e.paused = !1);
      },
    },
    constraints: {
      needsAssignment: {
        ...
        when: (e) => {
          ...
          const r = e.experiments,
            l = e.assignments;
          return r.some((s) => s.active && !l[s.id]);
        },
        require: (e) => {
          const r = e.experiments,
            l = e.assignments;
          return {
            type: "ASSIGN_VARIANT",
            experimentId: r.find((i) => i.active && !l[i.id]).id,
          };
        },
      },
      needsExposure: {
        ...
        when: (e) => {
          ...
          const r = e.assignments,
            l = e.exposures;
          return Object.keys(r).some((s) => !l[s]);
        },
        require: (e) => {
          const r = e.assignments,
            l = e.exposures,
            s = Object.keys(r).find((i) => !l[i]);
          return { type: "TRACK_EXPOSURE", experimentId: s, variantId: r[s] };
        },
      },
    },
    resolvers: {
      assignVariant: {
        ...
        resolve: async (e, r) => {
          const s = r.facts.experiments.find((o) => o.id === e.experimentId);
          if (!s) return;
          const i = ar(r.facts.userId, e.experimentId, s.variants);
          (r.facts.assignments = {
            ...r.facts.assignments,
            [e.experimentId]: i,
          }),
            de("resolver", `Assigned ${e.experimentId} → ${i}`);
        },
      },
      trackExposure: {
        ...
        resolve: async (e, r) => {
          const l = Date.now();
          (r.facts.exposures = { ...r.facts.exposures, [e.experimentId]: l }),
            de(
              "resolver",
              `Exposure tracked: ${e.experimentId} (variant: ${e.variantId}) at ${new Date(l).toLocaleTimeString()}`,
            );
        },
      },
    },
  }),
  ne = _t({ module: ur, plugins: [lr({ name: "ab-testing" })] });
ne.start();
const dr = document.getElementById("experiment-count"),
  fr = document.getElementById("assigned-count"),
  pr = document.getElementById("exposed-count"),
  mr = document.getElementById("user-id"),
  it = document.getElementById("experiments"),
  _e = document.getElementById("btn-pause"),
  hr = document.getElementById("btn-reset"),
  Ie = document.getElementById("ab-timeline");
function we(e) {
  const r = document.createElement("div");
  return (r.textContent = e), r.innerHTML;
}
function dt() {
  const e = ne.facts.experiments,
    r = ne.facts.assignments,
    l = ne.facts.exposures,
    s = ne.read("assignedCount"),
    i = ne.read("exposedCount");
  (dr.textContent = String(e.length)),
    (fr.textContent = String(s)),
    (pr.textContent = String(i)),
    (mr.textContent = ne.facts.userId),
    (_e.textContent = ne.facts.paused ? "Resume All" : "Pause All"),
    (_e.className = ne.facts.paused ? "ab-btn" : "ab-btn primary"),
    (it.innerHTML = "");
  for (const o of e) {
    const c = document.createElement("div");
    c.className = "experiment";
    const d = r[o.id],
      h = l[o.id];
    c.innerHTML = `
      <div class="experiment-name">${we(o.name)}</div>
      <div class="experiment-meta">
        ID: ${we(o.id)} &nbsp;|&nbsp;
        Status: ${o.active ? (ne.facts.paused ? "Paused" : "Active") : "Inactive"} &nbsp;|&nbsp;
        Assigned: ${d ?? "–"} &nbsp;|&nbsp;
        Exposed: ${h ? new Date(h).toLocaleTimeString() : "–"}
      </div>
      <div class="experiment-variants"></div>
    `;
    const p = c.querySelector(".experiment-variants");
    for (const u of o.variants) {
      const $ = document.createElement("button");
      ($.className = `variant-btn${d === u.id ? " active" : ""}`),
        ($.textContent = `${u.label} (${u.weight}%)`),
        $.addEventListener("click", () => {
          ne.events.assignVariant({ experimentId: o.id, variantId: u.id }),
            de("event", `Manual assignment: ${o.id} → ${u.id}`);
        }),
        p.appendChild($);
    }
    it.appendChild(c);
  ...
  if (Re.length === 0)
    ...
    for (const o of Re) {
      const c = document.createElement("div");
      c.className = `ab-timeline-entry ${o.type}`;
      const h = new Date(o.time).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      });
      (c.innerHTML = `
        <span class="ab-timeline-time">${h}</span>
        <span class="ab-timeline-event">${we(o.event)}</span>
        <span class="ab-timeline-detail">${we(o.detail)}</span>
      `),
        Ie.appendChild(c);
    }
  }
...
_e.addEventListener("click", () => {
  ne.facts.paused
    ? (ne.events.resumeAll(), de("event", "Resumed all experiments"))
    : (ne.events.pauseAll(), de("event", "Paused all experiments"));
});
hr.addEventListener("click", () => {
  ne.events.reset(),
    (Re.length = 0),
    de("event", "Reset all assignments and exposures");
});
ne.events.registerExperiment({
  id: "theme-icons",
  name: "Theme Icons",
  variants: [
...
de("event", "Registered experiment: theme-icons");
ne.events.registerExperiment({
  id: "cta-color",
...
de("event", "Registered experiment: cta-color");
dt();
