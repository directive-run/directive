...
function te(e, r, a, s, i, o) {
  return {
    ...
    describe(u) {
      return te(e, r, a, s, u, o);
    },
    refine(u, d) {
      const g = [...(o ?? []), { predicate: u, message: d }];
      return te([...e, u], r, a, s, i, g);
    },
    nullable() {
      return te(
...
var Z = {
  ...
  array() {
    const e = (r, a, s, i, o) => {
      ...
      return {
        ...
        nonEmpty() {
          return e([...r, (g) => g.length > 0], a, s, i, d);
        },
        maxLength(g) {
          return e([...r, (p) => p.length <= g], a, s, i, d);
        },
        minLength(g) {
          return e([...r, (p) => p.length >= g], a, s, i, d);
        },
  ...
  object() {
    const e = (r, a, s) => ({
      ...
      hasKeys(...i) {
        return e([...r, (o) => i.every((u) => u in o)], a, s);
      },
      default(i) {
        return e(r, i, s);
      },
...
function Rt(e) {
  ...
  function m(f, b) {
    ...
    if (O(w)) {
      ...
      if (!L.success) {
        const v = b === null ? "null" : Array.isArray(b) ? "array" : typeof b,
          ...
          n = N(w);
        throw new Error(
          `[Directive] Validation failed for "${f}": expected ${n}, got ${v} ${k}. ${t}`,
        );
      }
      return;
    }
    ...
    for (let L = 0; L < F.length; L++) {
      ...
      if (typeof v == "function" && !v(b)) {
        ...
        throw new Error(
          `[Directive] Validation failed for "${f}": expected ${H}, got ${k} ${t}${l}${n}`,
        );
      }
    }
  }
  ...
  function C(f, b, w) {
    ...
    try {
      ...
      while (j.length > 0) {
        ...
        const F = [...j];
        j.length = 0;
        for (const H of F) a?.(H.key, H.value, H.prev), E(H.key);
        y();
      }
    } finally {
      A = !1;
  ...
  const P = {
    ...
    subscribe(f, b) {
      for (const w of f) {
        const M = w;
        p.has(M) || p.set(M, new Set()), p.get(M).add(b);
      }
      return () => {
        for (const w of f) {
          const M = p.get(w);
          M && (M.delete(b), M.size === 0 && p.delete(w));
        }
      };
    },
    subscribeAll(f) {
  ...
  return (
    (P.registerKeys = (f) => {
      for (const b of Object.keys(f)) ye.has(b) || ((r[b] = f[b]), g.add(b));
    }),
    P
  );
}
...
function we(e, r = 50) {
  ...
  function i(o, u) {
    ...
    for (const g of Object.keys(d))
      if (a.has(g) || !i(d[g], u + 1)) return s.delete(d), !1;
    return s.delete(d), !0;
  }
  return i(e, 0);
}
...
function It(e, r, a) {
  return { requirement: e, id: jt(e, a), fromConstraint: r };
}
var Le = class ft {
    map = new Map();
    add(r) {
...
function qt(e) {
  ...
  function y() {
    ...
    function h(S, I) {
      ...
      n.add(S), I.push(S);
      const z = r[S];
      if (z?.after) for (const W of z.after) r[W] && h(W, I);
      I.pop(), n.delete(S), t.add(S), l.push(S);
    }
    for (const S of Object.keys(r)) h(S, []);
    (B = l), (m = new Map(B.map((S, I) => [S, I])));
  }
  y(), E();
  function C(t, n) {
  ...
  function f(t, n) {
    ...
    for (const h of l) {
      const S = x.get(h);
      S?.delete(t), S && S.size === 0 && x.delete(h);
    }
    for (const h of n) x.has(h) || x.set(h, new Set()), x.get(h).add(t);
    c.set(t, n);
  }
  function b(t) {
    const n = r[t];
    ...
    try {
      let h;
      if (n.deps) (h = n.when(a)), D.set(t, new Set(n.deps));
      else {
        const S = Ae(() => n.when(a));
        (h = S.value), D.set(t, S.deps);
      }
  ...
  async function w(t) {
    ...
    try {
      const S = n.when(a),
        I = await Ce(S, h, `Constraint "${t}" timed out after ${h}ms`);
      return (l.lastResult = I), (l.isEvaluating = !1), o?.(t, I), I;
    } catch (S) {
      return (
        (l.error = S instanceof Error ? S : new Error(String(S))),
        (l.lastResult = !1),
  ...
  function F(t) {
    ...
    if (typeof l == "function") {
      ...
      return { requirements: M(h), deps: S };
    }
  ...
  function H(t, n) {
    ...
    const l = c.get(t) ?? new Set();
    for (const h of n)
      l.add(h), x.has(h) || x.set(h, new Set()), x.get(h).add(t);
    c.set(t, l);
  }
  let L = null;
  function v() {
    return (
      L ||
        (L = Object.keys(r).sort((t, n) => {
          const l = P(t),
            h = P(n).priority - l.priority;
          if (h !== 0) return h;
          const S = m.get(t) ?? 0,
            I = m.get(n) ?? 0;
          return S - I;
  ...
  function k(t) {
    ...
    for (const l of n.after)
      if (r[l] && !g.has(l) && !O.has(l) && !$.has(l)) return !1;
    return !0;
  }
  return {
    async evaluate(t) {
      const n = new Le();
      O.clear();
      let l = v().filter((_) => !g.has(_)),
        h;
      if (!j || !t || t.size === 0) (h = l), (j = !0);
      else {
        const _ = new Set();
        for (const V of t) {
          const J = x.get(V);
        ...
        for (const V of T) g.has(V) || _.add(V);
        T.clear(), (h = [..._]);
        for (const V of l)
          if (!_.has(V)) {
            const J = A.get(V);
      ...
      async function I(_) {
        ...
        for (const K of _)
          if (k(K)) J.push(K);
            ...
            V.push(K);
            const Y = A.get(K);
            if (Y) for (const G of Y) n.add(G);
          }
        if (J.length === 0) return V;
        const re = [],
          ee = [];
        for (const K of J) P(K).isAsync ? ee.push(K) : re.push(K);
        const X = [];
        for (const K of re) {
          const Y = b(K);
        ...
        if (X.length > 0) {
          ...
          for (const { id: Y, active: G } of K) S(Y, G);
        }
        if (ee.length > 0) {
          const K = await Promise.all(
      ...
      let z = h,
        W = h.length + 1;
      while (z.length > 0 && W > 0) {
        const _ = z.length;
        if (((z = await I(z)), z.length === _)) break;
        W--;
      }
      return n.all();
    },
    ...
    disable(t) {
      g.add(t), (L = null), A.delete(t);
      const n = c.get(t);
      if (n) {
        for (const l of n) {
          const h = x.get(l);
          h && (h.delete(t), h.size === 0 && x.delete(l));
        }
        c.delete(t);
      }
      D.delete(t);
    ...
    markResolved(t) {
      ...
      const l = N.get(t);
      if (l) for (const h of l) T.add(h);
    },
    isResolved(t) {
      return $.has(t);
    },
    registerDefinitions(t) {
      for (const [n, l] of Object.entries(t)) (r[n] = l), R(n), T.add(n);
      (L = null), y(), E();
    },
  };
}
function Tt(e) {
  ...
  function N(y) {
    ...
    C.isComputing = !0;
    try {
      const { value: P, deps: f } = Ae(() => R(a, j));
      return (
        (C.cachedValue = P), (C.isStale = !1), B(y, f), s?.(y, P, [...f]), P
      );
    } catch (P) {
      throw (o?.(y, P), P);
    } finally {
  ...
  function B(y, C) {
    ...
    for (const f of P)
      if (u.has(f)) {
        ...
        const b = g.get(f);
        b?.delete(y), b && b.size === 0 && g.delete(f);
      }
    for (const f of C)
      r[f]
        ? (p.has(f) || p.set(f, new Set()), p.get(f).add(y))
        : (g.has(f) || g.set(f, new Set()), g.get(f).add(y));
    R.dependencies = C;
  }
  function m() {
    if (!(x > 0 || D)) {
    ...
    (j = new Proxy(
      {},
      {
        get(y, C) {
          ...
          Pe(C);
          const R = O(C);
          return R.isStale && N(C), R.cachedValue;
        },
      },
    )),
    {
      get(y) {
        const C = O(y);
        return C.isStale && N(y), C.cachedValue;
      },
      isStale(y) {
      ...
      subscribe(y, C) {
        ...
        return () => {
          for (const R of y) {
            const P = R,
              f = d.get(P);
            f?.delete(C), f && f.size === 0 && d.delete(P);
          }
        };
      },
      getProxy() {
      ...
      registerDefinitions(y) {
        for (const [C, R] of Object.entries(y)) (r[C] = R), $(C);
      },
    }
  );
}
function Bt(e) {
  ...
  function T($, O) {
    ...
    if (N.dependencies) {
      for (const B of N.dependencies) if (O.has(B)) return !0;
      return !1;
    }
    return !0;
  }
      ...
      try {
        if (O.hasExplicitDeps) {
          ...
          y instanceof Promise && (y = await y),
            A(O, y),
            (O.dependencies = B.size > 0 ? B : null);
        }
      } catch (B) {
        o?.($, B),
          console.error(`[Directive] Effect "${$}" threw an error:`, B);
      }
    }
  }
  for (const $ of Object.keys(r)) p($);
  return {
    async runEffects($) {
      const O = [];
      for (const N of Object.keys(r)) T(N, $) && O.push(N);
      await Promise.all(O.map(j)), (d = x());
    },
    async runAll() {
      const $ = Object.keys(r);
      await Promise.all(
        $.map((O) => (c(O).enabled ? j(O) : Promise.resolve())),
      ),
        (d = x());
    },
    disable($) {
    ...
    cleanupAll() {
      g = !0;
      for (const $ of u.values()) D($);
    },
    registerDefinitions($) {
      for (const [O, N] of Object.entries($)) (r[O] = N), p(O);
    },
  };
}
function _t(e = {}) {
...
function Lt(e = {}) {
  ...
  return {
    handleError(c, x, T, D) {
      ...
      let j = p(c, x, T instanceof Error ? T : new Error(String(T)));
      if (j === "retry-later") {
        const $ = (d.get(x) ?? 0) + 1;
        d.set(x, $),
          u.scheduleRetry(c, x, D, $) ||
            ((j = "skip"), d.delete(x), typeof process < "u");
      }
      try {
        s?.(A, j);
      } catch ($) {
...
function zt() {
  ...
  return {
    ...
    emitFactSet(s, i, o) {
      for (const u of e) r(() => u.onFactSet?.(s, i, o));
    },
    emitFactDelete(s, i) {
    ...
    emitDerivationCompute(s, i, o) {
      for (const u of e) r(() => u.onDerivationCompute?.(s, i, o));
    },
    ...
    emitResolverComplete(s, i, o) {
      for (const u of e) r(() => u.onResolverComplete?.(s, i, o));
    },
    emitResolverError(s, i, o) {
      for (const u of e) r(() => u.onResolverError?.(s, i, o));
    },
    emitResolverRetry(s, i, o) {
      for (const u of e) r(() => u.onResolverRetry?.(s, i, o));
    },
    ...
    for (let t = 1; t <= M.attempts; t++) {
      ...
      try {
        ...
        if (w.resolveBatchWithResults) {
          ...
          if (!z || b.some((W, _) => h[_]?.success)) return;
        } else {
          let h;
          s.batch(() => {
            ...
            k && k > 0
              ? await Ce(h, k, `Batch resolver "${f}" timed out after ${k}ms`)
              : await h;
          const S = Date.now() - L;
          for (const I of b)
            x.set(I.id, {
        ...
        if (
          ...
          return;
        if (M.shouldRetry && !M.shouldRetry(v, t)) break;
        if (t < M.attempts) {
          const l = Qe(M, t);
          for (const h of b) d?.(f, h, t + 1);
          if (
            (await new Promise((h) => {
              ...
              H.signal.addEventListener("abort", I, { once: !0 });
            }),
            H.signal.aborted)
          )
            return;
...
function Ft(e) {
  ...
  function N(B) {
    ...
    s.batch(() => {
      for (const [m, E] of Object.entries(B)) {
  ...
  return {
    ...
    takeSnapshot(B) {
      ...
      const m = { id: c++, timestamp: Date.now(), facts: O(), trigger: B };
      for (
    ...
    goBack(B = 1) {
      ...
      if (y) E = y.startIndex;
        ...
        const R = D.find((P) => P.endIndex < p && p - P.endIndex <= B);
        E = R ? R.startIndex : Math.max(0, p - B);
      } else E = Math.max(0, p - B);
      if (m === E) return;
      p = E;
      const C = g[p];
    ...
    goForward(B = 1) {
      ...
      let m = p,
        E = p,
        y = D.find((R) => p >= R.startIndex && p < R.endIndex);
      if ((y ? (E = y.endIndex) : (E = Math.min(g.length - 1, p + B)), m === E))
        return;
      p = E;
      const C = g[p];
      C && (this.restore(C), o?.(m, E));
    ...
    endChangeset() {
      !u ||
        A === null ||
        (p > j && D.push({ label: A, startIndex: j, endIndex: p }),
        (A = null),
        (j = -1));
    },
    pause() {
...
function pt(e) {
  ...
  let x = Lt({
    ...
    { store: j, facts: $ } = Dt({
      ...
      onChange: (t, n, l) => {
        c.emitFactSet(t, n, l),
          T(t),
          !A?.isRestoring && (g === 0 && (p = !0), w.changedKeys.add(t), M());
      },
      onBatch: (t) => {
        c.emitFactsBatch(t);
        const n = [];
  ...
  (T = (t) => O.invalidate(t)), (D = (t) => O.invalidateMany(t));
  const N = Bt({
      definitions: i,
      facts: $,
      store: j,
      ...
      onError: (t, n) => {
        x.handleError("effect", t, n), c.emitEffectError(t, n);
      },
    }),
    B = qt({
      definitions: o,
      ...
      onError: (t, n) => {
        x.handleError("constraint", t, n), c.emitConstraintError(t, n);
      },
    }),
    m = Nt({
      definitions: u,
      ...
      onError: (t, n, l) => {
        x.handleError("resolver", t, l, n), c.emitResolverError(t, n, l);
      },
  ...
  async function F() {
    if (!w.isReconciling) {
      ...
      try {
        ...
        const { added: S, removed: I } = h.diff(w.previousRequirements);
  ...
  const H = new Proxy(
    ...
    v = {
      ...
      subscribe(t, n) {
        const l = [],
          h = [];
        for (const I of t) I in s ? l.push(I) : I in r && h.push(I);
        const S = [];
        return (
      ...
      explain(t) {
        ...
        const l = B.getState(n.fromConstraint),
          ...
          S = {},
          I = j.toObject();
        for (const [V, J] of Object.entries(I)) S[V] = J;
        const z = [
          ...
          W = Object.entries(n.requirement)
            ...
            .join(", ");
        W && z.push(`├─ Requirement payload: { ${W} }`);
...
`)
      ...
      watchDistributableSnapshot(t, n) {
        let { includeDerivations: l, excludeDerivations: h } = t,
          S = Object.keys(s),
          I;
        if ((l ? (I = l.filter((W) => S.includes(W))) : (I = S), h)) {
          const W = new Set(h);
          I = I.filter((_) => !W.has(_));
        }
  ...
  function k(t) {
    ...
    Object.assign(r, t.schema),
      ...
      t.hooks?.onInit?.(v),
      w.isRunning && (t.hooks?.onStart?.(v), M());
  }
...
function Ht(e) {
  ...
  function u(d) {
    ...
    if (g?.crossModuleDeps)
      for (const p of Object.keys(g.crossModuleDeps)) r.includes(p) && u(p);
    o.pop(), s.delete(d), a.add(d), i.push(d);
  }
...
function Kt(e) {
  ...
  for (const m of Object.keys(r)) {
    ...
    if (E) {
      for (const y of Object.keys(E.schema.facts))
        if (y.includes(U))
          throw new Error(
            `[Directive] Schema key "${y}" in module "${m}" contains the reserved separator "${U}". Schema keys cannot contain "${U}".`,
          );
    }
  }
  const g = [];
  for (const m of i) {
    ...
    const y = E.crossModuleDeps && Object.keys(E.crossModuleDeps).length > 0,
      ...
      R = {};
    for (const [v, k] of Object.entries(E.schema.facts)) R[`${m}${U}${v}`] = k;
    const P = {};
    if (E.schema.derivations)
      for (const [v, k] of Object.entries(E.schema.derivations))
        P[`${m}${U}${v}`] = k;
    const f = {};
    if (E.schema.events)
      for (const [v, k] of Object.entries(E.schema.events))
        f[`${m}${U}${v}`] = k;
    const b = E.init
        ? (v) => {
            const k = ie(v, m);
      ...
      w = {};
    if (E.derive)
      for (const [v, k] of Object.entries(E.derive))
        w[`${m}${U}${v}`] = (t, n) => {
          const l = y ? ae(t, m, C) : ie(t, m),
            h = ze(n, m);
          return k(l, h);
        };
    const M = {};
    if (E.events)
      for (const [v, k] of Object.entries(E.events))
        M[`${m}${U}${v}`] = (t, n) => {
          const l = ie(t, m);
          k(l, n);
        };
    const F = {};
    if (E.constraints)
      for (const [v, k] of Object.entries(E.constraints)) {
        const t = k;
        F[`${m}${U}${v}`] = {
          ...t,
          deps: t.deps?.map((n) => `${m}${U}${n}`),
          when: (n) => {
            const l = y ? ae(n, m, C) : ie(n, m);
            return t.when(l);
          },
          require:
    ...
    const H = {};
    if (E.resolvers)
      for (const [v, k] of Object.entries(E.resolvers)) {
        const t = k;
        H[`${m}${U}${v}`] = {
          ...t,
          resolve: async (n, l) => {
            const h = Ie(l.facts, r, () => Object.keys(r));
            await t.resolve(n, { facts: h[m], signal: l.signal });
          },
        };
      }
    const L = {};
    if (E.effects)
      for (const [v, k] of Object.entries(E.effects)) {
        const t = k;
        L[`${m}${U}${v}`] = {
          ...t,
          run: (n, l) => {
            const h = y ? ae(n, m, C) : ie(n, m),
              S = l ? (y ? ae(l, m, C) : ie(l, m)) : void 0;
            return t.run(h, S);
          },
          deps: t.deps?.map((n) => `${m}${U}${n}`),
        };
    ...
    g.push({
      ...
      hooks: E.hooks,
      snapshotEvents:
        s && !s.has(m) ? [] : E.snapshotEvents?.map((v) => `${m}${U}${v}`),
    });
  }
  let p = null,
    c = null;
  function x(m) {
    for (const [E, y] of Object.entries(m))
      if (!ne.has(E) && a.has(E)) {
  ...
  for (const m of Object.keys(r)) {
    ...
    if (!E) continue;
    const y = [];
    for (const C of Object.keys(E.schema.facts)) y.push(`${m}${U}${C}`);
    if (E.schema.derivations)
      for (const C of Object.keys(E.schema.derivations)) y.push(`${m}${U}${C}`);
    T.set(m, y);
  }
  const D = { names: null };
  function A() {
  ...
  return {
    ...
    start() {
      if ((c.start(), B && B > 0)) {
        const m = Object.keys(g[0]?.events ?? {}).find((E) =>
          E.endsWith(`${U}tick`),
        );
        m &&
          (N = setInterval(() => {
    ...
    subscribe(m, E) {
      ...
      return c.subscribe(y, E);
    },
    subscribeModule(m, E) {
      const y = T.get(m);
      return !y || y.length === 0 ? () => {} : c.subscribe(y, E);
    },
    watch(m, E, y) {
      return c.watch(ce(m), E, y);
    },
    when(m, E) {
      return c.when(() => m(j), E);
    },
    ...
    getDistributableSnapshot(m) {
      const E = {
        ...
        y = c.getDistributableSnapshot(E),
        C = {};
    ...
    watchDistributableSnapshot(m, E) {
      ...
      return c.watchDistributableSnapshot(y, (C) => {
        const R = {};
        for (const [P, f] of Object.entries(C.data)) {
          const b = P.indexOf(U);
          if (b > 0) {
            const w = P.slice(0, b),
              M = P.slice(b + U.length);
            R[w] || (R[w] = {}), (R[w][M] = f);
          } else R._root || (R._root = {}), (R._root[P] = f);
        }
        E({ ...C, data: R });
      });
    },
    registerModule(m, E) {
      ...
      for (const v of Object.keys(E.schema.facts))
        if (v.includes(U))
          throw new Error(
            `[Directive] Schema key "${v}" in module "${m}" contains the reserved separator "${U}".`,
          );
      const y = E,
      ...
      if (y.effects)
        for (const [v, k] of Object.entries(y.effects)) {
          ...
          H[`${m}${U}${v}`] = {
            ...t,
            run: (n, l) => {
              const h = C ? ae(n, m, R) : ie(n, m),
                S = l ? (C ? ae(l, m, R) : ie(l, m)) : void 0;
              return t.run(h, S);
            },
            deps: t.deps?.map((n) => `${m}${U}${n}`),
          };
        }
      a.add(m), (r[m] = y), (D.names = null);
      const L = [];
      for (const v of Object.keys(y.schema.facts)) L.push(`${m}${U}${v}`);
      ...
      T.set(m, L),
        c.registerModule({
          ...
          hooks: y.hooks,
          snapshotEvents:
            s && !s.has(m) ? [] : y.snapshotEvents?.map((v) => `${m}${U}${v}`),
        });
    },
  };
}
    ...
    {},
    {
      get(i, o) {
        if (typeof o != "symbol" && !ne.has(o))
          return o === "$store" || o === "$snapshot" ? e[o] : e[`${r}${U}${o}`];
      },
      set(i, o, u) {
      ...
      has(i, o) {
        return typeof o == "symbol" || ne.has(o) ? !1 : `${r}${U}${o}` in e;
      },
      deleteProperty(i, o) {
        return typeof o == "symbol" || ne.has(o)
          ? !1
...
function ze(e, r) {
  ...
  const s = new Proxy(
    ...
    {
      get(i, o) {
        if (typeof o != "symbol" && !ne.has(o)) return e[`${r}${U}${o}`];
      },
      has(i, o) {
        return typeof o == "symbol" || ne.has(o) ? !1 : `${r}${U}${o}` in e;
      },
...
function er() {
  return { entries: new mt(Zt), inflight: new Map() };
}
function tr() {
  return {
...
function or(e, r, a, s) {
  ...
  function N(K, Y) {
    ...
    const Ue = document.createElement("thead"),
      Ke = document.createElement("tr");
    for (const Et of ["Key", "Value"]) {
    ...
    Ue.appendChild(Ke), ue.appendChild(Ue);
    const Ve = document.createElement("tbody");
    return (
      ue.appendChild(Ve),
      G.appendChild(ue),
      { details: G, tbody: Ve, countSpan: fe }
    );
  }
  function B(K, Y) {
    const G = document.createElement("details");
    ...
    return (
      ...
      G.appendChild(ue),
      { details: G, list: ue, countSpan: fe }
    );
  }
  const m = N("Facts", !0);
  ...
  const y = B("Inflight", q.yellow);
  g.appendChild(y.details);
  const C = B("Unmet", q.red);
  ...
  Object.assign(I.style, {
    ...
    g.addEventListener(
      ...
      (K) => {
        const Y = g,
          G = Y.scrollTop === 0 && K.deltaY < 0,
          oe = Y.scrollTop + Y.clientHeight >= Y.scrollHeight && K.deltaY > 0;
        (G || oe) && K.preventDefault();
      },
      { passive: !1 },
    );
  let z = a,
  ...
  d.addEventListener("click", _), x.addEventListener("click", V);
  function J(K) {
    K.key === "Escape" && z && V();
  }
...
function at(e, r, a, s) {
  ...
  const o = new Set(i);
  for (const [u, d] of r) o.has(u) || (d.remove(), r.delete(u));
  for (const u of i) {
    let d;
    try {
      d = ht(a.read(u));
...
function ar(e, r, a, s) {
  ...
  (p.style.color = q.closeBtn), (p.textContent = d);
  ...
  for (
    ...
    e.eventsList.childElementCount > sr;
  )
  ...
  if (r.resolverStats.size > 0) {
    ...
    for (const [u, d] of o) {
      ...
      (p.style.paddingLeft = "8px"),
        ...
        d.errors > 0 && (p.style.color = q.red),
...
function fr(e, r, a, s, i) {
  ...
  for (const o of i) {
    ...
    const d = r.recentlyActiveConstraints.has(o),
      g = u.rect.getAttribute("stroke") ?? q.muted;
...
function ut(e, r, a) {
  ...
  const x = g.map((v) => v.resolverId).sort(),
    T = dr(i, o, u, c, p, x),
    D = Te.get(e.flowSvg);
  ...
  const N = O(i.map((v) => ({ id: v, label: me(v, Q.labelMaxChars) }))),
    B = O(o.map((v) => ({ id: v, label: me(v, Q.labelMaxChars) }))),
    m = O(
      u.map((v) => ({
        id: v.id,
        label: me(v.id, Q.labelMaxChars),
        active: v.active,
        priority: v.priority,
      })),
    ),
    ...
    y = O(p.map((v) => ({ id: v, label: me(v, Q.labelMaxChars) }))),
    C = Math.max(N.length, B.length, m.length, E.length, y.length, 1),
    R = Q.startY + 12 + C * (Q.nodeH + Q.nodeGap) + 8;
  e.flowSvg.replaceChildren(),
  ...
  function b(v, k, t, n, l, h, S, I) {
    ...
    W.setAttribute("x", String(k)),
      ...
      W.setAttribute("rx", "3"),
      W.setAttribute("fill", I ? h + "33" : "none"),
    ...
    return (
      ...
      f.nodes.set(`${v}:${n}`, { g: z, rect: W, text: _ }),
      { midX: k + Q.nodeW / 2, midY: t }
    );
  }
  function w(v, k, t, n, l, h) {
  ...
  const M = new Map(),
    ...
    L = new Map();
  for (const v of N) {
    const k = a.recentlyChangedFacts.has(v.id),
      t = b(0, j[0], v.y, v.id, v.label, q.text, !1, k);
    M.set(v.id, t);
  }
  for (const v of B) {
    const k = a.recentlyComputedDerivations.has(v.id),
      t = b(1, j[1], v.y, v.id, v.label, q.accent, !1, k);
    F.set(v.id, t);
  }
  for (const v of m) {
    const k = a.recentlyActiveConstraints.has(v.id),
      t = b(
        2,
        j[2],
        v.y,
  ...
  for (const v of E) {
    const k = v.status === "unmet" ? q.red : q.yellow,
      t = b(3, j[3], v.y, v.id, me(v.type, Q.labelMaxChars), k, !1, !1);
    L.set(v.id, t);
  }
  for (const v of y) {
    const k = g.some((t) => t.resolverId === v.id);
    b(4, j[4], v.y, v.id, v.label, k ? q.green : q.muted, !k, !1);
  }
  for (const v of B) {
    const k = a.derivationDeps.get(v.id),
      t = F.get(v.id);
    if (k && t)
      for (const n of k) {
        const l = M.get(n);
  ...
  for (const v of E) {
    ...
    k &&
      t &&
      w(k.midX + Q.nodeW / 2, k.midY, t.midX - Q.nodeW / 2, t.midY, q.muted);
  }
  for (const v of g) {
    const k = L.get(v.id);
    if (k) {
      const t = y.find((n) => n.id === v.resolverId);
      t && w(k.midX + Q.nodeW / 2, k.midY, j[4], t.y, q.green);
    }
  }
  Te.set(e.flowSvg, f);
}
...
function mr(e, r) {
  ...
  const u = i - s || 1,
    ...
    p = new Set();
  for (const D of a)
    p.has(D.resolver) || (p.add(D.resolver), g.push(D.resolver));
  for (const D of r.inflight.keys()) p.has(D) || (p.add(D), g.push(D));
  const c = g.slice(-12),
    x = ge * c.length + 20;
  e.timelineSvg.setAttribute("viewBox", `0 0 ${Oe} ${x}`),
  ...
  for (let D = 0; D <= T; D++) {
    ...
    $.setAttribute("x", String(A)),
      $.setAttribute("y", "8"),
      $.setAttribute("fill", q.muted),
      $.setAttribute("font-size", "6"),
      $.setAttribute("font-family", q.font),
    ...
    O.setAttribute("x1", String(A)),
      ...
      O.setAttribute("stroke", q.border),
  ...
  for (let D = 0; D < c.length; D++) {
    ...
    N.setAttribute("x", String(ve - 4)),
      N.setAttribute("y", String(j + ge / 2 + 3)),
      N.setAttribute("fill", q.muted),
      N.setAttribute("font-size", "7"),
      N.setAttribute("font-family", q.font),
      N.setAttribute("text-anchor", "end"),
      (N.textContent = me(A, 12)),
      e.timelineSvg.appendChild(N);
    const B = a.filter((E) => E.resolver === A);
    for (const E of B) {
      const y = ve + ((E.startMs - s) / u) * d,
        C = Math.max(((E.endMs - E.startMs) / u) * d, st),
    ...
    if (m !== void 0) {
      ...
      (R.textContent = `${A}: inflight ${(o - m).toFixed(0)}ms`),
        C.appendChild(R),
...
function gr(e = {}) {
  ...
  let c = (n, l) => {
    ...
    $ = ir(),
    O = er(),
    N = i && typeof window < "u" && typeof document < "u" && Fe(),
    B = null,
    m = 0,
    E = 1,
    y = 2,
  ...
  function v() {
    ...
    const n = x.refs,
      ...
      h = m;
    if (((m = 0), h & E)) {
      for (const S of F) lr(T, S);
      F.clear();
      for (const [S, { value: I, flash: z }] of M)
        Re(T, n.factsBody, S, I, z, x.flashTimers);
      M.clear(), (n.factsCount.textContent = String(T.size));
    }
    if ((h & y && at(n, D, l, x.flashTimers), h & R))
  ...
  function k(n, l) {
    x && a && ar(x.refs, n, l, p.events.size);
  }
  function t(n, l) {
    $.isRecording &&
      $.recordedEvents.length < rr &&
      $.recordedEvents.push({ timestamp: Date.now(), type: n, data: Gt(l) });
  }
  return {
    name: "devtools",
    onInit: (n) => {
      if (
        ...
        try {
          ...
          for (const [z, W] of Object.entries(I)) Re(T, h.factsBody, z, W, !1);
          h.factsCount.textContent = String(Object.keys(I).length);
        } catch {}
        at(h, D, l);
        const S = ke(l);
        S &&
          ...
          h.recordBtn.addEventListener("click", () => {
            if (
              ...
              (h.recordBtn.style.color = $.isRecording ? q.red : q.text),
              $.isRecording)
            ) {
          ...
          h.exportBtn.addEventListener("click", () => {
            const I =
              ...
              W = new Blob([z], { type: "application/json" }),
    ...
    onFactDelete: (n, l) => {
      c("fact.delete", { key: n, prev: l }),
        ...
        x && (F.add(n), M.delete(n), L(E), k("fact.delete", { key: n }));
    },
    onFactsBatch: (n) => {
      if (
    ...
    onResolverComplete: (n, l, h) => {
      ...
      if (
        ...
        A.resolverStats.size > lt)
      ) {
        const z = A.resolverStats.keys().next().value;
        z !== void 0 && A.resolverStats.delete(z);
      }
    ...
    onResolverError: (n, l, h) => {
      ...
      const S = A.resolverStats.get(n) ?? { count: 0, totalMs: 0, errors: 0 };
      if ((S.errors++, A.resolverStats.set(n, S), A.resolverStats.size > lt)) {
        const z = A.resolverStats.keys().next().value;
        z !== void 0 && A.resolverStats.delete(z);
      }
      const I = O.inflight.get(n);
    ...
    onTimeTravel: (n, l) => {
      if (
        ...
        try {
          ...
          for (const [I, z] of Object.entries(S))
            Re(T, x.refs.factsBody, I, z, !1);
...
const Er = {
  ...
  We = {
    facts: { role: Z.string(), userName: Z.string(), token: Z.string() },
    derivations: { isAuthenticated: Z.boolean() },
    events: { login: { userId: Z.string() }, logout: {} },
    requirements: {},
  },
  xr = Ne("auth", {
    schema: We,
    init: (e) => {
      (e.role = ""), (e.userName = ""), (e.token = "");
    },
    derive: { isAuthenticated: (e) => e.token !== "" },
    events: {
      login: (e, { userId: r }) => {
        const a = Er[r];
        a &&
          ((e.token = a.token), (e.userName = a.userName), (e.role = a.role));
      },
      logout: (e) => {
        (e.token = ""), (e.userName = ""), (e.role = "");
      },
    },
  }),
  gt = {
    facts: { permissions: Z.object(), loaded: Z.boolean() },
    derivations: {
      canEdit: Z.boolean(),
    ...
    events: { reset: {} },
    requirements: { FETCH_PERMISSIONS: { role: Z.string() } },
  },
  $r = Ne("permissions", {
    schema: gt,
    crossModuleDeps: { auth: We },
    init: (e) => {
      (e.permissions = []), (e.loaded = !1);
    },
    derive: {
      canEdit: (e) => e.self.permissions.includes("content.edit"),
      canPublish: (e) => e.self.permissions.includes("content.publish"),
      canDelete: (e) => e.self.permissions.includes("content.delete"),
      canManageUsers: (e) => e.self.permissions.includes("users.manage"),
      canViewAnalytics: (e) => e.self.permissions.includes("analytics.view"),
      isAdmin: (e, r) => r.canManageUsers,
      permissionCount: (e) => e.self.permissions.length,
    },
    events: {
      reset: (e) => {
        (e.permissions = []), (e.loaded = !1);
    ...
    constraints: {
      loadPermissions: {
        when: (e) => e.auth.token !== "" && !e.self.loaded,
        require: (e) => ({ type: "FETCH_PERMISSIONS", role: e.auth.role }),
      },
    },
    resolvers: {
      fetchPermissions: {
        ...
        timeout: 5e3,
        resolve: async (e, r) => {
          const a = await yr(e.role);
          (r.facts.permissions = a), (r.facts.loaded = !0);
        },
      },
    },
  }),
  Cr = {
    facts: {
      articles: Z.object(),
      loaded: Z.boolean(),
      publishRequested: Z.string(),
      deleteRequested: Z.string(),
      actionStatus: Z.string(),
    },
    derivations: {},
    events: {
      requestPublish: { articleId: Z.string() },
      requestDelete: { articleId: Z.string() },
      clearAction: {},
    },
    requirements: {
      LOAD_CONTENT: {},
      PUBLISH_ARTICLE: { articleId: Z.string() },
      DELETE_ARTICLE: { articleId: Z.string() },
    },
  },
  kr = Ne("content", {
    schema: Cr,
    crossModuleDeps: { auth: We, permissions: gt },
    init: (e) => {
      (e.articles = []),
        (e.loaded = !1),
        (e.publishRequested = ""),
    ...
    constraints: {
      ...
      publishArticle: {
        when: (e) =>
          e.self.publishRequested !== "" &&
          e.permissions.permissions.includes("content.publish"),
        require: (e) => ({
          type: "PUBLISH_ARTICLE",
          articleId: e.self.publishRequested,
        }),
      },
      deleteArticle: {
        when: (e) =>
          e.self.deleteRequested !== "" &&
          e.permissions.permissions.includes("content.delete"),
        require: (e) => ({
          type: "DELETE_ARTICLE",
          articleId: e.self.deleteRequested,
        }),
      },
    },
    resolvers: {
      loadContent: {
        ...
        resolve: async (e, r) => {
          const a = await br();
          (r.facts.articles = a), (r.facts.loaded = !0);
        },
      },
      publishArticle: {
        ...
        timeout: 5e3,
        resolve: async (e, r) => {
          (r.facts.actionStatus = "publishing"), await wr(e.articleId);
          const a = r.facts.articles;
          (r.facts.articles = a.map((s) =>
            s.id === e.articleId ? { ...s, status: "published" } : s,
          )),
            (r.facts.publishRequested = ""),
            (r.facts.actionStatus = "done");
        },
      },
      deleteArticle: {
        ...
        timeout: 5e3,
        resolve: async (e, r) => {
          (r.facts.actionStatus = "deleting"), await Sr(e.articleId);
          const a = r.facts.articles;
          (r.facts.articles = a.filter((s) => s.id !== e.articleId)),
            (r.facts.deleteRequested = ""),
            (r.facts.actionStatus = "done");
        },
      },
    },
    events: {
      requestPublish: (e, { articleId: r }) => {
        (e.publishRequested = r), (e.actionStatus = "idle");
      },
      requestDelete: (e, { articleId: r }) => {
        (e.deleteRequested = r), (e.actionStatus = "idle");
      },
      clearAction: (e) => {
        (e.publishRequested = ""),
          (e.deleteRequested = ""),
  ...
  se = Ut({
    modules: { auth: xr, permissions: $r, content: kr },
    plugins: [gr({ name: "permissions" })],
  });
se.start();
const vt = document.getElementById("pm-user-admin"),
  yt = document.getElementById("pm-user-editor"),
  bt = document.getElementById("pm-user-viewer"),
  wt = document.getElementById("pm-logout"),
  Rr = document.getElementById("pm-perm-edit"),
  Ar = document.getElementById("pm-perm-publish"),
  Dr = document.getElementById("pm-perm-delete"),
  Or = document.getElementById("pm-perm-manage-users"),
  jr = document.getElementById("pm-perm-analytics"),
  Ir = document.getElementById("pm-perm-invite"),
  Mr = document.getElementById("pm-perm-settings"),
  be = document.getElementById("pm-article-list"),
  Ee = document.getElementById("pm-admin-panel"),
  de = document.getElementById("pm-action-status"),
  Be = document.getElementById("pm-toast");
let _e = null;
function xe(e, r = "denied") {
  (Be.textContent = e),
    (Be.className = `pm-toast visible ${r}`),
    _e && clearTimeout(_e),
    (_e = setTimeout(() => {
...
function pe(e, r) {
  e.classList.toggle("granted", r), e.classList.toggle("denied", !r);
}
function St() {
  const e = se.facts,
    r = se.derive,
    a = r.auth.isAuthenticated,
    s = e.auth.userName,
    i = e.auth.role,
    o = r.permissions.canEdit,
    u = r.permissions.canPublish,
    d = r.permissions.canDelete,
    g = r.permissions.canManageUsers,
    p = r.permissions.canViewAnalytics,
    c = e.permissions.permissions,
    x = e.content.articles,
    T = e.content.loaded,
    D = e.content.actionStatus;
  vt.classList.toggle("active", i === "admin"),
    yt.classList.toggle("active", i === "editor"),
    bt.classList.toggle("active", i === "viewer"),
    (wt.disabled = !a),
    pe(Rr, o),
    pe(Ar, u),
    pe(Dr, d),
    pe(Or, g),
    pe(jr, p),
    pe(Ir, c.includes("users.invite")),
    pe(Mr, c.includes("settings.manage")),
    a
      ? T
        ? x.length === 0
          ? (be.innerHTML = '<div class="pm-empty">No articles</div>')
          : (be.innerHTML = x
              .map((A) => {
                const j = A.status;
                let $ = "";
                return (
                  o &&
                    ($ += `<button class="pm-article-action edit" data-article-id="${A.id}" data-action="edit">Edit</button>`),
                  u &&
                    A.status === "draft" &&
                    ($ += `<button class="pm-article-action publish" data-article-id="${A.id}" data-action="publish">Publish</button>`),
                  d &&
                    ($ += `<button class="pm-article-action delete" data-article-id="${A.id}" data-action="delete">Delete</button>`),
                  `
          <div class="pm-article-card">
            <div class="pm-article-header">
              <span class="pm-article-title">${$e(A.title)}</span>
              <span class="pm-article-status ${j}">${A.status}</span>
            </div>
            <div class="pm-article-meta">by ${$e(A.author)}</div>
            <div class="pm-article-actions">${$ || '<span class="pm-no-actions">No actions available</span>'}</div>
          </div>
        `
                );
              })
              .join(""))
        : (be.innerHTML = '<div class="pm-empty">Loading articles...</div>')
      : (be.innerHTML = '<div class="pm-empty">Sign in to view articles</div>'),
    D === "publishing"
      ? ((de.textContent = "Publishing..."),
        (de.className = "pm-action-status active"))
      : D === "deleting"
        ? ((de.textContent = "Deleting..."),
          (de.className = "pm-action-status active"))
        : D === "done"
          ? ((de.textContent = "Done"),
            (de.className = "pm-action-status done"))
          : ((de.textContent = ""), (de.className = "pm-action-status")),
    g
      ...
      <div class="pm-panel-body">
        <div class="pm-admin-info">
          ...
          <div class="pm-admin-detail">Logged in as ${$e(s)} (${$e(i)})</div>
          <div class="pm-admin-detail">${c.length} permissions granted</div>
        </div>
    ...
    `))
      : (Ee.classList.remove("visible"), (Ee.innerHTML = ""));
}
se.subscribe(
  [
    "auth::role",
...
function He(e) {
  se.events.permissions.reset(),
    se.events.content.clearAction(),
    se.events.auth.logout(),
    setTimeout(() => {
      se.events.auth.login({ userId: e });
    }, 50);
}
vt.addEventListener("click", () => He("alice"));
yt.addEventListener("click", () => He("bob"));
bt.addEventListener("click", () => He("carol"));
...
be.addEventListener("click", (e) => {
  const r = e.target,
    a = r.dataset.articleId,
    s = r.dataset.action;
  if (!a || !s) return;
  const i = se.derive,
    o = i.permissions.canPublish,
    u = i.permissions.canDelete,
    d = i.permissions.canEdit;
  if (s === "edit") {
    if (!d) {
      xe("Permission Denied: You cannot edit content");
      return;
    }
    xe("Editing is a UI-only action in this demo", "success");
    return;
  }
  if (s === "publish") {
    if (!o) {
      xe("Permission Denied: You cannot publish content");
      return;
    }
    se.events.content.requestPublish({ articleId: a });
    return;
  }
  if (s === "delete") {
    if (!u) {
      xe("Permission Denied: You cannot delete content");
      return;
    }
    se.events.content.requestDelete({ articleId: a });
    return;
  }
});
function $e(e) {
