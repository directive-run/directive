...
function Ae(e = [], t, l, i, s, o) {
  return {
    ...
    _refinements: o,
    validate(d) {
      return Ae([...e, d], t, l, i, s, o);
    },
  };
}
function te(e, t, l, i, s, o) {
  return {
    ...Ae(e, t, l, i, s, o),
    default(d) {
      return te(e, t, d, i, s, o);
    },
    transform(d) {
      return te(
        [],
    ...
    brand() {
      return te(e, `Branded<${t}>`, l, i, s, o);
    },
    describe(d) {
      return te(e, t, l, i, d, o);
    },
    refine(d, u) {
      const h = [...(o ?? []), { predicate: d, message: u }];
      return te([...e, d], t, l, i, s, h);
    },
    nullable() {
      return te(
  ...
  array() {
    const e = (t, l, i, s, o) => {
      ...
      return {
        ...
        maxLength(h) {
          return e([...t, (f) => f.length <= h], l, i, s, u);
        },
        minLength(h) {
          return e([...t, (f) => f.length >= h], l, i, s, u);
        },
  ...
  function m(p, b) {
    ...
    if (D(w)) {
      ...
      if (!L.success) {
        const y = b === null ? "null" : Array.isArray(b) ? "array" : typeof b,
          ...
          n = P(w);
        throw new Error(
          `[Directive] Validation failed for "${p}": expected ${n}, got ${y} ${R}. ${r}`,
        );
      }
      return;
    }
    ...
    for (let L = 0; L < N.length; L++) {
      ...
      if (typeof y == "function" && !y(b)) {
        ...
        throw new Error(
          `[Directive] Validation failed for "${p}": expected ${W}, got ${R} ${r}${a}${n}`,
        );
      }
    }
  }
  ...
  function C(p, b, w) {
    ...
    try {
      ...
      while (O.length > 0) {
        ...
        const N = [...O];
        O.length = 0;
        for (const W of N) l?.(W.key, W.value, W.prev), E(W.key);
        v();
      }
    } finally {
      I = !1;
      ...
      if ((i && q.length > 0 && i([...q]), A.size > 0)) {
        ...
        try {
          ...
          while (O.length > 0) {
            ...
            const b = [...O];
            O.length = 0;
            for (const w of b) l?.(w.key, w.value, w.prev), E(w.key);
            v();
          }
        } finally {
          I = !1;
  ...
  const F = {
    ...
    set(p, b) {
      ...
      Object.is(w, b) ||
        ...
        h.add(p),
        $ > 0
          ? (q.push({ key: p, value: b, prev: w, type: "set" }), A.add(p))
          : C(p, b, w));
    },
    delete(p) {
      const b = u.get(p);
      u.delete(p),
        h.delete(p),
        $ > 0
          ? (q.push({ key: p, value: void 0, prev: b, type: "delete" }),
            A.add(p))
          : C(p, void 0, b);
    },
    batch(p) {
    ...
    subscribe(p, b) {
      for (const w of p) {
        const M = w;
        f.has(M) || f.set(M, new Set()), f.get(M).add(b);
      }
      return () => {
        for (const w of p) {
          const M = f.get(w);
          M && (M.delete(b), M.size === 0 && f.delete(w));
        }
      };
    },
    subscribeAll(p) {
    ...
    toObject() {
      const p = {};
      for (const b of h) u.has(b) && (p[b] = u.get(b));
      return p;
    },
  };
  return (
    (F.registerKeys = (p) => {
      for (const b of Object.keys(p)) ve.has(b) || ((t[b] = p[b]), h.add(b));
    }),
    F
  );
}
  ...
  return new Proxy(
    {},
    {
      get(i, s) {
        ...
        if (typeof s != "symbol" && !ve.has(s)) return e.get(s);
      },
      set(i, s, o) {
  ...
  function i(s, o) {
    ...
    if (d === "object") {
      ...
      const h = `{${Object.keys(u)
        .sort()
        .map((f) => `${JSON.stringify(f)}:${i(u[f], o + 1)}`)
        .join(",")}}`;
      return l.delete(u), h;
    }
    return '"[unknown]"';
...
function Bt(e, t, l) {
  return { requirement: e, id: qt(e, l), fromConstraint: t };
}
var _e = class gt {
    map = new Map();
    add(t) {
...
function _t(e) {
    ...
    for (const [r, n] of Object.entries(t))
      if (n.after)
        for (const a of n.after)
          t[a] && (P.has(a) || P.set(a, new Set()), P.get(a).add(r));
  }
  function v() {
    const r = new Set(),
      n = new Set(),
    ...
    function g(S, j) {
      ...
      n.add(S), j.push(S);
      const z = t[S];
      if (z?.after) for (const H of z.after) t[H] && g(H, j);
      j.pop(), n.delete(S), r.add(S), a.push(S);
    }
    for (const S of Object.keys(t)) g(S, []);
    (T = a), (m = new Map(T.map((S, j) => [S, j])));
  }
  v(), E();
  function C(r, n) {
  ...
  function p(r, n) {
    ...
    for (const g of a) {
      const S = $.get(g);
      S?.delete(r), S && S.size === 0 && $.delete(g);
    }
  ...
  function b(r) {
    ...
    try {
      let g;
      if (n.deps) (g = n.when(l)), A.set(r, new Set(n.deps));
      else {
        const S = ke(() => n.when(l));
  ...
  async function w(r) {
    ...
    try {
      const S = n.when(l),
        j = await xe(S, g, `Constraint "${r}" timed out after ${g}ms`);
      return (a.lastResult = j), (a.isEvaluating = !1), o?.(r, j), j;
    } catch (S) {
      return (
        (a.error = S instanceof Error ? S : new Error(String(S))),
        (a.lastResult = !1),
  ...
  function N(r) {
    ...
    if (typeof a == "function") {
      const { value: g, deps: S } = ke(() => a(l));
      return { requirements: M(g), deps: S };
    }
  ...
  for (const r of Object.keys(t)) k(r);
  function R(r) {
    const n = u.get(r);
    if (!n || n.after.length === 0) return !0;
    for (const a of n.after)
      if (t[a] && !h.has(a) && !D.has(a) && !x.has(a)) return !1;
    return !0;
  }
  return {
    async evaluate(r) {
      const n = new _e();
      D.clear();
      let a = y().filter((_) => !h.has(_)),
        g;
      if (!O || !r || r.size === 0) (g = a), (O = !0);
      else {
        const _ = new Set();
        for (const V of r) {
          const Y = $.get(V);
      ...
      function S(_, V) {
        ...
        const Y = A.get(_);
        if (!V) {
          Y !== void 0 && p(_, Y), D.add(_), I.set(_, []);
          return;
        }
        D.delete(_);
        let re, ee;
      ...
      async function j(_) {
        ...
        for (const U of _)
          if (R(U)) Y.push(U);
            ...
            V.push(U);
            const J = I.get(U);
            if (J) for (const X of J) n.add(X);
          }
        if (Y.length === 0) return V;
        const re = [],
          ee = [];
        for (const U of Y) F(U).isAsync ? ee.push(U) : re.push(U);
        const Q = [];
        for (const U of re) {
          const J = b(U);
          if (J instanceof Promise) {
            Q.push({ id: U, promise: J });
            continue;
          }
          S(U, J);
        }
        if (Q.length > 0) {
          const U = await Promise.all(
            Q.map(async ({ id: J, promise: X }) => ({
              id: J,
              active: await X,
            })),
          );
          for (const { id: J, active: X } of U) S(J, X);
        }
        if (ee.length > 0) {
          const U = await Promise.all(
      ...
      while (z.length > 0 && H > 0) {
        const _ = z.length;
        if (((z = await j(z)), z.length === _)) break;
        H--;
      }
      return n.all();
    },
    ...
    enable(r) {
      h.delete(r), (L = null), q.add(r);
    },
    invalidate(r) {
      const n = $.get(r);
      if (n) for (const a of n) q.add(a);
    },
    markResolved(r) {
      x.add(r);
      const n = u.get(r);
      ...
      const a = P.get(r);
      if (a) for (const g of a) q.add(g);
    },
    isResolved(r) {
      return x.has(r);
    },
    registerDefinitions(r) {
      for (const [n, a] of Object.entries(r)) (t[n] = a), k(n), q.add(n);
      (L = null), v(), E();
    },
  };
}
function Ft(e) {
  ...
  function P(v) {
    ...
    C.isComputing = !0;
    try {
      const { value: F, deps: p } = ke(() => k(l, O));
      return (
        (C.cachedValue = F), (C.isStale = !1), T(v, p), i?.(v, F, [...p]), F
      );
    } catch (F) {
      throw (o?.(v, F), F);
    } finally {
  ...
  function T(v, C) {
    ...
    for (const p of F)
      if (d.has(p)) {
        ...
        const b = h.get(p);
        b?.delete(v), b && b.size === 0 && h.delete(p);
      }
    for (const p of C)
      t[p]
        ...
        while (q.size > 0) {
          if (++v > I) {
            ...
            throw (
              (q.clear(),
              new Error(
                `[Directive] Infinite derivation notification loop detected after ${I} iterations. Remaining: ${k.join(", ")}. This usually means a derivation listener is mutating facts that re-trigger the same derivation.`,
              ))
            );
          }
          const C = [...q];
          q.clear();
          for (const k of C) u.get(k)?.forEach((F) => F());
        }
      } finally {
        A = !1;
      }
  ...
  function E(v, C = new Set()) {
    ...
    (k.isStale = !0), s?.(v), q.add(v);
    const F = f.get(v);
    if (F) for (const p of F) E(p, C);
  }
  return (
    (O = new Proxy(
      {},
    ...
    )),
    {
      get(v) {
        const C = D(v);
        return C.isStale && P(v), C.cachedValue;
      },
      isStale(v) {
      ...
      subscribe(v, C) {
        for (const k of v) {
          const F = k;
          u.has(F) || u.set(F, new Set()), u.get(F).add(C);
        }
        return () => {
          for (const k of v) {
            const F = k,
              p = u.get(F);
            p?.delete(C), p && p.size === 0 && u.delete(F);
          }
        };
      },
      getProxy() {
      ...
      registerDefinitions(v) {
        for (const [C, k] of Object.entries(v)) (t[C] = k), x(C);
      },
    }
  );
}
function Lt(e) {
  ...
  function q(x, D) {
    ...
    if (P.dependencies) {
      for (const T of P.dependencies) if (D.has(T)) return !0;
      return !1;
    }
    return !0;
  }
      ...
      try {
        if (D.hasExplicitDeps) {
          ...
          v instanceof Promise && (v = await v),
            I(D, v),
            (D.dependencies = T.size > 0 ? T : null);
        }
      } catch (T) {
        o?.(x, T),
  ...
  for (const x of Object.keys(t)) f(x);
  return {
    async runEffects(x) {
      const D = [];
      for (const P of Object.keys(t)) q(P, x) && D.push(P);
      await Promise.all(D.map(O)), (u = $());
    },
    async runAll() {
    ...
    cleanupAll() {
      h = !0;
      for (const x of d.values()) A(x);
    },
    registerDefinitions(x) {
      for (const [D, P] of Object.entries(x)) (t[D] = P), f(D);
    },
  };
}
function zt(e = {}) {
  ...
  return {
    ...
    processDueRetries() {
      const u = Date.now(),
        h = [];
      for (const [f, c] of o) c.nextRetryTime <= u && (h.push(c), o.delete(f));
      return h;
    },
    cancelRetry(u) {
      o.delete(u);
...
function Nt(e = {}) {
  const { config: t = {}, onError: l, onRecovery: i } = e,
    ...
    u = new Map();
  function h(c, $, q, A) {
    if (q instanceof Ue) return q;
    const I = q instanceof Error ? q.message : String(q),
      O = c !== "system";
    return new Ue(I, c, $, A, O);
  }
  function f(c, $, q) {
    const A = (() => {
      switch (c) {
    ...
    return typeof A == "string" ? A : Pt[c];
  }
  return {
    handleError(c, $, q, A) {
      const I = h(c, $, q, A);
      s.push(I), s.length > o && s.shift();
      try {
        l?.(I);
      } catch (x) {
...
function Wt(e) {
  ...
  function T(p, b) {
    return D(p) ? b.type === p.requirement : P(p) ? p.requirement(b) : !1;
  }
  function m(p) {
    const b = p.type,
      w = I.get(b);
    ...
    for (const [M, N] of Object.entries(t))
      if (T(N, p)) {
        if (!I.has(b)) {
          if (I.size >= O) {
            const L = I.keys().next().value;
            L !== void 0 && I.delete(L);
          }
          I.set(b, []);
        }
        const W = I.get(b);
        return W.includes(M) || W.push(M), M;
      }
    return null;
  }
  function E(p) {
    ...
    for (let L = 1; L <= N.attempts; L++) {
      ...
      try {
        ...
        if (M.resolve) {
          let n;
          i.batch(() => {
            n = M.resolve(b.requirement, R);
          });
          const a = M.timeout;
          a && a > 0
            ? await xe(n, a, `Resolver "${p}" timed out after ${a}ms`)
            : await n;
        }
        ...
        if (
          ((W = R instanceof Error ? R : new Error(String(R))),
          w.signal.aborted)
        )
          return;
    ...
    for (let r = 1; r <= M.attempts; r++) {
      ...
      try {
        ...
        if (w.resolveBatchWithResults) {
          ...
          if (
            ...
            R && R > 0
              ? (g = await xe(
                  S,
                  R,
                  `Batch resolver "${p}" timed out after ${R}ms`,
                ))
              : (g = await S),
            g.length !== b.length)
          )
            throw new Error(
              `[Directive] Batch resolver "${p}" returned ${g.length} results but expected ${b.length}. Results array must match input order.`,
            );
          let j = Date.now() - L,
            z = !1;
          ...
          if (!z || b.some((H, _) => g[_]?.success)) return;
        } else {
          let g;
          i.batch(() => {
            ...
            R && R > 0
              ? await xe(g, R, `Batch resolver "${p}" timed out after ${R}ms`)
              : await g;
          const S = Date.now() - L;
          for (const j of b)
            $.set(j.id, {
        ...
        if (r < M.attempts) {
          ...
          if (
            (await new Promise((g) => {
              ...
              W.signal.addEventListener("abort", j, { once: !0 });
            }),
            W.signal.aborted)
          )
            return;
  ...
  function k(p, b) {
    ...
    if (!w) return;
    const M = { ...Ye, ...w.batch };
    A.has(p) || A.set(p, { resolverId: p, requirements: [], timer: null });
    const N = A.get(p);
  ...
  return {
    ...
    cancelAll() {
      ...
      for (const p of A.values()) p.timer && clearTimeout(p.timer);
      A.clear();
    },
    getStatus(p) {
      const b = c.get(p);
...
function Kt(e) {
  ...
  return {
    ...
    takeSnapshot(T) {
      ...
      const m = { id: c++, timestamp: Date.now(), facts: D(), trigger: T };
      for (
    ...
    goBack(T = 1) {
      ...
      let m = f,
        E = f,
        v = A.find((k) => f > k.startIndex && f <= k.endIndex);
      if (v) E = v.startIndex;
      else if (A.find((k) => f === k.startIndex)) {
        const k = A.find((F) => F.endIndex < f && f - F.endIndex <= T);
        E = k ? k.startIndex : Math.max(0, f - T);
      } else E = Math.max(0, f - T);
      if (m === E) return;
      f = E;
      const C = h[f];
      C && (this.restore(C), o?.(m, E));
    },
    goForward(T = 1) {
      if (!d || h.length === 0) return;
      let m = f,
    ...
    endChangeset() {
      !d ||
        I === null ||
        (f > O && A.push({ label: I, startIndex: O, endIndex: f }),
        (I = null),
        (O = -1));
    },
    pause() {
...
function yt(e) {
  ...
  let $ = Nt({
    ...
    { store: O, facts: x } = Ot({
      ...
      onChange: (r, n, a) => {
        c.emitFactSet(r, n, a),
          q(r),
          !I?.isRestoring && (h === 0 && (f = !0), w.changedKeys.add(r), M());
      },
      onBatch: (r) => {
        c.emitFactsBatch(r);
        const n = [];
        for (const a of r) n.push(a.key);
        if ((A(n), !I?.isRestoring)) {
          h === 0 && (f = !0);
  ...
  (q = (r) => D.invalidate(r)), (A = (r) => D.invalidateMany(r));
  const P = Lt({
      definitions: s,
      facts: x,
      store: O,
    ...
    if (!w.isReconciling) {
      ...
      try {
        w.changedKeys.size > 0 &&
          ...
          (f = !1));
        const r = x.$snapshot();
        c.emitReconcileStart(r), await P.runEffects(w.changedKeys);
        const n = new Set(w.changedKeys);
        w.changedKeys.clear();
        const a = await T.evaluate(n),
          g = new _e();
        for (const _ of a) g.add(_), c.emitRequirementCreated(_);
        ...
        const z = m.getInflightInfo(),
          H = {
            ...
            canceled: j.map((_) => ({
              id: _.id,
              resolverId: z.find((V) => V.id === _.id)?.resolverId ?? "unknown",
            })),
          };
        ...
        (w.isReconciling = !1),
          w.changedKeys.size > 0 ? M() : w.reconcileScheduled || (b = 0),
          F();
      }
    }
  }
  const W = new Proxy(
      ...
      {
        get(r, n) {
          if (typeof n != "symbol" && !le.has(n)) return D.get(n);
        },
        has(r, n) {
    ...
    y = {
      ...
      derive: W,
      events: L,
      constraints: { disable: (r) => T.disable(r), enable: (r) => T.enable(r) },
      effects: {
        disable: (r) => P.disable(r),
        enable: (r) => P.enable(r),
        isEnabled: (r) => P.isEnabled(r),
      },
      initialize() {
        if (!w.isInitialized) {
          w.isInitializing = !0;
      ...
      subscribe(r, n) {
        const a = [],
          g = [];
        for (const j of r) j in i ? a.push(j) : j in t && g.push(j);
        const S = [];
        return (
          a.length > 0 && S.push(D.subscribe(a, n)),
          g.length > 0 && S.push(O.subscribe(g, n)),
          () => {
            for (const j of S) j();
          }
        );
      ...
      inspect() {
        return {
          ...
          constraints: T.getAllStates().map((r) => ({
            id: r.id,
      ...
      explain(r) {
        ...
        if (!n) return null;
        const a = T.getState(n.fromConstraint),
          g = m.getStatus(r),
          S = {},
          j = O.toObject();
          ...
          _.length > 0 &&
            ...
            _.forEach(([V, Y], re) => {
              const ee = re === _.length - 1 ? "   └─" : "   ├─",
                Q = typeof Y == "object" ? JSON.stringify(Y) : String(Y);
              z.push(
...
`)
      ...
      getDistributableSnapshot(r = {}) {
        let {
          ...
          _ = Object.keys(i),
          V;
        if ((n ? (V = n.filter((ee) => _.includes(ee))) : (V = _), a)) {
          const ee = new Set(a);
        ...
        if (g && g.length > 0) {
          const ee = O.toObject();
          for (const Q of g) Q in ee && (H[Q] = ee[Q]);
        }
        const Y = Date.now(),
          re = { data: H, createdAt: Y };
        return (
      ...
      watchDistributableSnapshot(r, n) {
        let { includeDerivations: a, excludeDerivations: g } = r,
          S = Object.keys(i),
          j;
        if ((a ? (j = a.filter((H) => S.includes(H))) : (j = S), g)) {
          const H = new Set(g);
          j = j.filter((_) => !H.has(_));
        }
  ...
  function R(r) {
    ...
    Object.assign(t, r.schema),
      ...
      r.effects &&
        (Object.assign(s, r.effects), P.registerDefinitions(r.effects)),
      r.constraints &&
        (Object.assign(o, r.constraints), T.registerDefinitions(r.constraints)),
      r.resolvers &&
        (Object.assign(d, r.resolvers), m.registerDefinitions(r.resolvers)),
      O.registerKeys(r.schema),
      e.modules.push(r),
      r.init &&
        O.batch(() => {
          r.init(x);
      ...
      r.hooks?.onInit?.(y),
      w.isRunning && (r.hooks?.onStart?.(y), M());
  }
  (y.registerModule = R), c.emitInit(y);
...
function Vt(e) {
  ...
  function d(u) {
    ...
    if (h?.crossModuleDeps)
      for (const f of Object.keys(h.crossModuleDeps)) t.includes(f) && d(f);
...
function Yt(e) {
  ...
  if (Array.isArray(o)) {
    ...
    s = m;
  } else o === "declaration" ? (s = Object.keys(t)) : (s = Vt(t));
  let d = e.debug,
    u = e.errorBoundary;
  e.zeroConfig &&
  ...
  for (const m of s) {
    ...
    const v = E.crossModuleDeps && Object.keys(E.crossModuleDeps).length > 0,
      ...
      k = {};
    for (const [y, R] of Object.entries(E.schema.facts)) k[`${m}${K}${y}`] = R;
    const F = {};
    if (E.schema.derivations)
      for (const [y, R] of Object.entries(E.schema.derivations))
        F[`${m}${K}${y}`] = R;
    const p = {};
    if (E.schema.events)
      for (const [y, R] of Object.entries(E.schema.events))
        p[`${m}${K}${y}`] = R;
    const b = E.init
        ? (y) => {
            const R = ie(y, m);
      ...
      w = {};
    if (E.derive)
      for (const [y, R] of Object.entries(E.derive))
        w[`${m}${K}${y}`] = (r, n) => {
          const a = v ? ae(r, m, C) : ie(r, m),
            g = Fe(n, m);
          return R(a, g);
        };
    const M = {};
    if (E.events)
      for (const [y, R] of Object.entries(E.events))
        M[`${m}${K}${y}`] = (r, n) => {
          const a = ie(r, m);
          R(a, n);
        };
    const N = {};
    if (E.constraints)
      for (const [y, R] of Object.entries(E.constraints)) {
        const r = R;
        N[`${m}${K}${y}`] = {
          ...r,
          deps: r.deps?.map((n) => `${m}${K}${n}`),
          when: (n) => {
            const a = v ? ae(n, m, C) : ie(n, m);
            return r.when(a);
          },
          require:
    ...
    const W = {};
    if (E.resolvers)
      for (const [y, R] of Object.entries(E.resolvers)) {
        const r = R;
        W[`${m}${K}${y}`] = {
          ...r,
          resolve: async (n, a) => {
            const g = Ie(a.facts, t, () => Object.keys(t));
            await r.resolve(n, { facts: g[m], signal: a.signal });
          },
        };
      }
    const L = {};
    if (E.effects)
      for (const [y, R] of Object.entries(E.effects)) {
        const r = R;
        L[`${m}${K}${y}`] = {
          ...r,
          run: (n, a) => {
            const g = v ? ae(n, m, C) : ie(n, m),
              S = a ? (v ? ae(a, m, C) : ie(a, m)) : void 0;
            return r.run(g, S);
          },
          deps: r.deps?.map((n) => `${m}${K}${n}`),
        };
    ...
    h.push({
      ...
      hooks: E.hooks,
      snapshotEvents:
        i && !i.has(m) ? [] : E.snapshotEvents?.map((y) => `${m}${K}${y}`),
    });
  }
  let f = null,
    c = null;
  function $(m) {
    for (const [E, v] of Object.entries(m))
      if (!ne.has(E) && l.has(E)) {
        ...
        for (const [C, k] of Object.entries(v))
          ne.has(C) || (c.facts[`${E}${K}${C}`] = k);
      }
  }
  c = yt({
    modules: h.map((m) => ({
  ...
  for (const m of Object.keys(t)) {
    ...
    if (!E) continue;
    const v = [];
    for (const C of Object.keys(E.schema.facts)) v.push(`${m}${K}${C}`);
    if (E.schema.derivations)
      for (const C of Object.keys(E.schema.derivations)) v.push(`${m}${K}${C}`);
    q.set(m, v);
  }
  const A = { names: null };
  function I() {
  ...
  return {
    ...
    getDistributableSnapshot(m) {
      ...
      for (const [k, F] of Object.entries(v.data)) {
        ...
        if (p > 0) {
          const b = k.slice(0, p),
            w = k.slice(p + K.length);
          C[b] || (C[b] = {}), (C[b][w] = F);
        } else C._root || (C._root = {}), (C._root[k] = F);
      }
      return { ...v, data: C };
    },
    watchDistributableSnapshot(m, E) {
      ...
      return c.watchDistributableSnapshot(v, (C) => {
        const k = {};
        for (const [F, p] of Object.entries(C.data)) {
          const b = F.indexOf(K);
          if (b > 0) {
            const w = F.slice(0, b),
              M = F.slice(b + K.length);
            k[w] || (k[w] = {}), (k[w][M] = p);
          } else k._root || (k._root = {}), (k._root[F] = p);
        }
        E({ ...C, data: k });
      });
    },
    registerModule(m, E) {
      ...
      const v = E,
        ...
        k = C ? Object.keys(v.crossModuleDeps) : [],
        F = {};
      for (const [y, R] of Object.entries(v.schema.facts))
        F[`${m}${K}${y}`] = R;
      const p = v.init
          ? (y) => {
              const R = ie(y, m);
        ...
        b = {};
      if (v.derive)
        for (const [y, R] of Object.entries(v.derive))
          b[`${m}${K}${y}`] = (r, n) => {
            const a = C ? ae(r, m, k) : ie(r, m),
              g = Fe(n, m);
            return R(a, g);
          };
      const w = {};
      if (v.events)
        for (const [y, R] of Object.entries(v.events))
          w[`${m}${K}${y}`] = (r, n) => {
            const a = ie(r, m);
            R(a, n);
          };
      const M = {};
      if (v.constraints)
        for (const [y, R] of Object.entries(v.constraints)) {
          const r = R;
          M[`${m}${K}${y}`] = {
            ...r,
            deps: r.deps?.map((n) => `${m}${K}${n}`),
            when: (n) => {
              const a = C ? ae(n, m, k) : ie(n, m);
              return r.when(a);
            },
            require:
              typeof r.require == "function"
      ...
      const N = {};
      if (v.resolvers)
        for (const [y, R] of Object.entries(v.resolvers)) {
          const r = R;
          N[`${m}${K}${y}`] = {
            ...r,
            resolve: async (n, a) => {
              const g = Ie(a.facts, t, I);
              await r.resolve(n, { facts: g[m], signal: a.signal });
            },
          };
        }
      const W = {};
      if (v.effects)
        for (const [y, R] of Object.entries(v.effects)) {
          const r = R;
          W[`${m}${K}${y}`] = {
            ...r,
            run: (n, a) => {
              const g = C ? ae(n, m, k) : ie(n, m),
                S = a ? (C ? ae(a, m, k) : ie(a, m)) : void 0;
              return r.run(g, S);
            },
            deps: r.deps?.map((n) => `${m}${K}${n}`),
          };
        }
      l.add(m), (t[m] = v), (A.names = null);
      const L = [];
      ...
      q.set(m, L),
        c.registerModule({
          ...
          hooks: v.hooks,
          snapshotEvents:
            i && !i.has(m) ? [] : v.snapshotEvents?.map((y) => `${m}${K}${y}`),
        });
    },
  };
}
    ...
    onAfterModuleInit: () => {
      if (e.initialFacts)
        for (const [f, c] of Object.entries(e.initialFacts))
          ne.has(f) || (o.facts[f] = c);
      if (s) {
        for (const [f, c] of Object.entries(s)) ne.has(f) || (o.facts[f] = c);
        s = null;
      }
    },
  });
...
function nr() {
  return { entries: new vt(rr), inflight: new Map() };
}
function ir() {
  return {
...
function cr(e, t, l, i) {
  ...
  function P(U, J) {
    ...
    J && (X.open = !0), (X.style.marginBottom = "4px");
    ...
    (oe.textContent = `${U} (`),
      ...
      (pe.textContent = "0"),
      X.appendChild(oe);
    const fe = document.createElement("table");
    ...
    He.appendChild(We), fe.appendChild(He);
    const Ke = document.createElement("tbody");
    return (
      fe.appendChild(Ke),
      X.appendChild(fe),
      { details: X, tbody: Ke, countSpan: pe }
    );
  }
  function T(U, J) {
    const X = document.createElement("details");
    ...
    (oe.textContent = `${U} (`),
      ...
      (pe.textContent = "0"),
      X.appendChild(oe);
    const fe = document.createElement("ul");
    return (
      Object.assign(fe.style, { margin: "0", paddingLeft: "16px" }),
      X.appendChild(fe),
      { details: X, list: fe, countSpan: pe }
    );
  }
  const m = P("Facts", !0);
  ...
  Object.assign(j.style, {
    ...
    h.addEventListener(
      ...
      (U) => {
        const J = h,
          X = J.scrollTop === 0 && U.deltaY < 0,
          oe = J.scrollTop + J.clientHeight >= J.scrollHeight && U.deltaY > 0;
        (X || oe) && U.preventDefault();
      },
      { passive: !1 },
    );
  let z = l,
  ...
  function Q() {
    ...
    for (const U of H) clearTimeout(U);
...
function ur(e, t, l, i) {
  ...
  (c.style.color = B.accent), (c.textContent = ` ${t} `);
...
function at(e, t, l) {
  ...
  if (s.length === 0 && o.length === 0 && d.length === 0 && f.length === 0) {
    Me.delete(e.flowSvg),
  ...
  const I = Z.nodeW + Z.colGap,
    O = [5, 5 + I, 5 + I * 2, 5 + I * 3, 5 + I * 4],
    x = O[4] + Z.nodeW + 5;
  function D(y) {
    let R = Z.startY + 12;
    return y.map((r) => {
      const n = { ...r, y: R };
      return (R += Z.nodeH + Z.nodeGap), n;
    });
  }
  const P = D(s.map((y) => ({ id: y, label: me(y, Z.labelMaxChars) }))),
    T = D(o.map((y) => ({ id: y, label: me(y, Z.labelMaxChars) }))),
    m = D(
      d.map((y) => ({
        id: y.id,
    ...
    v = D(f.map((y) => ({ id: y, label: me(y, Z.labelMaxChars) }))),
    C = Math.max(P.length, T.length, m.length, E.length, v.length, 1),
    k = Z.startY + 12 + C * (Z.nodeH + Z.nodeGap) + 8;
  e.flowSvg.replaceChildren(),
    e.flowSvg.setAttribute("viewBox", `0 0 ${x} ${k}`),
    e.flowSvg.setAttribute(
      "aria-label",
      `Dependency graph: ${s.length} facts, ${o.length} derivations, ${d.length} constraints, ${c.size} requirements, ${f.length} resolvers`,
    );
  const F = ["Facts", "Derivations", "Constraints", "Reqs", "Resolvers"];
  for (const [y, R] of F.entries()) {
    const r = document.createElementNS("http://www.w3.org/2000/svg", "text");
    r.setAttribute("x", String(O[y] ?? 0)),
      r.setAttribute("y", "10"),
  ...
  function b(y, R, r, n, a, g, S, j) {
    ...
    H.setAttribute("x", String(R)),
      ...
      H.setAttribute("width", String(Z.nodeW)),
      H.setAttribute("height", String(Z.nodeH)),
      H.setAttribute("rx", "3"),
      H.setAttribute("fill", j ? g + "33" : "none"),
      H.setAttribute("stroke", g),
      H.setAttribute("stroke-width", j ? "2" : "1"),
      H.setAttribute("opacity", S ? "0.35" : "1"),
      z.appendChild(H);
    const _ = document.createElementNS("http://www.w3.org/2000/svg", "text");
    return (
      _.setAttribute("x", String(R + 4)),
      _.setAttribute("y", String(r + 4)),
      ...
      p.nodes.set(`${y}:${n}`, { g: z, rect: H, text: _ }),
      { midX: R + Z.nodeW / 2, midY: r }
    );
  }
  function w(y, R, r, n, a, g) {
  ...
  const M = new Map(),
    ...
    L = new Map();
  for (const y of P) {
    const R = l.recentlyChangedFacts.has(y.id),
      r = b(0, O[0], y.y, y.id, y.label, B.text, !1, R);
    M.set(y.id, r);
  }
  for (const y of T) {
    const R = l.recentlyComputedDerivations.has(y.id),
      r = b(1, O[1], y.y, y.id, y.label, B.accent, !1, R);
    N.set(y.id, r);
  }
  for (const y of m) {
    const R = l.recentlyActiveConstraints.has(y.id),
      r = b(
        2,
        O[2],
        y.y,
  ...
  for (const y of E) {
    const R = y.status === "unmet" ? B.red : B.yellow,
      r = b(3, O[3], y.y, y.id, me(y.type, Z.labelMaxChars), R, !1, !1);
    L.set(y.id, r);
  }
  for (const y of v) {
    const R = h.some((r) => r.resolverId === y.id);
    b(4, O[4], y.y, y.id, y.label, R ? B.green : B.muted, !R, !1);
  }
  for (const y of T) {
    const R = l.derivationDeps.get(y.id),
      r = N.get(y.id);
    if (R && r)
      for (const n of R) {
        const a = M.get(n);
  ...
  for (const y of E) {
    const R = W.get(y.fromConstraint),
      r = L.get(y.id);
    R &&
      r &&
      w(R.midX + Z.nodeW / 2, R.midY, r.midX - Z.nodeW / 2, r.midY, B.muted);
  }
  for (const y of h) {
    const R = L.get(y.id);
    if (R) {
      const r = v.find((n) => n.id === y.resolverId);
      r && w(R.midX + Z.nodeW / 2, R.midY, O[4], r.y, B.green);
    }
  }
  Me.set(e.flowSvg, p);
}
...
function yr(e, t) {
  ...
  let i = 1 / 0,
    s = -1 / 0;
  for (const A of l)
    A.startMs < i && (i = A.startMs), A.endMs > s && (s = A.endMs);
  const o = performance.now();
  for (const A of t.inflight.values()) A < i && (i = A), o > s && (s = o);
  const d = s - i || 1,
    u = De - ye - 10,
    h = [],
    f = new Set();
  for (const A of l)
    f.has(A.resolver) || (f.add(A.resolver), h.push(A.resolver));
  for (const A of t.inflight.keys()) f.has(A) || (f.add(A), h.push(A));
  const c = h.slice(-12),
    $ = ge * c.length + 20;
  ...
  for (let A = 0; A <= q; A++) {
    ...
    D.setAttribute("x1", String(I)),
      D.setAttribute("y1", "10"),
      D.setAttribute("x2", String(I)),
  ...
  for (let A = 0; A < c.length; A++) {
    ...
    if (m !== void 0) {
      ...
      (k.textContent = `${I}: inflight ${(o - m).toFixed(0)}ms`),
        C.appendChild(k),
...
function br(e = {}) {
  ...
  let c = (n, a) => {
    ...
    x = lr(),
    D = nr(),
    P = s && typeof window < "u" && typeof document < "u" && ze(),
    T = null,
    m = 0,
    E = 1,
    v = 2,
  ...
  function y() {
    ...
    const n = $.refs,
      ...
      g = m;
    if (((m = 0), g & E)) {
      for (const S of N) dr(q, S);
      N.clear();
      for (const [S, { value: j, flash: z }] of M)
        Re(q, n.factsBody, S, j, z, $.flashTimers);
      M.clear(), (n.factsCount.textContent = String(q.size));
    }
    if ((g & v && ot(n, A, a, $.flashTimers), g & k))
      if (W) je(n, W.inflight.length, W.unmet.length);
      else {
        const S = Ce(a);
        S && je(n, S.inflight.length, S.unmet.length);
      }
    if (g & C)
      if (W) Oe(n, W.inflight, W.unmet);
      else {
        const S = Ce(a);
  ...
  function R(n, a) {
    $ && l && ur($.refs, n, a, f.events.size);
  }
  function r(n, a) {
    x.isRecording &&
      x.recordedEvents.length < sr &&
      x.recordedEvents.push({ timestamp: Date.now(), type: n, data: Zt(a) });
  }
  return {
    name: "devtools",
    onInit: (n) => {
      if (
        ...
        try {
          ...
          for (const [z, H] of Object.entries(j)) Re(q, g.factsBody, z, H, !1);
    ...
    onStart: (n) => {
      c("start", {}), R("start", {}), r("start", {});
    },
    onStop: (n) => {
      c("stop", {}), R("stop", {}), r("stop", {});
    },
    onDestroy: (n) => {
      c("destroy", {}),
        h.systems.delete(t),
    ...
    onFactDelete: (n, a) => {
      c("fact.delete", { key: n, prev: a }),
        ...
        $ && (N.add(n), M.delete(n), L(E), R("fact.delete", { key: n }));
    },
    onFactsBatch: (n) => {
      if (
    ...
    onReconcileEnd: (n) => {
      if (
        ...
        const a = performance.now() - I.lastReconcileStartMs;
        I.reconcileCount++,
          (I.reconcileTotalMs += a),
          (I.lastReconcileStartMs = 0);
      }
      if (x.isRecording && f.system && x.snapshots.length < or)
        try {
          x.snapshots.push({
            timestamp: Date.now(),
    ...
    onResolverComplete: (n, a, g) => {
      ...
      const S = I.resolverStats.get(n) ?? { count: 0, totalMs: 0, errors: 0 };
      if (
        (S.count++,
        (S.totalMs += g),
        I.resolverStats.set(n, S),
        I.resolverStats.size > st)
      ) {
        const z = I.resolverStats.keys().next().value;
        z !== void 0 && I.resolverStats.delete(z);
      }
    ...
    onResolverError: (n, a, g) => {
      ...
      const S = I.resolverStats.get(n) ?? { count: 0, totalMs: 0, errors: 0 };
      if ((S.errors++, I.resolverStats.set(n, S), I.resolverStats.size > st)) {
        const z = I.resolverStats.keys().next().value;
        z !== void 0 && I.resolverStats.delete(z);
      }
      const j = D.inflight.get(n);
    ...
    onTimeTravel: (n, a) => {
      if (
        ...
        try {
          ...
          for (const [j, z] of Object.entries(S))
            Re(q, $.refs.factsBody, j, z, !1);
...
async function wr(e, t, l) {
  if ((await Pe(t), Ne(l)))
    throw new Error("Failed to fetch profile: connection timeout");
  return {
    name: `User ${e.charAt(0).toUpperCase()}${e.slice(1)}`,
    avatar: `https://api.dicebear.com/7.x/initials/svg?seed=${e}`,
  };
}
async function Sr(e, t, l) {
  if ((await Pe(t), Ne(l)))
    throw new Error("Failed to fetch preferences: server error");
  const i = ["dark", "light", "auto"],
    s = ["en-US", "en-GB", "fr-FR", "de-DE", "ja-JP"],
    o = e.split("").reduce((d, u) => d + u.charCodeAt(0), 0);
  return { theme: i[o % i.length], locale: s[o % s.length] };
}
async function Er(e, t, l) {
  if ((await Pe(t), Ne(l)))
    throw new Error("Failed to fetch permissions: authorization error");
  const i = ["admin", "editor", "viewer"],
    s = ["dashboard", "reports", "settings", "billing", "api"],
    o = e.split("").reduce((h, f) => h + f.charCodeAt(0), 0),
    d = i[o % i.length],
    u = d === "admin" ? 5 : d === "editor" ? 3 : 2;
  return { role: d, features: s.slice(0, u) };
}
function de() {
  return {
    data: null,
...
const Le = {
  facts: {
    userId: G.string(),
    profile: G.object(),
    preferences: G.object(),
    permissions: G.object(),
    profileDelay: G.number(),
    preferencesDelay: G.number(),
    permissionsDelay: G.number(),
    profileFailRate: G.number(),
    preferencesFailRate: G.number(),
    permissionsFailRate: G.number(),
    loadRequested: G.boolean(),
    eventLog: G.object(),
  },
  derivations: {
    loadedCount: G.number(),
    totalResources: G.number(),
    allLoaded: G.boolean(),
    anyError: G.boolean(),
    anyLoading: G.boolean(),
    combinedStatus: G.string(),
    canStart: G.boolean(),
  },
  events: {
    setUserId: { value: G.string() },
    start: {},
    retryResource: { resource: G.string() },
    reloadAll: {},
    setDelay: { resource: G.string(), value: G.number() },
    setFailRate: { resource: G.string(), value: G.number() },
  },
  requirements: {
    FETCH_PROFILE: { userId: G.string() },
    FETCH_PREFERENCES: { userId: G.string() },
    FETCH_PERMISSIONS: { userId: G.string() },
  },
};
function ue(e, t, l, i) {
  const s = [...e.eventLog];
  s.push({ timestamp: Date.now(), event: t, resource: l, detail: i }),
    (e.eventLog = s);
}
const $r = jt("dashboard-loader", {
    schema: Le,
    init: (e) => {
      (e.userId = ""),
        (e.profile = de()),
        ...
        (e.permissions = de()),
        (e.profileDelay = 1e3),
        (e.preferencesDelay = 1500),
        (e.permissionsDelay = 2e3),
        (e.profileFailRate = 0),
        (e.preferencesFailRate = 0),
        (e.permissionsFailRate = 0),
        (e.loadRequested = !1),
        (e.eventLog = []);
    },
    derive: {
      loadedCount: (e) =>
        [e.profile, e.preferences, e.permissions].filter(
          (l) => l.status === "success",
        ).length,
      totalResources: () => 3,
      allLoaded: (e) =>
        [e.profile, e.preferences, e.permissions].every(
          (l) => l.status === "success",
        ),
      anyError: (e) =>
        [e.profile, e.preferences, e.permissions].some(
          (l) => l.status === "error",
        ),
      anyLoading: (e) =>
        [e.profile, e.preferences, e.permissions].some(
      ...
      combinedStatus: (e, t) => {
        ...
        if (
          ...
          return "Not started";
        const d = [e.profile, e.preferences, e.permissions].filter(
          (u) => u.status === "error",
        ).length;
        return s
          ? `Loading ${l} of 3...`
      ...
      canStart: (e) => {
        const t = e.userId.trim(),
          l = [e.profile, e.preferences, e.permissions].every(
            (i) => i.status === "idle",
          );
        return t.length > 0 && l;
      },
    ...
    events: {
      ...
      retryResource: (e, { resource: t }) => {
        const l = e[t];
        !l ||
          l.status !== "error" ||
          (e[t] = { ...l, status: "idle", error: null });
      },
      reloadAll: (e) => {
        (e.profile = de()),
          (e.preferences = de()),
      ...
      setDelay: (e, { resource: t, value: l }) => {
        const i = `${t}Delay`;
        i in e && (e[i] = l);
      },
      setFailRate: (e, { resource: t, value: l }) => {
        const i = `${t}FailRate`;
        i in e && (e[i] = l);
      },
    },
    constraints: {
      needsProfile: {
        ...
        when: (e) => {
          const t = e.userId.trim(),
            l = e.profile;
          return e.loadRequested && t !== "" && l.status === "idle";
        },
        require: (e) => ({ type: "FETCH_PROFILE", userId: e.userId.trim() }),
      },
      needsPreferences: {
        priority: 90,
        when: (e) => {
      ...
      fetchProfile: {
        ...
        resolve: async (e, t) => {
          ...
          (t.facts.profile = {
            ...
            ue(t.facts, "loading", "profile", `Attempt ${l.attempts + 1}`);
          try {
            const i = await wr(
              e.userId,
              t.facts.profileDelay,
              t.facts.profileFailRate,
            );
            (t.facts.profile = {
              data: i,
              status: "success",
              error: null,
              attempts: t.facts.profile.attempts,
              startedAt: t.facts.profile.startedAt,
              completedAt: Date.now(),
            }),
              ue(t.facts, "success", "profile", i.name);
          } catch (i) {
      ...
      fetchPreferences: {
        ...
        resolve: async (e, t) => {
          ...
          (t.facts.preferences = {
            ...
            ue(t.facts, "loading", "preferences", `Attempt ${l.attempts + 1}`);
          try {
            const i = await Sr(
              e.userId,
              t.facts.preferencesDelay,
              t.facts.preferencesFailRate,
            );
            (t.facts.preferences = {
              data: i,
              status: "success",
              error: null,
              attempts: t.facts.preferences.attempts,
              startedAt: t.facts.preferences.startedAt,
              completedAt: Date.now(),
            }),
              ue(t.facts, "success", "preferences", `${i.theme} / ${i.locale}`);
          } catch (i) {
            const s = i instanceof Error ? i.message : "Unknown error";
            throw (
              ((t.facts.preferences = {
              ...
              ue(t.facts, "error", "preferences", s),
              i)
            );
          }
        },
      ...
      fetchPermissions: {
        ...
        resolve: async (e, t) => {
          ...
          (t.facts.permissions = {
            ...
            ue(t.facts, "loading", "permissions", `Attempt ${l.attempts + 1}`);
          try {
            const i = await Er(
              e.userId,
              t.facts.permissionsDelay,
              t.facts.permissionsFailRate,
            );
            (t.facts.permissions = {
              data: i,
              status: "success",
              error: null,
              attempts: t.facts.permissions.attempts,
              startedAt: t.facts.permissions.startedAt,
              completedAt: Date.now(),
            }),
              ue(
                t.facts,
            ...
            throw (
              ...
              ue(t.facts, "error", "permissions", s),
              i)
            );
          }
        },
  ...
  se = Jt({ module: $r, plugins: [br({ name: "dashboard-loader" })] });
se.start();
const xr = [...Object.keys(Le.facts), ...Object.keys(Le.derivations)],
  Se = document.getElementById("dl-user-input"),
  wt = document.getElementById("dl-start-btn"),
  St = document.getElementById("dl-reload-btn"),
  Cr = document.getElementById("dl-combined-status"),
  $e = document.getElementById("dl-progress-fill"),
  Rr = document.getElementById("dl-loaded-count"),
  kr = document.getElementById("dl-profile-body"),
  Ar = document.getElementById("dl-profile-status"),
  Dr = document.getElementById("dl-profile-timing"),
  Ir = document.getElementById("dl-profile-attempts"),
  Or = document.getElementById("dl-prefs-body"),
  jr = document.getElementById("dl-prefs-status"),
  Mr = document.getElementById("dl-prefs-timing"),
  qr = document.getElementById("dl-prefs-attempts"),
  Br = document.getElementById("dl-perms-body"),
  Tr = document.getElementById("dl-perms-status"),
  _r = document.getElementById("dl-perms-timing"),
  Fr = document.getElementById("dl-perms-attempts"),
  qe = document.getElementById("dl-timeline"),
  ct = document.getElementById("dl-profile-delay"),
  Lr = document.getElementById("dl-profile-delay-val"),
  dt = document.getElementById("dl-profile-failrate"),
  zr = document.getElementById("dl-profile-failrate-val"),
  ut = document.getElementById("dl-prefs-delay"),
  Pr = document.getElementById("dl-prefs-delay-val"),
  ft = document.getElementById("dl-prefs-failrate"),
  Nr = document.getElementById("dl-prefs-failrate-val"),
  pt = document.getElementById("dl-perms-delay"),
  Hr = document.getElementById("dl-perms-delay-val"),
  mt = document.getElementById("dl-perms-failrate"),
  Wr = document.getElementById("dl-perms-failrate-val");
function Kr(e) {
  return e.startedAt
    ? `${(((e.completedAt ?? Date.now()) - e.startedAt) / 1e3).toFixed(1)}s`
    : "";
}
function Be(e, t, l, i, s, o, d) {
  if (
    ((l.textContent = e.status),
    (l.className = `dl-status-badge ${e.status}`),
    (i.textContent = Kr(e)),
    e.attempts > 1
      ? (s.textContent = `Attempt ${e.attempts}`)
      : (s.textContent = ""),
    e.status === "idle")
  )
    t.innerHTML =
  ...
  else if (e.status === "success" && e.data) {
    const u = e.data;
    let h = "";
    for (const [f, c] of Object.entries(u)) {
      const $ = Array.isArray(c) ? c.join(", ") : String(c);
      h += `<div><span class="dl-data-label">${be(f)}</span> ${be($)}</div>`;
    }
    t.innerHTML = `<div class="dl-data-preview">${h}</div>`;
  } else
    e.status === "error" &&
      (t.innerHTML = `
      <span class="dl-error-msg">${be(e.error ?? "Unknown error")}</span>
      <button class="dl-btn retry-btn" data-testid="${o}" data-resource="${d}">Retry</button>
    `);
}
function Et() {
  const e = se.facts,
  ...
  const h = t.anyError,
    f = t.allLoaded,
    c = e.eventLog;
  (Cr.textContent = d), (Rr.textContent = `${o} / 3`);
  const $ = Math.round((o / 3) * 100);
  if (
    (($e.style.width = `${$}%`),
    ($e.className = "dl-progress-fill"),
    f ? $e.classList.add("complete") : h && $e.classList.add("has-error"),
    (wt.disabled = !u),
    (St.disabled = [l, i, s].every((q) => q.status === "idle")),
    Be(l, kr, Ar, Dr, Ir, "dashboard-loader-profile-retry", "profile"),
    Be(i, Or, jr, Mr, qr, "dashboard-loader-prefs-retry", "preferences"),
    Be(s, Br, Tr, _r, Fr, "dashboard-loader-perms-retry", "permissions"),
    (Lr.textContent = `${e.profileDelay}ms`),
    (zr.textContent = `${e.profileFailRate}%`),
    (Pr.textContent = `${e.preferencesDelay}ms`),
    (Nr.textContent = `${e.preferencesFailRate}%`),
    (Hr.textContent = `${e.permissionsDelay}ms`),
    (Wr.textContent = `${e.permissionsFailRate}%`),
    c.length === 0)
  )
    qe.innerHTML =
    ...
    for (let q = c.length - 1; q >= 0; q--) {
      ...
      I.className = `dl-timeline-entry ${A.event}`;
      const x = new Date(A.timestamp).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      });
      (I.innerHTML = `
        <span class="dl-timeline-time">${x}</span>
        <span class="dl-timeline-resource">${be(A.resource)}</span>
        <span class="dl-timeline-detail">${be(A.detail)}</span>
      `),
        qe.appendChild(I);
    }
  }
...
function $t() {
  ...
  e.length !== 0 && (se.events.setUserId({ value: e }), se.events.start());
}
...
Se.addEventListener("input", () => {
  se.events.setUserId({ value: Se.value });
});
...
document.addEventListener("click", (e) => {
  const t = e.target;
  t.classList.contains("retry-btn") &&
    t.dataset.resource &&
    se.events.retryResource({ resource: t.dataset.resource });
});
ct.addEventListener("input", () => {
  se.events.setDelay({ resource: "profile", value: Number(ct.value) });
});
dt.addEventListener("input", () => {
  se.events.setFailRate({ resource: "profile", value: Number(dt.value) });
});
ut.addEventListener("input", () => {
  se.events.setDelay({ resource: "preferences", value: Number(ut.value) });
});
ft.addEventListener("input", () => {
  se.events.setFailRate({ resource: "preferences", value: Number(ft.value) });
});
pt.addEventListener("input", () => {
  se.events.setDelay({ resource: "permissions", value: Number(pt.value) });
});
mt.addEventListener("input", () => {
  se.events.setFailRate({ resource: "permissions", value: Number(mt.value) });
});
function be(e) {
