...
function ee(e, n, a, o, i, s) {
  return {
    ...
    describe(u) {
      return ee(e, n, a, o, u, s);
    },
    refine(u, d) {
      const p = [...(s ?? []), { predicate: u, message: d }];
      return ee([...e, u], n, a, o, i, p);
    },
    nullable() {
      return ee(
  ...
  array() {
    const e = (n, a, o, i, s) => {
      ...
      return {
        ...
        nonEmpty() {
          return e([...n, (p) => p.length > 0], a, o, i, d);
        },
        maxLength(p) {
          return e([...n, (m) => m.length <= p], a, o, i, d);
        },
        minLength(p) {
          return e([...n, (m) => m.length >= p], a, o, i, d);
        },
          ...
          [
            ...
            (s) => {
              for (const [u, d] of Object.entries(i)) {
                const p = s[u],
                  m = d;
                if (m && !m._validators.every((c) => c(p))) return !1;
              }
              return !0;
            },
          ],
...
function yt(e) {
    ...
    for (let L = 0; L < N.length; L++) {
      ...
      if (typeof v == "function" && !v(b)) {
        ...
        typeof M._lastFailedIndex == "number" &&
          ...
          ((r = ` (element at index ${M._lastFailedIndex} failed)`),
  ...
  const z = {
    ...
    subscribe(f, b) {
      for (const w of f) {
        const M = w;
        m.has(M) || m.set(M, new Set()), m.get(M).add(b);
      }
      return () => {
        for (const w of f) {
          const M = m.get(w);
          M && (M.delete(b), M.size === 0 && m.delete(w));
        }
      };
    },
    subscribeAll(f) {
    ...
    toObject() {
      const f = {};
      for (const b of p) d.has(b) && (f[b] = d.get(b));
      return f;
    },
  };
  return (
    (z.registerKeys = (f) => {
      for (const b of Object.keys(f)) ye.has(b) || ((n[b] = f[b]), p.add(b));
    }),
    z
  );
}
...
function be(e, n = 50) {
  ...
  function i(s, u) {
    ...
    if ((o.add(d), Array.isArray(d))) {
      for (const p of d) if (!i(p, u + 1)) return o.delete(d), !1;
      return o.delete(d), !0;
    }
    for (const p of Object.keys(d))
      if (a.has(p) || !i(d[p], u + 1)) return o.delete(d), !1;
    return o.delete(d), !0;
  }
  return i(e, 0);
}
...
function kt(e) {
  ...
  function y() {
    ...
    function g(S, I) {
      ...
      r.add(S), I.push(S);
      const P = n[S];
      if (P?.after) for (const W of P.after) n[W] && g(W, I);
      I.pop(), r.delete(S), t.add(S), l.push(S);
    }
    for (const S of Object.keys(n)) g(S, []);
    (B = l), (h = new Map(B.map((S, I) => [S, I])));
  }
  y(), E();
  function $(t, r) {
  ...
  async function w(t) {
    ...
    try {
      const S = r.when(a),
        I = await xe(S, g, `Constraint "${t}" timed out after ${g}ms`);
  ...
  function C(t) {
    ...
    for (const l of r.after)
      if (n[l] && !p.has(l) && !A.has(l) && !k.has(l)) return !1;
    return !0;
  }
  return {
    async evaluate(t) {
      ...
      if (!j || !t || t.size === 0) (g = l), (j = !0);
        ...
        for (const V of t) {
          const Y = x.get(V);
          if (Y) for (const te of Y) p.has(te) || _.add(te);
        }
        for (const V of T) p.has(V) || _.add(V);
        T.clear(), (g = [..._]);
        for (const V of l)
          if (!_.has(V)) {
            const Y = D.get(V);
...
function Rt(e) {
  ...
  function F(y) {
    ...
    try {
      ...
      return (
        ($.cachedValue = z), ($.isStale = !1), B(y, f), o?.(y, z, [...f]), z
      );
    } catch (z) {
      throw (s?.(y, z), z);
    } finally {
    ...
    (j = new Proxy(
      {},
      {
        get(y, $) {
          ...
          Be($);
          const R = A($);
          return R.isStale && F($), R.cachedValue;
        },
      },
    )),
    {
      get(y) {
        const $ = A(y);
        return $.isStale && F(y), $.cachedValue;
      },
      isStale(y) {
      ...
      registerDefinitions(y) {
        for (const [$, R] of Object.entries(y)) (n[$] = R), k($);
      },
    }
  );
}
function At(e) {
  ...
  function T(k, A) {
    ...
    if (F.dependencies) {
      for (const B of F.dependencies) if (A.has(B)) return !0;
      return !1;
    }
    return !0;
  }
  ...
  return {
    async runEffects(k) {
      const A = [];
      for (const F of Object.keys(n)) T(F, k) && A.push(F);
      await Promise.all(A.map(j)), (d = x());
    },
    async runAll() {
      const k = Object.keys(n);
      await Promise.all(
        k.map((A) => (c(A).enabled ? j(A) : Promise.resolve())),
      ),
        (d = x());
    },
    disable(k) {
    ...
    registerDefinitions(k) {
      for (const [A, F] of Object.entries(k)) (n[A] = F), m(A);
    },
  };
}
function jt(e = {}) {
  const {
    ...
    s = new Map();
  function u(d) {
    const p = n * Math.pow(o, d - 1);
    return Math.min(p, i);
  }
  return {
    scheduleRetry(d, p, m, c, x) {
      if (c > a) return null;
      const T = u(c),
        O = {
          source: d,
    ...
    processDueRetries() {
      const d = Date.now(),
        p = [];
      for (const [m, c] of s) c.nextRetryTime <= d && (p.push(c), s.delete(m));
      return p;
    },
    cancelRetry(d) {
      s.delete(d);
...
function Dt(e = {}) {
  ...
  return {
    handleError(c, x, T, O) {
      ...
      let j = m(c, x, T instanceof Error ? T : new Error(String(T)));
      if (j === "retry-later") {
        const k = (d.get(x) ?? 0) + 1;
        d.set(x, k),
          u.scheduleRetry(c, x, O, k) ||
...
function Mt(e) {
  ...
  function B(f, b) {
    return A(f) ? b.type === f.requirement : F(f) ? f.requirement(b) : !1;
  }
  function h(f) {
    const b = f.type,
      w = D.get(b);
    ...
    for (const [M, N] of Object.entries(n))
      if (B(N, f)) {
        if (!D.has(b)) {
          if (D.size >= j) {
    ...
    for (let L = 1; L <= N.attempts; L++) {
      ...
      try {
        ...
        if (
          ...
          return;
        if (N.shouldRetry && !N.shouldRetry(K, L)) break;
        if (L < N.attempts) {
          if (w.signal.aborted) return;
          const t = Ve(N, L);
          if (
...
function qt(e) {
  ...
  return {
    ...
    takeSnapshot(B) {
      ...
      const h = { id: c++, timestamp: Date.now(), facts: A(), trigger: B };
      for (
        m < p.length - 1 && p.splice(m + 1), p.push(h), m = p.length - 1;
        p.length > d;
      )
        p.shift(), m--;
      return i?.(h), h;
    ...
    goBack(B = 1) {
      if (!u || p.length === 0) return;
      let h = m,
        E = m,
        y = O.find((R) => m > R.startIndex && m <= R.endIndex);
      if (y) E = y.startIndex;
      else if (O.find((R) => m === R.startIndex)) {
        const R = O.find((z) => z.endIndex < m && m - z.endIndex <= B);
        E = R ? R.startIndex : Math.max(0, m - B);
      } else E = Math.max(0, m - B);
      if (h === E) return;
      m = E;
      const $ = p[m];
      $ && (this.restore($), s?.(h, E));
    ...
    goForward(B = 1) {
      ...
      let h = m,
        E = m,
        y = O.find((R) => m >= R.startIndex && m < R.endIndex);
      if ((y ? (E = y.endIndex) : (E = Math.min(p.length - 1, m + B)), h === E))
        return;
      m = E;
      const $ = p[m];
      $ && (this.restore($), s?.(h, E));
    ...
    export() {
      return JSON.stringify({ version: 1, snapshots: p, currentIndex: m });
    },
    import(B) {
      if (u)
        try {
          ...
          (p.length = 0), p.push(...h.snapshots), (m = h.currentIndex);
          const E = p[m];
          E && this.restore(E);
        } catch (h) {
    ...
    endChangeset() {
      !u ||
        D === null ||
        (m > j && O.push({ label: D, startIndex: j, endIndex: m }),
        (D = null),
        (j = -1));
    },
    pause() {
...
function ct(e) {
  ...
  if (e.modules.some((t) => t.snapshotEvents)) {
    ...
    for (const t of e.modules) {
      const r = t;
      if (r.snapshotEvents) for (const l of r.snapshotEvents) d.add(l);
      else if (r.events) for (const l of Object.keys(r.events)) d.add(l);
    }
  }
  let p = 0,
    m = !1,
  ...
  let x = Dt({
    ...
    { store: j, facts: k } = wt({
      ...
      onChange: (t, r, l) => {
        c.emitFactSet(t, r, l),
          T(t),
          !D?.isRestoring && (p === 0 && (m = !0), w.changedKeys.add(t), M());
      },
      onBatch: (t) => {
        c.emitFactsBatch(t);
        const r = [];
        for (const l of t) r.push(l.key);
        if ((O(r), !D?.isRestoring)) {
          p === 0 && (m = !0);
  ...
  const F = At({
      ...
      onError: (t, r) => {
        x.handleError("effect", t, r), c.emitEffectError(t, r);
      },
    }),
    B = kt({
      definitions: s,
      ...
      onError: (t, r) => {
        x.handleError("constraint", t, r), c.emitConstraintError(t, r);
      },
    }),
    h = Mt({
      definitions: u,
      ...
      onComplete: (t, r, l) => {
        c.emitResolverComplete(t, r, l),
          ...
          B.markResolved(r.fromConstraint);
      },
      onError: (t, r, l) => {
        x.handleError("resolver", t, l, r), c.emitResolverError(t, r, l);
      },
  ...
  const K = new Proxy(
    ...
    v = {
      ...
      explain(t) {
        const r = w.previousRequirements.all().find((V) => V.id === t);
        if (!r) return null;
        const l = B.getState(r.fromConstraint),
          g = h.getStatus(t),
          S = {},
          I = j.toObject();
        for (const [V, Y] of Object.entries(I)) S[V] = Y;
        const P = [
...
`)
      ...
      watchDistributableSnapshot(t, r) {
        let { includeDerivations: l, excludeDerivations: g } = t,
          S = Object.keys(o),
          I;
        if ((l ? (I = l.filter((W) => S.includes(W))) : (I = S), g)) {
          const W = new Set(g);
  ...
  function C(t) {
    ...
    Object.assign(n, t.schema),
      ...
      t.constraints &&
        (Object.assign(s, t.constraints), B.registerDefinitions(t.constraints)),
      t.resolvers &&
  ...
  (v.registerModule = C), c.emitInit(v);
...
function _t(e) {
  ...
  function u(d) {
    ...
    if (p?.crossModuleDeps)
      for (const m of Object.keys(p.crossModuleDeps)) n.includes(m) && u(m);
    s.pop(), o.delete(d), a.add(d), i.push(d);
  }
  for (const d of n) u(d);
  return i;
}
var Je = new WeakMap(),
  Ye = new WeakMap(),
...
function zt(e) {
  ...
  if (Array.isArray(s)) {
    const h = s,
      E = Object.keys(n).filter((y) => !h.includes(y));
    if (E.length > 0)
      throw new Error(
  ...
  for (const h of Object.keys(n)) {
    if (h.includes(U))
      throw new Error(
        `[Directive] Module name "${h}" contains the reserved separator "${U}". Module names cannot contain "${U}".`,
      );
    const E = n[h];
    if (E) {
      for (const y of Object.keys(E.schema.facts))
        if (y.includes(U))
          throw new Error(
            `[Directive] Schema key "${y}" in module "${h}" contains the reserved separator "${U}". Schema keys cannot contain "${U}".`,
          );
    }
  }
  const p = [];
  for (const h of i) {
    ...
    const y = E.crossModuleDeps && Object.keys(E.crossModuleDeps).length > 0,
      $ = y ? Object.keys(E.crossModuleDeps) : [],
      R = {};
    for (const [v, C] of Object.entries(E.schema.facts)) R[`${h}${U}${v}`] = C;
    const z = {};
    if (E.schema.derivations)
      for (const [v, C] of Object.entries(E.schema.derivations))
        z[`${h}${U}${v}`] = C;
    const f = {};
    if (E.schema.events)
      for (const [v, C] of Object.entries(E.schema.events))
        f[`${h}${U}${v}`] = C;
    const b = E.init
        ? (v) => {
            const C = ie(v, h);
      ...
      w = {};
    if (E.derive)
      for (const [v, C] of Object.entries(E.derive))
        w[`${h}${U}${v}`] = (t, r) => {
          const l = y ? ae(t, h, $) : ie(t, h),
            g = Te(r, h);
          return C(l, g);
        };
    const M = {};
    if (E.events)
      for (const [v, C] of Object.entries(E.events))
        M[`${h}${U}${v}`] = (t, r) => {
          const l = ie(t, h);
          C(l, r);
        };
    const N = {};
    if (E.constraints)
      for (const [v, C] of Object.entries(E.constraints)) {
        const t = C;
        N[`${h}${U}${v}`] = {
          ...t,
          deps: t.deps?.map((r) => `${h}${U}${r}`),
          when: (r) => {
            const l = y ? ae(r, h, $) : ie(r, h);
            return t.when(l);
          },
          require:
    ...
    const K = {};
    if (E.resolvers)
      for (const [v, C] of Object.entries(E.resolvers)) {
        const t = C;
        K[`${h}${U}${v}`] = {
          ...t,
          resolve: async (r, l) => {
            const g = je(l.facts, n, () => Object.keys(n));
            await t.resolve(r, { facts: g[h], signal: l.signal });
          },
        };
      }
    const L = {};
    if (E.effects)
      for (const [v, C] of Object.entries(E.effects)) {
        const t = C;
        L[`${h}${U}${v}`] = {
          ...t,
          run: (r, l) => {
            const g = y ? ae(r, h, $) : ie(r, h),
              S = l ? (y ? ae(l, h, $) : ie(l, h)) : void 0;
            return t.run(g, S);
          },
          deps: t.deps?.map((r) => `${h}${U}${r}`),
        };
    ...
    p.push({
      ...
      hooks: E.hooks,
      snapshotEvents:
        o && !o.has(h) ? [] : E.snapshotEvents?.map((v) => `${h}${U}${v}`),
    });
  }
  let m = null,
    c = null;
  ...
  for (const h of Object.keys(n)) {
    ...
    if (!E) continue;
    const y = [];
    for (const $ of Object.keys(E.schema.facts)) y.push(`${h}${U}${$}`);
    if (E.schema.derivations)
      for (const $ of Object.keys(E.schema.derivations)) y.push(`${h}${U}${$}`);
    T.set(h, y);
  }
  const O = { names: null };
  function D() {
  ...
  return {
    ...
    watchDistributableSnapshot(h, E) {
      ...
      return c.watchDistributableSnapshot(y, ($) => {
        const R = {};
        for (const [z, f] of Object.entries($.data)) {
          const b = z.indexOf(U);
          if (b > 0) {
            const w = z.slice(0, b),
              M = z.slice(b + U.length);
            R[w] || (R[w] = {}), (R[w][M] = f);
    ...
    registerModule(h, E) {
      ...
      if (h.includes(U))
        throw new Error(
          `[Directive] Module name "${h}" contains the reserved separator "${U}".`,
        );
      if (re.has(h))
        throw new Error(
      ...
      for (const v of Object.keys(E.schema.facts))
        if (v.includes(U))
          throw new Error(
            `[Directive] Schema key "${v}" in module "${h}" contains the reserved separator "${U}".`,
          );
      const y = E,
      ...
      if (y.effects)
        for (const [v, C] of Object.entries(y.effects)) {
          ...
          K[`${h}${U}${v}`] = {
            ...t,
            run: (r, l) => {
              const g = $ ? ae(r, h, R) : ie(r, h),
                S = l ? ($ ? ae(l, h, R) : ie(l, h)) : void 0;
              return t.run(g, S);
            },
            deps: t.deps?.map((r) => `${h}${U}${r}`),
          };
        }
      a.add(h), (n[h] = y), (O.names = null);
      const L = [];
      for (const v of Object.keys(y.schema.facts)) L.push(`${h}${U}${v}`);
      ...
      T.set(h, L),
        c.registerModule({
          ...
          hooks: y.hooks,
          snapshotEvents:
            o && !o.has(h) ? [] : y.snapshotEvents?.map((v) => `${h}${U}${v}`),
        });
    },
  };
}
    ...
    {},
    {
      get(i, s) {
        if (typeof s != "symbol" && !re.has(s))
          return s === "$store" || s === "$snapshot" ? e[s] : e[`${n}${U}${s}`];
      },
      set(i, s, u) {
      ...
      has(i, s) {
        return typeof s == "symbol" || re.has(s) ? !1 : `${n}${U}${s}` in e;
      },
      deleteProperty(i, s) {
        return typeof s == "symbol" || re.has(s)
          ? !1
...
function je(e, n, a) {
  ...
  const i = new Proxy(
    ...
    {
      get(s, u) {
        if (typeof u != "symbol" && !re.has(u) && Object.hasOwn(n, u))
          return ie(e, u);
      },
      has(s, u) {
        return typeof u == "symbol" || re.has(u) ? !1 : Object.hasOwn(n, u);
      },
      ownKeys() {
        return a();
      },
      ...
      {},
      {
        get(p, m) {
          if (typeof m != "symbol" && !re.has(m)) {
            if (m === "self") return ie(e, n);
            if (s.has(m)) return ie(e, m);
          }
        },
        has(p, m) {
...
function Te(e, n) {
  ...
  const o = new Proxy(
    ...
    {
      get(i, s) {
        if (typeof s != "symbol" && !re.has(s)) return e[`${n}${U}${s}`];
      },
      has(i, s) {
        return typeof s == "symbol" || re.has(s) ? !1 : `${n}${U}${s}` in e;
      },
    ...
    onAfterModuleInit: () => {
      if (e.initialFacts)
        for (const [m, c] of Object.entries(e.initialFacts))
          re.has(m) || (s.facts[m] = c);
      if (i) {
        for (const [m, c] of Object.entries(i)) re.has(m) || (s.facts[m] = c);
        i = null;
      }
    },
  });
...
function Ht() {
  return { entries: new ut(Ut), inflight: new Map() };
}
function Vt() {
  return {
...
function Qt(e, n, a, o) {
  ...
  function F(H, J) {
    ...
    const Pe = document.createElement("thead"),
    ...
    Pe.appendChild(Fe), ue.appendChild(Pe);
  ...
  L.setAttribute("width", "100%"),
    ...
    L.setAttribute("viewBox", `0 0 ${Ae} 60`),
  ...
  function te(H) {
    H.key === "d" &&
      H.shiftKey &&
      (H.ctrlKey || H.metaKey) &&
      (H.preventDefault(), P ? V() : _());
  }
  document.addEventListener("keydown", te);
  function Z() {
    i || (document.body.appendChild(d), document.body.appendChild(p));
  }
  document.body
    ? Z()
  ...
  function X() {
    ...
    W.clear(), d.remove(), p.remove(), u.remove();
  }
  return {
    refs: {
      container: p,
...
function Ce(e, n, a, o, i, s) {
  ...
  if (d) {
    ...
    Object.assign(p.style, { padding: "2px 4px", color: q.muted }),
      (p.textContent = a);
...
function nt(e, n, a, o) {
  ...
  const s = new Set(i);
  for (const [u, d] of n) s.has(u) || (d.remove(), n.delete(u));
  for (const u of i) {
    let d;
    try {
      d = dt(a.read(u));
...
function er(e, n, a, o) {
  ...
  (c.style.color = q.accent), (c.textContent = ` ${n} `);
...
function tr(e, n) {
  ...
  if (n.resolverStats.size > 0) {
    ...
    for (const [u, d] of s) {
      const p = d.count > 0 ? (d.totalMs / d.count).toFixed(1) : "0",
        m = document.createElement("div");
      (m.style.paddingLeft = "8px"),
        (m.textContent = `${u}: ${d.count}x, avg ${p}ms${d.errors > 0 ? `, ${d.errors} err` : ""}`),
...
function ir(e, n, a, o, i) {
  for (const s of a) {
    const u = e.nodes.get(`0:${s}`);
    if (!u) continue;
    const d = n.recentlyChangedFacts.has(s);
  ...
  for (const s of o) {
    ...
    if (!u) continue;
    const d = n.recentlyComputedDerivations.has(s);
  ...
  for (const s of i) {
    ...
    if (!u) continue;
    const d = n.recentlyActiveConstraints.has(s),
      p = u.rect.getAttribute("stroke") ?? q.muted;
    u.rect.setAttribute("fill", d ? p + "33" : "none"),
      u.rect.setAttribute("stroke-width", d ? "2" : "1");
  }
}
function ot(e, n, a) {
  ...
  const F = A(i.map((v) => ({ id: v, label: fe(v, Q.labelMaxChars) }))),
    B = A(s.map((v) => ({ id: v, label: fe(v, Q.labelMaxChars) }))),
    h = A(
      u.map((v) => ({
        id: v.id,
    ...
    y = A(m.map((v) => ({ id: v, label: fe(v, Q.labelMaxChars) }))),
    $ = Math.max(F.length, B.length, h.length, E.length, y.length, 1),
  ...
  function b(v, C, t, r, l, g, S, I) {
    ...
    return (
      ...
      f.nodes.set(`${v}:${r}`, { g: P, rect: W, text: _ }),
  ...
  for (const v of F) {
    const C = a.recentlyChangedFacts.has(v.id),
      t = b(0, j[0], v.y, v.id, v.label, q.text, !1, C);
    M.set(v.id, t);
  }
  for (const v of B) {
    const C = a.recentlyComputedDerivations.has(v.id),
      t = b(1, j[1], v.y, v.id, v.label, q.accent, !1, C);
    N.set(v.id, t);
  }
  for (const v of h) {
  ...
  for (const v of E) {
    const C = v.status === "unmet" ? q.red : q.yellow,
      t = b(3, j[3], v.y, v.id, fe(v.type, Q.labelMaxChars), C, !1, !1);
    L.set(v.id, t);
  }
  for (const v of y) {
    const C = p.some((t) => t.resolverId === v.id);
    b(4, j[4], v.y, v.id, v.label, C ? q.green : q.muted, !C, !1);
  }
  for (const v of B) {
...
function sr(e, n) {
  ...
  const u = i - o || 1,
    ...
    m = new Set();
  for (const O of a)
    m.has(O.resolver) || (m.add(O.resolver), p.push(O.resolver));
  for (const O of n.inflight.keys()) m.has(O) || (m.add(O), p.push(O));
  const c = p.slice(-12),
    x = he * c.length + 20;
  ...
  for (let O = 0; O < c.length; O++) {
    ...
    F.setAttribute("x", String(ge - 4)),
      F.setAttribute("y", String(j + he / 2 + 3)),
      F.setAttribute("fill", q.muted),
      F.setAttribute("font-size", "7"),
      F.setAttribute("font-family", q.font),
      F.setAttribute("text-anchor", "end"),
      (F.textContent = fe(D, 12)),
      e.timelineSvg.appendChild(F);
    const B = a.filter((E) => E.resolver === D);
    for (const E of B) {
      const y = ge + ((E.startMs - o) / u) * d,
        $ = Math.max(((E.endMs - E.startMs) / u) * d, et),
      ...
      const z = document.createElementNS("http://www.w3.org/2000/svg", "title"),
        f = E.endMs - E.startMs;
      (z.textContent = `${D}: ${f.toFixed(1)}ms${E.error ? " (error)" : ""}`),
        R.appendChild(z),
        e.timelineSvg.appendChild(R);
    }
    const h = n.inflight.get(D);
    if (h !== void 0) {
      const E = ge + ((h - o) / u) * d,
        y = Math.max(((s - h) / u) * d, et),
      ...
      (R.textContent = `${D}: inflight ${(s - h).toFixed(0)}ms`),
        $.appendChild(R),
      ...
      n = {
        ...
        subscribe(a, o) {
          ...
          if (!i) {
            let s = !1,
              u = setInterval(() => {
                const p = o ? e.get(o) : e.values().next().value;
                p && !s && ((s = !0), p.subscribers.add(a));
              }, 100),
              d = setTimeout(() => clearInterval(u), 1e4);
            return () => {
              clearInterval(u), clearTimeout(d);
              for (const p of e.values()) p.subscribers.delete(a);
            };
          }
          return (
            i.subscribers.add(a),
...
function ar(e = {}) {
  ...
  let c = (r, l) => {
    ...
    k = Gt(),
    A = Ht(),
    F = i && typeof window < "u" && typeof document < "u" && Le(),
    B = null,
    h = 0,
    E = 1,
    y = 2,
  ...
  function v() {
    ...
    const r = x.refs,
      ...
      g = h;
    if (((h = 0), g & E)) {
      for (const S of N) Zt(T, S);
      N.clear();
      for (const [S, { value: I, flash: P }] of M)
        Ce(T, r.factsBody, S, I, P, x.flashTimers);
      M.clear(), (r.factsCount.textContent = String(T.size));
    }
    if ((g & y && nt(r, O, l, x.flashTimers), g & R))
      if (K) De(r, K.inflight.length, K.unmet.length);
      else {
        const S = $e(l);
        S && De(r, S.inflight.length, S.unmet.length);
      }
    if (g & $)
      if (K) Oe(r, K.inflight, K.unmet);
      else {
        const S = $e(l);
        S && Oe(r, S.inflight, S.unmet);
      }
    g & z && tr(r, D),
      g & f && ot(r, l, j),
      g & b && it(r, l),
  ...
  function C(r, l) {
    x && a && er(x.refs, r, l, m.events.size);
  }
  function t(r, l) {
    k.isRecording &&
      k.recordedEvents.length < Jt &&
      k.recordedEvents.push({ timestamp: Date.now(), type: r, data: Nt(l) });
  }
  return {
    name: "devtools",
    onInit: (r) => {
      if (
        ...
        try {
          ...
          for (const [P, W] of Object.entries(I)) Ce(T, g.factsBody, P, W, !1);
    ...
    onFactDelete: (r, l) => {
      c("fact.delete", { key: r, prev: l }),
        ...
        x && (N.add(r), M.delete(r), L(E), C("fact.delete", { key: r }));
    },
    onFactsBatch: (r) => {
      if (
        ...
        for (const l of r)
          l.type === "delete"
            ...
            : (j.recentlyChangedFacts.add(l.key),
              ...
              N.delete(l.key));
        L(E), C("facts.batch", { count: r.length });
      }
    },
    ...
    onReconcileEnd: (r) => {
      ...
      if (k.isRecording && m.system && k.snapshots.length < Yt)
        try {
          k.snapshots.push({
            timestamp: Date.now(),
    ...
    onResolverComplete: (r, l, g) => {
      ...
      const S = D.resolverStats.get(r) ?? { count: 0, totalMs: 0, errors: 0 };
      if (
        (S.count++,
        (S.totalMs += g),
        D.resolverStats.set(r, S),
        D.resolverStats.size > rt)
      ) {
        const P = D.resolverStats.keys().next().value;
        P !== void 0 && D.resolverStats.delete(P);
      }
      const I = A.inflight.get(r);
      A.inflight.delete(r),
        I !== void 0 &&
          A.entries.push({
            resolver: r,
    ...
    onResolverError: (r, l, g) => {
      ...
      const S = D.resolverStats.get(r) ?? { count: 0, totalMs: 0, errors: 0 };
      if ((S.errors++, D.resolverStats.set(r, S), D.resolverStats.size > rt)) {
        const P = D.resolverStats.keys().next().value;
        P !== void 0 && D.resolverStats.delete(P);
      }
      const I = A.inflight.get(r);
...
const Me = {
  ...
  pe = [];
function ve(e, n, a) {
  pe.unshift({ time: Date.now(), event: e, detail: n, type: a }),
    pe.length > 50 && (pe.length = 50);
}
const ze = {
    facts: {
      users: ne.object(),
      loadingIds: ne.object(),
      batchCount: ne.number(),
      totalRequests: ne.number(),
      batchWindowMs: ne.number(),
      failItemId: ne.number(),
      validationErrors: ne.object(),
    },
    derivations: {
      userCount: ne.number(),
      loadingCount: ne.number(),
      batchEfficiency: ne.string(),
      hasValidationErrors: ne.boolean(),
    },
    events: {
      loadUser: { id: ne.number() },
      loadRange: { start: ne.number(), count: ne.number() },
      setBatchWindow: { value: ne.number() },
      setFailItemId: { value: ne.number() },
      injectSchemaError: {},
      clearUsers: {},
      resetAll: {},
    },
    requirements: { LOAD_USER: { userId: ne.number() } },
  },
  cr = St("batch-loader", {
    schema: ze,
    init: (e) => {
    ...
    derive: {
      userCount: (e) => e.users.length,
      loadingCount: (e) => e.loadingIds.length,
      batchEfficiency: (e) =>
        e.totalRequests === 0
          ? "N/A"
          : `${e.batchCount} batches / ${e.totalRequests} requests`,
      hasValidationErrors: (e) => e.validationErrors.length > 0,
    },
    events: {
      loadUser: (e, { id: n }) => {
        !e.loadingIds.includes(n) &&
          !e.users.find((a) => a.id === n) &&
          ((e.loadingIds = [...e.loadingIds, n]),
          (e.totalRequests = e.totalRequests + 1));
      },
      loadRange: (e, { start: n, count: a }) => {
        const o = [];
        for (let i = n; i < n + a; i++)
          !e.loadingIds.includes(i) &&
            !e.users.find((s) => s.id === i) &&
            o.push(i);
        o.length > 0 &&
          ((e.loadingIds = [...e.loadingIds, ...o]),
          (e.totalRequests = e.totalRequests + o.length));
      },
      setBatchWindow: (e, { value: n }) => {
        e.batchWindowMs = n;
      },
      setFailItemId: (e, { value: n }) => {
        e.failItemId = n;
      },
      injectSchemaError: (e) => {
        (e.users = "not-an-array"),
          (e.validationErrors = [
            ...e.validationErrors,
    ...
    constraints: {
      needsLoad: {
        priority: 50,
        when: (e) => e.loadingIds.length > 0,
        require: (e) => ({ type: "LOAD_USER", userId: e.loadingIds[0] }),
      },
    },
    resolvers: {
      loadUser: {
        requirement: "LOAD_USER",
        batch: { enabled: !0, windowMs: 50 },
        resolveBatchWithResults: async (e, n) => {
          const a = e.map((p) => p.userId);
          ve(
            "batch",
            `batch formed: ${a.length} items [${a.join(", ")}]`,
            "batch",
          ),
            (n.facts.batchCount = n.facts.batchCount + 1),
            await new Promise((p) => setTimeout(p, 150 + Math.random() * 100));
          const o = n.facts.failItemId,
            i = a.map((p) =>
              p === o
                ? (ve("error", `user ${p}: simulated failure`, "error"),
                  { success: !1, error: new Error(`Failed to load user ${p}`) })
                : Me[p]
                  ? { success: !0 }
                  : (ve("error", `user ${p}: not found`, "error"),
                    { success: !1, error: new Error(`User ${p} not found`) }),
            ),
            s = a.filter((p) => p !== o && Me[p]).map((p) => Me[p]);
          if (s.length > 0) {
            const p = n.facts.users;
            n.facts.users = [...p, ...s];
          }
          const u = n.facts.loadingIds;
          n.facts.loadingIds = u.filter((p) => !a.includes(p));
          const d = i.filter((p) => p.success).length;
          return (
            ve(
              "success",
              `batch resolved: ${d}/${a.length} success`,
              "success",
            ),
            i
          );
  ...
  se = Tt({ module: cr, plugins: [ar({ name: "batch-resolver" })] });
se.start();
const Se = document.getElementById("bl-user-list"),
  ur = document.getElementById("bl-insp-user-count"),
  dr = document.getElementById("bl-batch-window-val"),
  st = document.getElementById("bl-fail-item"),
  qe = document.getElementById("bl-timeline");
function Ee(e) {
...
function ft() {
  ...
  if (n.length === 0 && a.length === 0)
    ...
    for (const o of a) {
      const i = document.createElement("div");
      (i.className = "bl-user-item loading"),
        (i.innerHTML = `<span class="bl-user-id">#${o}</span> Loading...`),
        Se.appendChild(i);
    }
    for (const o of n) {
      const i = document.createElement("div");
      (i.className = "bl-user-item"),
        i.setAttribute("data-testid", `bl-user-${o.id}`),
        (i.innerHTML = `
        <span class="bl-user-id">#${o.id}</span>
        <span class="bl-user-name">${Ee(o.name)}</span>
        <span class="bl-user-role">${Ee(o.role)}</span>
      `),
        Se.appendChild(i);
    }
  }
  if (
    ((ur.textContent = String(se.read("userCount"))),
    (dr.textContent = `${e.batchWindowMs}ms`),
    pe.length === 0)
  )
    qe.innerHTML =
    ...
    for (const o of pe) {
      const i = document.createElement("div");
      i.className = `bl-timeline-entry ${o.type}`;
      const u = new Date(o.time).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      });
      (i.innerHTML = `
        <span class="bl-timeline-time">${u}</span>
        <span class="bl-timeline-event">${Ee(o.event)}</span>
        <span class="bl-timeline-detail">${Ee(o.detail)}</span>
      `),
        qe.appendChild(i);
    }
  }
}
const fr = [...Object.keys(ze.facts), ...Object.keys(ze.derivations)];
se.subscribe(fr, ft);
for (let e = 1; e <= 5; e++)
  document.getElementById(`bl-load-${e}`).addEventListener("click", () => {
    se.events.loadUser({ id: e });
  });
document.getElementById("bl-load-all-5").addEventListener("click", () => {
  se.events.loadRange({ start: 1, count: 5 });
});
document.getElementById("bl-load-20").addEventListener("click", () => {
  se.events.loadRange({ start: 1, count: 20 });
});
document.getElementById("bl-inject-schema").addEventListener("click", () => {
  se.events.injectSchemaError();
});
document.getElementById("bl-clear").addEventListener("click", () => {
  se.events.clearUsers();
});
document.getElementById("bl-reset").addEventListener("click", () => {
  se.events.resetAll();
});
st.addEventListener("change", () => {
  se.events.setFailItemId({ value: Number(st.value) });
});
ft();
