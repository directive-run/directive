...
function te(e, t, l, i, s, o) {
  return {
    ...
    refine(d, c) {
      const g = [...(o ?? []), { predicate: d, message: c }];
      return te([...e, d], t, l, i, s, g);
    },
    nullable() {
      return te(
  ...
  array() {
    const e = (t, l, i, s, o) => {
      ...
      return {
        ...
        nonEmpty() {
          return e([...t, (g) => g.length > 0], l, i, s, c);
        },
        maxLength(g) {
          return e([...t, (p) => p.length <= g], l, i, s, c);
        },
        minLength(g) {
          return e([...t, (p) => p.length >= g], l, i, s, c);
        },
          ...
          [
            ...
            (o) => {
              for (const [d, c] of Object.entries(s)) {
                const g = o[d],
                  p = c;
                if (p && !p._validators.every((u) => u(g))) return !1;
              }
              return !0;
            },
          ],
  ...
  function m(f, b) {
    ...
    for (let F = 0; F < N.length; F++) {
      ...
      if (typeof v == "function" && !v(b)) {
        ...
        throw new Error(
          `[Directive] Validation failed for "${f}": expected ${H}, got ${k} ${r}${a}${n}`,
        );
      }
    }
  }
  ...
  function $(f, b, w) {
    ...
    try {
      ...
      while (j.length > 0) {
        ...
        const N = [...j];
        j.length = 0;
        for (const H of N) l?.(H.key, H.value, H.prev), x(H.key);
        y();
      }
    } finally {
      D = !1;
  ...
  const L = {
    ...
    set(f, b) {
      ...
      Object.is(w, b) ||
        ...
        g.add(f),
        E > 0
          ? (T.push({ key: f, value: b, prev: w, type: "set" }), O.add(f))
          : $(f, b, w));
    },
    delete(f) {
      const b = c.get(f);
    ...
    toObject() {
      const f = {};
      for (const b of g) c.has(b) && (f[b] = c.get(b));
      return f;
    },
  };
  return (
  ...
  return new Proxy(
    {},
    {
      get(i, s) {
        ...
        if (typeof s != "symbol" && !we.has(s)) return e.get(s);
      },
      set(i, s, o) {
...
function yt(e, t = 50) {
  ...
  function i(s, o) {
    ...
    if (Array.isArray(s)) {
      ...
      l.add(s);
      const c = `[${s.map((g) => i(g, o + 1)).join(",")}]`;
      return l.delete(s), c;
    }
    if (d === "object") {
      const c = s;
      if (l.has(c)) return '"[circular]"';
      l.add(c);
      const g = `{${Object.keys(c)
        .sort()
        .map((p) => `${JSON.stringify(p)}:${i(c[p], o + 1)}`)
        .join(",")}}`;
      return l.delete(c), g;
    }
    return '"[unknown]"';
...
function Se(e, t = 50) {
  ...
  function s(o, d) {
    ...
    if ((i.add(c), Array.isArray(c))) {
      for (const g of c) if (!s(g, d + 1)) return i.delete(c), !1;
      return i.delete(c), !0;
    }
    for (const g of Object.keys(c))
      if (l.has(g) || !s(c[g], d + 1)) return i.delete(c), !1;
    return i.delete(c), !0;
  }
  return s(e, 0);
}
...
function Ft(e, t, l) {
  return { requirement: e, id: Lt(e, l), fromConstraint: t };
}
var Le = class bt {
    map = new Map();
    add(t) {
...
function Pt(e) {
  ...
  function y() {
    ...
    function h(S, M) {
      ...
      n.add(S), M.push(S);
      const z = t[S];
      if (z?.after) for (const W of z.after) t[W] && h(W, M);
  ...
  async function w(r) {
    ...
    try {
      const S = n.when(l),
        M = await xe(S, h, `Constraint "${r}" timed out after ${h}ms`);
  ...
  function v() {
    return (
      F ||
        (F = Object.keys(t).sort((r, n) => {
          const a = L(r),
            h = L(n).priority - a.priority;
          if (h !== 0) return h;
          const S = m.get(r) ?? 0,
            M = m.get(n) ?? 0;
          return S - M;
  ...
  function k(r) {
    ...
    for (const a of n.after)
      if (t[a] && !g.has(a) && !A.has(a) && !C.has(a)) return !1;
    return !0;
  }
  return {
    async evaluate(r) {
      ...
      async function M(_) {
        ...
        const re = [],
          ee = [];
        for (const U of Y) L(U).isAsync ? ee.push(U) : re.push(U);
        const Q = [];
        for (const U of re) {
          const J = b(U);
      ...
      while (z.length > 0 && W > 0) {
        const _ = z.length;
        if (((z = await M(z)), z.length === _)) break;
        W--;
      }
      return n.all();
    },
...
function Nt(e) {
  ...
  function P(y) {
    ...
    $.isComputing = !0;
    try {
      const { value: L, deps: f } = Ce(() => R(l, j));
      return (
        ($.cachedValue = L), ($.isStale = !1), B(y, f), i?.(y, L, [...f]), L
      );
    } catch (L) {
      throw (o?.(y, L), L);
    } finally {
    ...
    if (!(E > 0 || O)) {
      ...
      try {
        ...
        while (T.size > 0) {
          ...
          const $ = [...T];
          T.clear();
          for (const R of $) c.get(R)?.forEach((L) => L());
        }
      } finally {
        O = !1;
      }
  ...
  function x(y, $ = new Set()) {
    if ($.has(y)) return;
    $.add(y);
    const R = d.get(y);
    if (!R || R.isStale) return;
    (R.isStale = !0), s?.(y), T.add(y);
    const L = p.get(y);
    ...
    (j = new Proxy(
      {},
      {
        get(y, $) {
          ...
          _e($);
          const R = A($);
          return R.isStale && P($), R.cachedValue;
        },
      },
    )),
    {
      get(y) {
        const $ = A(y);
        return $.isStale && P(y), $.cachedValue;
      },
      isStale(y) {
      ...
      subscribe(y, $) {
        for (const R of y) {
          const L = R;
          c.has(L) || c.set(L, new Set()), c.get(L).add($);
        }
        return () => {
          for (const R of y) {
            const L = R,
              f = c.get(L);
            f?.delete($), f && f.size === 0 && c.delete(L);
          }
        };
      },
      getProxy() {
      ...
      registerDefinitions(y) {
        for (const [$, R] of Object.entries(y)) (t[$] = R), C($);
      },
    }
  );
}
function Wt(e) {
  ...
  function p(C) {
    const A = t[C];
    if (!A) throw new Error(`[Directive] Unknown effect: ${C}`);
    const P = {
      id: C,
      enabled: !0,
  ...
  function T(C, A) {
    ...
    if (P.dependencies) {
      for (const B of P.dependencies) if (A.has(B)) return !0;
      return !1;
    }
    return !0;
  }
      ...
      try {
        if (A.hasExplicitDeps) {
          ...
          y instanceof Promise && (y = await y),
            D(A, y),
            (A.dependencies = B.size > 0 ? B : null);
        }
      } catch (B) {
        o?.(C, B),
          console.error(`[Directive] Effect "${C}" threw an error:`, B);
      }
    }
  }
  for (const C of Object.keys(t)) p(C);
  return {
    async runEffects(C) {
      const A = [];
      for (const P of Object.keys(t)) T(P, C) && A.push(P);
      await Promise.all(A.map(j)), (c = E());
    },
    async runAll() {
      const C = Object.keys(t);
      await Promise.all(
        C.map((A) => (u(A).enabled ? j(A) : Promise.resolve())),
      ),
        (c = E());
    },
    disable(C) {
    ...
    cleanupAll() {
      g = !0;
      for (const C of d.values()) O(C);
    },
    registerDefinitions(C) {
      for (const [A, P] of Object.entries(C)) (t[A] = P), p(A);
    },
  };
}
function Ht(e = {}) {
  ...
  return {
    ...
    processDueRetries() {
      const c = Date.now(),
        g = [];
      for (const [p, u] of o) u.nextRetryTime <= c && (g.push(u), o.delete(p));
      return g;
    },
    cancelRetry(c) {
      o.delete(c);
...
function Ut(e = {}) {
  ...
  function p(u, E, T) {
    ...
    return typeof O == "string" ? O : Kt[u];
  }
  return {
    handleError(u, E, T, O) {
      const D = g(u, E, T, O);
      s.push(D), s.length > o && s.shift();
      try {
        l?.(D);
      } catch (C) {
      ...
      let j = p(u, E, T instanceof Error ? T : new Error(String(T)));
      if (j === "retry-later") {
        const C = (c.get(E) ?? 0) + 1;
        c.set(E, C),
          d.scheduleRetry(u, E, O, C) ||
            ((j = "skip"), c.delete(E), typeof process < "u");
      }
      try {
        i?.(D, j);
      } catch (C) {
    ...
    for (let F = 1; F <= N.attempts; F++) {
      ...
      try {
        ...
        if (
          ...
          return;
        if (N.shouldRetry && !N.shouldRetry(H, F)) break;
        if (F < N.attempts) {
          if (w.signal.aborted) return;
          const r = Xe(N, F);
          if (
      ...
      try {
        ...
        if (w.resolveBatchWithResults) {
          ...
          i.batch(() => {
            ...
            k && k > 0
              ? await xe(h, k, `Batch resolver "${f}" timed out after ${k}ms`)
              : await h;
          const S = Date.now() - F;
          for (const M of b)
            E.set(M.id, {
...
function Yt(e) {
  ...
  return {
    ...
    takeSnapshot(B) {
      ...
      const m = { id: u++, timestamp: Date.now(), facts: A(), trigger: B };
      for (
    ...
    goBack(B = 1) {
      ...
      let m = p,
        x = p,
        y = O.find((R) => p > R.startIndex && p <= R.endIndex);
      if (y) x = y.startIndex;
      else if (O.find((R) => p === R.startIndex)) {
        const R = O.find((L) => L.endIndex < p && p - L.endIndex <= B);
        x = R ? R.startIndex : Math.max(0, p - B);
      } else x = Math.max(0, p - B);
      if (m === x) return;
      p = x;
      const $ = g[p];
      $ && (this.restore($), o?.(m, x));
    ...
    goForward(B = 1) {
      ...
      let m = p,
        x = p,
        y = O.find((R) => p >= R.startIndex && p < R.endIndex);
      if ((y ? (x = y.endIndex) : (x = Math.min(g.length - 1, p + B)), m === x))
        return;
      p = x;
      const $ = g[p];
      $ && (this.restore($), o?.(m, x));
...
function wt(e) {
  ...
  if (e.modules.some((r) => r.snapshotEvents)) {
    ...
    for (const r of e.modules) {
      const n = r;
      if (n.snapshotEvents) for (const a of n.snapshotEvents) c.add(a);
      else if (n.events) for (const a of Object.keys(n.events)) c.add(a);
    }
  }
  let g = 0,
    p = !1,
    u = Vt();
  for (const r of e.plugins ?? []) u.register(r);
  let E = Ut({
      config: e.errorBoundary,
      onError: (r) => u.emitError(r),
      onRecovery: (r, n) => u.emitErrorRecovery(r, n),
    }),
    T = () => {},
    O = () => {},
    D = null,
    ...
    A = Nt({
      definitions: i,
      facts: C,
      onCompute: (r, n, a) => u.emitDerivationCompute(r, n, a),
      onInvalidate: (r) => u.emitDerivationInvalidate(r),
      onError: (r, n) => {
        E.handleError("derivation", r, n);
      },
    });
  (T = (r) => A.invalidate(r)), (O = (r) => A.invalidateMany(r));
  const P = Wt({
      definitions: s,
      facts: C,
      store: j,
      onRun: (r) => u.emitEffectRun(r),
      onError: (r, n) => {
        E.handleError("effect", r, n), u.emitEffectError(r, n);
      },
    }),
    B = Pt({
      definitions: o,
      facts: C,
      onEvaluate: (r, n) => u.emitConstraintEvaluate(r, n),
      onError: (r, n) => {
        E.handleError("constraint", r, n), u.emitConstraintError(r, n);
      },
    }),
    m = Jt({
      definitions: d,
      facts: C,
      store: j,
      onStart: (r, n) => u.emitResolverStart(r, n),
      onComplete: (r, n, a) => {
        u.emitResolverComplete(r, n, a),
          u.emitRequirementMet(n, r),
          B.markResolved(n.fromConstraint);
      },
      onError: (r, n, a) => {
        E.handleError("resolver", r, a, n), u.emitResolverError(r, n, a);
      },
      onRetry: (r, n, a) => u.emitResolverRetry(r, n, a),
      onCancel: (r, n) => {
        u.emitResolverCancel(r, n), u.emitRequirementCanceled(n);
      },
    ...
    if (!w.isReconciling) {
      ...
      try {
        w.changedKeys.size > 0 &&
          ...
          (p = !1));
        const r = C.$snapshot();
        u.emitReconcileStart(r), await P.runEffects(w.changedKeys);
        const n = new Set(w.changedKeys);
        w.changedKeys.clear();
        const a = await B.evaluate(n),
          h = new Le();
        for (const _ of a) h.add(_), u.emitRequirementCreated(_);
...
`)
  ...
  function k(r) {
    ...
    if (r.snapshotEvents) {
      c === null && (c = new Set(Object.keys(l)));
      for (const a of r.snapshotEvents) c.add(a);
    } else if (c !== null && r.events)
      for (const a of Object.keys(r.events)) c.add(a);
    Object.assign(t, r.schema),
      r.events && Object.assign(l, r.events),
      ...
      r.constraints &&
        (Object.assign(o, r.constraints), B.registerDefinitions(r.constraints)),
      r.resolvers &&
      ...
      r.hooks?.onInit?.(v),
      w.isRunning && (r.hooks?.onStart?.(v), I());
  }
  (v.registerModule = k), u.emitInit(v);
...
function Xt(e) {
  ...
  function d(c) {
    ...
    o.pop(), i.delete(c), l.add(c), s.push(c);
  }
  for (const c of t) d(c);
  return s;
}
var Qe = new WeakMap(),
  Ze = new WeakMap(),
...
function Zt(e) {
  ...
  for (const m of s) {
    ...
    const y = x.crossModuleDeps && Object.keys(x.crossModuleDeps).length > 0,
      $ = y ? Object.keys(x.crossModuleDeps) : [],
      R = {};
    for (const [v, k] of Object.entries(x.schema.facts)) R[`${m}${K}${v}`] = k;
    const L = {};
    ...
    if (x.resolvers)
      for (const [v, k] of Object.entries(x.resolvers)) {
        ...
        H[`${m}${K}${v}`] = {
          ...r,
          resolve: async (n, a) => {
            const h = Me(a.facts, t, () => Object.keys(t));
    ...
    if (x.effects)
      for (const [v, k] of Object.entries(x.effects)) {
        ...
        F[`${m}${K}${v}`] = {
          ...r,
          run: (n, a) => {
            const h = y ? ae(n, m, $) : se(n, m),
              S = a ? (y ? ae(a, m, $) : se(a, m)) : void 0;
            return r.run(h, S);
          },
          deps: r.deps?.map((n) => `${m}${K}${n}`),
        };
    ...
    g.push({
      ...
      hooks: x.hooks,
      snapshotEvents:
        i && !i.has(m) ? [] : x.snapshotEvents?.map((v) => `${m}${K}${v}`),
    });
  }
  let p = null,
    u = null;
  function E(m) {
    for (const [x, y] of Object.entries(m))
      if (!ne.has(x) && l.has(x)) {
        ...
        for (const [$, R] of Object.entries(y))
          ne.has($) || (u.facts[`${x}${K}${$}`] = R);
      }
  }
  u = wt({
    modules: g.map((m) => ({
  ...
  for (const m of Object.keys(t)) {
    ...
    if (!x) continue;
    const y = [];
    for (const $ of Object.keys(x.schema.facts)) y.push(`${m}${K}${$}`);
    if (x.schema.derivations)
      for (const $ of Object.keys(x.schema.derivations)) y.push(`${m}${K}${$}`);
    T.set(m, y);
  }
  const O = { names: null };
  function D() {
  ...
  return {
    ...
    getDistributableSnapshot(m) {
      const x = {
        ...
        $ = {};
      for (const [R, L] of Object.entries(y.data)) {
        const f = R.indexOf(K);
        if (f > 0) {
          const b = R.slice(0, f),
    ...
    watchDistributableSnapshot(m, x) {
      ...
      return u.watchDistributableSnapshot(y, ($) => {
        const R = {};
    ...
    registerModule(m, x) {
      ...
      const y = x,
        $ = y.crossModuleDeps && Object.keys(y.crossModuleDeps).length > 0,
        R = $ ? Object.keys(y.crossModuleDeps) : [],
        L = {};
      ...
      if (y.effects)
        for (const [v, k] of Object.entries(y.effects)) {
          ...
          H[`${m}${K}${v}`] = {
            ...r,
            run: (n, a) => {
              const h = $ ? ae(n, m, R) : se(n, m),
                S = a ? ($ ? ae(a, m, R) : se(a, m)) : void 0;
              return r.run(h, S);
            },
            deps: r.deps?.map((n) => `${m}${K}${n}`),
          };
        }
      l.add(m), (t[m] = y), (O.names = null);
      const F = [];
      for (const v of Object.keys(y.schema.facts)) F.push(`${m}${K}${v}`);
      ...
      T.set(m, F),
        u.registerModule({
          ...
          hooks: y.hooks,
          snapshotEvents:
            i && !i.has(m) ? [] : y.snapshotEvents?.map((v) => `${m}${K}${v}`),
        });
    },
  };
}
    ...
    onAfterModuleInit: () => {
      if (e.initialFacts)
        for (const [p, u] of Object.entries(e.initialFacts))
          ne.has(p) || (o.facts[p] = u);
      if (s) {
        for (const [p, u] of Object.entries(s)) ne.has(p) || (o.facts[p] = u);
        s = null;
      }
    },
  });
...
var or = 200,
  ...
  be = 80,
  it = 2,
  st = ["#8b9aff", "#4ade80", "#fbbf24", "#c084fc", "#f472b6", "#22d3ee"];
function lr() {
  return { entries: new St(or), inflight: new Map() };
}
function ar() {
  return {
...
function pr(e, t, l, i) {
  ...
  c.setAttribute("aria-label", "Open Directive DevTools"),
    c.setAttribute("aria-expanded", String(l)),
  ...
  function P(U, J) {
    ...
    return (
      de.appendChild(Ue),
      X.appendChild(de),
      { details: X, tbody: Ue, countSpan: fe }
    );
  }
  function B(U, J) {
    const X = document.createElement("details");
  ...
  c.addEventListener("click", _), E.addEventListener("click", V);
  function Y(U) {
    U.key === "Escape" && z && V();
  }
  ...
  function ee() {
    s || (document.body.appendChild(c), document.body.appendChild(g));
  }
  document.body
    ? ee()
  ...
  function Q() {
    ...
    W.clear(), c.remove(), g.remove(), d.remove();
  }
  return {
    refs: {
      container: g,
...
function ke(e, t, l, i, s, o) {
  let d = Et(i),
    c = e.get(l);
  if (c) {
    const g = c.cells;
    if (g[1] && ((g[1].textContent = d), s && o)) {
      const p = g[1];
...
function Te(e, t, l) {
  const i = t === 0 && l === 0;
  (e.statusEl.style.color = i ? q.green : q.yellow),
...
function lt(e, t, l, i) {
  ...
  const o = new Set(s);
  for (const [d, c] of t) o.has(d) || (c.remove(), t.delete(d));
  for (const d of s) {
    let c;
    try {
      c = Et(l.read(d));
...
function hr(e, t, l, i) {
  ...
  (u.style.color = q.accent), (u.textContent = ` ${t} `);
  ...
  for (
    ...
    e.eventsList.childElementCount > fr;
  )
  ...
  if (t.resolverStats.size > 0) {
    ...
    for (const [d, c] of o) {
      ...
      (p.style.paddingLeft = "8px"),
        (p.textContent = `${d}: ${c.count}x, avg ${g}ms${c.errors > 0 ? `, ${c.errors} err` : ""}`),
...
var qe = new WeakMap();
function yr(e, t, l, i, s, o) {
  return [
    e.join(","),
    t.join(","),
    l.map((d) => `${d.id}:${d.active}`).join(","),
    [...i.entries()].map(([d, c]) => `${d}:${c.status}:${c.type}`).join(","),
    s.join(","),
    o.join(","),
  ].join("|");
}
function br(e, t, l, i, s) {
  for (const o of l) {
    ...
    if (!d) continue;
    const c = t.recentlyChangedFacts.has(o);
  ...
  for (const o of i) {
    ...
    if (!d) continue;
    const c = t.recentlyComputedDerivations.has(o);
  ...
  for (const o of s) {
    ...
    if (!d) continue;
    const c = t.recentlyActiveConstraints.has(o),
      g = d.rect.getAttribute("stroke") ?? q.muted;
    d.rect.setAttribute("fill", c ? g + "33" : "none"),
      d.rect.setAttribute("stroke-width", c ? "2" : "1");
  }
}
function ut(e, t, l) {
  ...
  if (s.length === 0 && o.length === 0 && d.length === 0 && p.length === 0) {
    qe.delete(e.flowSvg),
  ...
  const P = A(s.map((v) => ({ id: v, label: pe(v, Z.labelMaxChars) }))),
    B = A(o.map((v) => ({ id: v, label: pe(v, Z.labelMaxChars) }))),
    m = A(
      d.map((v) => ({
        id: v.id,
        label: pe(v.id, Z.labelMaxChars),
        active: v.active,
        priority: v.priority,
      })),
    ),
    ...
    y = A(p.map((v) => ({ id: v, label: pe(v, Z.labelMaxChars) }))),
    $ = Math.max(P.length, B.length, m.length, x.length, y.length, 1),
    R = Z.startY + 12 + $ * (Z.nodeH + Z.nodeGap) + 8;
  e.flowSvg.replaceChildren(),
    e.flowSvg.setAttribute("viewBox", `0 0 ${C} ${R}`),
    e.flowSvg.setAttribute(
      "aria-label",
      `Dependency graph: ${s.length} facts, ${o.length} derivations, ${d.length} constraints, ${u.size} requirements, ${p.length} resolvers`,
    );
  ...
  for (const v of P) {
    const k = l.recentlyChangedFacts.has(v.id),
      r = b(0, j[0], v.y, v.id, v.label, q.text, !1, k);
    I.set(v.id, r);
  }
  for (const v of B) {
    const k = l.recentlyComputedDerivations.has(v.id),
      r = b(1, j[1], v.y, v.id, v.label, q.accent, !1, k);
    N.set(v.id, r);
  }
  for (const v of m) {
  ...
  for (const v of x) {
    const k = v.status === "unmet" ? q.red : q.yellow,
      r = b(3, j[3], v.y, v.id, pe(v.type, Z.labelMaxChars), k, !1, !1);
    F.set(v.id, r);
  }
  for (const v of y) {
    const k = g.some((r) => r.resolverId === v.id);
  ...
  for (const v of x) {
    ...
    k &&
      r &&
      w(k.midX + Z.nodeW / 2, k.midY, r.midX - Z.nodeW / 2, r.midY, q.muted);
  }
  for (const v of g) {
    const k = F.get(v.id);
    if (k) {
      const r = y.find((n) => n.id === v.resolverId);
      r && w(k.midX + Z.nodeW / 2, k.midY, j[4], r.y, q.green);
    }
  }
  qe.set(e.flowSvg, f);
}
...
function Sr(e, t) {
  ...
  const u = g.slice(-12),
    E = ye * u.length + 20;
  e.timelineSvg.setAttribute("viewBox", `0 0 ${Ae} ${E}`),
  ...
  for (let O = 0; O <= T; O++) {
    ...
    C.setAttribute("x", String(D)),
      C.setAttribute("y", "8"),
      C.setAttribute("fill", q.muted),
      C.setAttribute("font-size", "6"),
      C.setAttribute("font-family", q.font),
  ...
  for (let O = 0; O < u.length; O++) {
    ...
    P.setAttribute("x", String(be - 4)),
      P.setAttribute("y", String(j + ye / 2 + 3)),
      P.setAttribute("fill", q.muted),
      P.setAttribute("font-size", "7"),
      P.setAttribute("font-family", q.font),
      P.setAttribute("text-anchor", "end"),
      (P.textContent = pe(D, 12)),
      e.timelineSvg.appendChild(P);
    const B = l.filter((x) => x.resolver === D);
    for (const x of B) {
      const y = be + ((x.startMs - i) / d) * c,
        $ = Math.max(((x.endMs - x.startMs) / d) * c, it),
      ...
      const L = document.createElementNS("http://www.w3.org/2000/svg", "title"),
        f = x.endMs - x.startMs;
      (L.textContent = `${D}: ${f.toFixed(1)}ms${x.error ? " (error)" : ""}`),
        R.appendChild(L),
    ...
    if (m !== void 0) {
      const x = be + ((m - i) / d) * c,
        y = Math.max(((o - m) / d) * c, it),
        $ = document.createElementNS("http://www.w3.org/2000/svg", "rect");
      $.setAttribute("x", String(x)),
        $.setAttribute("y", String(j + 2)),
        $.setAttribute("width", String(y)),
        $.setAttribute("height", String(ye - 4)),
        $.setAttribute("rx", "2"),
        $.setAttribute("fill", A),
      ...
      t = {
        ...
        importSession(l, i) {
          try {
            ...
            const d = o.maxEvents,
              c = s.events,
              g = c.length > d ? c.length - d : 0;
            o.events.clear();
            for (let p = g; p < c.length; p++) {
              const u = c[p];
              u &&
                typeof u == "object" &&
                !Array.isArray(u) &&
...
function xr(e = {}) {
  ...
  let u = (n, a) => {
    ...
    C = dr(),
    A = lr(),
    P = s && typeof window < "u" && typeof document < "u" && Pe(),
    B = null,
    m = 0,
    x = 1,
    y = 2,
  ...
  function v() {
    ...
    const n = E.refs,
      ...
      h = m;
    if (((m = 0), h & x)) {
      for (const S of N) mr(T, S);
      N.clear();
      for (const [S, { value: M, flash: z }] of I)
        ke(T, n.factsBody, S, M, z, E.flashTimers);
      I.clear(), (n.factsCount.textContent = String(T.size));
    }
    if ((h & y && lt(n, O, a, E.flashTimers), h & R))
      if (H) Te(n, H.inflight.length, H.unmet.length);
      else {
        const S = $e(a);
        S && Te(n, S.inflight.length, S.unmet.length);
      }
    if (h & $)
      if (H) Ie(n, H.inflight, H.unmet);
      else {
        const S = $e(a);
        S && Ie(n, S.inflight, S.unmet);
      }
    h & L && gr(n, D),
      h & f && ut(n, a, j),
      h & b && at(n, a),
  ...
  function k(n, a) {
    E && l && hr(E.refs, n, a, p.events.size);
  }
  function r(n, a) {
    C.isRecording &&
      C.recordedEvents.length < ur &&
      C.recordedEvents.push({ timestamp: Date.now(), type: n, data: nr(a) });
  }
  return {
    name: "devtools",
    onInit: (n) => {
      if (
        ...
        try {
          ...
          for (const [z, W] of Object.entries(M)) ke(T, h.factsBody, z, W, !1);
          h.factsCount.textContent = String(Object.keys(M).length);
        } catch {}
        lt(h, O, a);
        const S = $e(a);
        S &&
          (Te(h, S.inflight.length, S.unmet.length),
          Ie(h, S.inflight, S.unmet)),
          at(h, a),
          vr(h, a),
          ut(h, a, j),
          h.recordBtn.addEventListener("click", () => {
            if (
              ...
              (h.recordBtn.textContent = C.isRecording ? "⏹ Stop" : "⏺ Record"),
              (h.recordBtn.style.color = C.isRecording ? q.red : q.text),
              C.isRecording)
            ) {
    ...
    onStart: (n) => {
      u("start", {}), k("start", {}), r("start", {});
    },
    onStop: (n) => {
      u("stop", {}), k("stop", {}), r("stop", {});
    },
    onDestroy: (n) => {
      u("destroy", {}),
        g.systems.delete(t),
        ...
        E && (E.destroy(), (E = null), T.clear(), O.clear());
    },
    onFactSet: (n, a, h) => {
      u("fact.set", { key: n, value: a, prev: h }),
    ...
    onFactDelete: (n, a) => {
      u("fact.delete", { key: n, prev: a }),
        ...
        E && (N.add(n), I.delete(n), F(x), k("fact.delete", { key: n }));
    },
    onFactsBatch: (n) => {
      if (
        ...
        for (const a of n)
          a.type === "delete"
            ...
            : (j.recentlyChangedFacts.add(a.key),
              ...
              N.delete(a.key));
        F(x), k("facts.batch", { count: n.length });
      }
    },
    onDerivationCompute: (n, a, h) => {
      u("derivation.compute", { id: n, value: a, deps: h }),
    ...
    onReconcileEnd: (n) => {
      ...
      if (C.isRecording && p.system && C.snapshots.length < cr)
        try {
          C.snapshots.push({
            timestamp: Date.now(),
    ...
    onResolverComplete: (n, a, h) => {
      u("resolver.complete", { resolver: n, requirementId: a.id, duration: h }),
        r("resolver.complete", {
          resolver: n,
          requirementId: a.id,
          duration: h,
      ...
      if (
        ...
        const z = D.resolverStats.keys().next().value;
        z !== void 0 && D.resolverStats.delete(z);
      }
    ...
    onResolverRetry: (n, a, h) => {
      u("resolver.retry", { resolver: n, requirementId: a.id, attempt: h }),
...
function Oe(e) {
  ...
  let l = "";
  for (let i = 0; i < e; i++) l += t[Math.floor(Math.random() * t.length)];
  return l;
}
async function $r(e, t, l, i) {
  if ((await Ne(800), !e.trim())) throw new Error("Email is required");
  if (xt(l)) throw new Error("Login failed: invalid credentials");
  return {
    token: `tok_${Oe(16)}`,
    refreshToken: `ref_${Oe(16)}`,
    expiresIn: i,
  };
}
async function kr(e, t, l) {
  if ((await Ne(500), xt(t)))
...
async function Cr(e) {
  await Ne(400);
  const t = e.split("").reduce((o, d) => o + d.charCodeAt(0), 0),
    l = ["Alice Johnson", "Bob Smith", "Carol White", "Dan Brown"],
    i = ["admin", "viewer", "editor", "moderator"],
    s = ["alice@test.com", "bob@test.com", "carol@test.com", "dan@test.com"];
  return {
    id: `user_${t % 1e3}`,
    name: l[t % l.length],
    email: s[t % s.length],
...
const ze = {
  facts: {
    ...
    user: G.object(),
    status: G.string(),
    loginRequested: G.boolean(),
    now: G.number(),
    tokenTTL: G.number(),
    refreshBuffer: G.number(),
    loginFailRate: G.number(),
    refreshFailRate: G.number(),
    eventLog: G.object(),
  },
  derivations: {
    isAuthenticated: G.boolean(),
    isExpiringSoon: G.boolean(),
    canRefresh: G.boolean(),
    tokenTimeRemaining: G.number(),
    canLogin: G.boolean(),
  },
  events: {
    setEmail: { value: G.string() },
    setPassword: { value: G.string() },
    requestLogin: {},
    logout: {},
    forceExpire: {},
    setTokenTTL: { value: G.number() },
    setRefreshBuffer: { value: G.number() },
    setLoginFailRate: { value: G.number() },
    setRefreshFailRate: { value: G.number() },
    tick: {},
  },
  requirements: {
    LOGIN: { email: G.string(), password: G.string() },
    REFRESH_TOKEN: { refreshToken: G.string() },
    FETCH_USER: { token: G.string() },
  },
};
function ue(e, t, l) {
  const i = [...e.eventLog];
  i.push({ timestamp: Date.now(), event: t, detail: l }), (e.eventLog = i);
}
const Rr = Bt("auth-flow", {
    schema: ze,
    init: (e) => {
    ...
    derive: {
      isAuthenticated: (e) => e.status === "authenticated",
      isExpiringSoon: (e) =>
        e.token === "" ? !1 : e.now > e.expiresAt - e.refreshBuffer * 1e3,
      canRefresh: (e) => e.refreshToken !== "" && e.status !== "refreshing",
      tokenTimeRemaining: (e) =>
        e.token === ""
          ? 0
          : Math.max(0, Math.round((e.expiresAt - e.now) / 1e3)),
      canLogin: (e) =>
        e.email.trim() !== "" &&
        e.password.trim() !== "" &&
        (e.status === "idle" || e.status === "expired"),
    },
    events: {
      setEmail: (e, { value: t }) => {
        e.email = t;
      },
      setPassword: (e, { value: t }) => {
        e.password = t;
      },
      requestLogin: (e) => {
        (e.loginRequested = !0),
      ...
      setTokenTTL: (e, { value: t }) => {
        e.tokenTTL = t;
      },
      setRefreshBuffer: (e, { value: t }) => {
        e.refreshBuffer = t;
      },
      setLoginFailRate: (e, { value: t }) => {
        e.loginFailRate = t;
      },
      setRefreshFailRate: (e, { value: t }) => {
        e.refreshFailRate = t;
      },
      tick: (e) => {
        e.now = Date.now();
    ...
    constraints: {
      needsLogin: {
        priority: 100,
        when: (e) => e.loginRequested && e.status === "authenticating",
        require: (e) => ({
          type: "LOGIN",
          email: e.email,
          password: e.password,
      ...
      refreshNeeded: {
        ...
        when: (e) => {
          const t =
              e.token !== "" && e.now > e.expiresAt - e.refreshBuffer * 1e3,
            l = e.refreshToken !== "" && e.status !== "refreshing";
          return t && l && e.status === "authenticated";
        },
        require: (e) => ({
          type: "REFRESH_TOKEN",
      ...
      needsUser: {
        ...
        after: ["refreshNeeded"],
        when: (e) =>
          e.token !== "" && e.user === null && e.status !== "authenticating",
        require: (e) => ({ type: "FETCH_USER", token: e.token }),
      },
    },
    resolvers: {
      login: {
        ...
        timeout: 1e4,
        resolve: async (e, t) => {
          ue(t.facts, "login", "Authenticating...");
          try {
            const l = await $r(
              e.email,
              e.password,
              t.facts.loginFailRate,
              t.facts.tokenTTL,
            );
            (t.facts.token = l.token),
              (t.facts.refreshToken = l.refreshToken),
              (t.facts.expiresAt = Date.now() + l.expiresIn * 1e3),
              (t.facts.status = "authenticated"),
              (t.facts.user = null),
              ue(t.facts, "login-success", `Token: ${l.token.slice(0, 12)}...`);
          } catch (l) {
            const i = l instanceof Error ? l.message : "Unknown error";
            throw (
              ((t.facts.status = "idle"),
              (t.facts.loginRequested = !1),
              ue(t.facts, "login-error", i),
              l)
            );
          }
        },
      ...
      refreshToken: {
        ...
        timeout: 1e4,
        resolve: async (e, t) => {
          (t.facts.status = "refreshing"),
            ue(t.facts, "refresh", "Refreshing token...");
          try {
            const l = await kr(
              e.refreshToken,
              t.facts.refreshFailRate,
              t.facts.tokenTTL,
            );
            (t.facts.token = l.token),
              (t.facts.refreshToken = l.refreshToken),
              ...
              ue(
                t.facts,
                "refresh-success",
                `New token: ${l.token.slice(0, 12)}...`,
              );
          } catch (l) {
            const i = l instanceof Error ? l.message : "Unknown error";
            throw (
              ((t.facts.token = ""),
              (t.facts.refreshToken = ""),
              (t.facts.expiresAt = 0),
              (t.facts.status = "expired"),
              ue(t.facts, "refresh-error", i),
              l)
            );
          }
        },
      ...
      fetchUser: {
        ...
        resolve: async (e, t) => {
          ue(t.facts, "fetch-user", "Fetching user profile...");
          try {
            const l = await Cr(e.token);
            (t.facts.user = l),
              ue(t.facts, "fetch-user-success", `${l.name} (${l.role})`);
          } catch (l) {
            const i = l instanceof Error ? l.message : "Unknown error";
            ue(t.facts, "fetch-user-error", i);
          }
        },
      },
    },
    effects: {
      logStatusChange: {
        ...
        run: (e, t) => {
          t &&
            t.status !== e.status &&
            ue(e, "status", `${t.status} → ${e.status}`);
        },
      },
    },
  }),
  ie = Qt({ module: Rr, plugins: [xr({ name: "auth-flow" })] });
ie.start();
const Ar = [...Object.keys(ze.facts), ...Object.keys(ze.derivations)],
  ct = document.getElementById("af-status-badge"),
  dt = document.getElementById("af-user-display"),
  he = document.getElementById("af-countdown-fill"),
  ft = document.getElementById("af-countdown-text"),
  ge = document.getElementById("af-email"),
  ve = document.getElementById("af-password"),
  $t = document.getElementById("af-login-btn"),
  kt = document.getElementById("af-logout-btn"),
  Ct = document.getElementById("af-force-expire-btn"),
  De = document.getElementById("af-login-error"),
  pt = document.getElementById("af-token-ttl"),
  Or = document.getElementById("af-ttl-val"),
  mt = document.getElementById("af-refresh-buffer"),
  Dr = document.getElementById("af-buffer-val"),
  ht = document.getElementById("af-login-failrate"),
  jr = document.getElementById("af-login-fail-val"),
  gt = document.getElementById("af-refresh-failrate"),
  Mr = document.getElementById("af-refresh-fail-val"),
  Be = document.getElementById("af-timeline");
let je = "";
function Rt() {
  const e = ie.facts,
    t = ie.derive,
    ...
    i = e.token,
    s = e.user,
    o = t.tokenTimeRemaining,
    d = t.canLogin,
    c = e.eventLog;
  if (
    ((ct.textContent = l),
    (ct.className = `af-status-badge ${l}`),
    s ? (dt.textContent = `${s.name} (${s.role})`) : (dt.innerHTML = "&mdash;"),
    i !== "")
  ) {
    const u = e.tokenTTL,
      E = Math.min(100, Math.round((o / u) * 100));
    (he.style.width = `${E}%`),
      (he.className = "af-countdown-fill"),
      E <= 20
        ? he.classList.add("danger")
        : E <= 50 && he.classList.add("warning"),
      (ft.textContent = `${o}s remaining`);
  } else
    (he.style.width = "0%"),
      (he.className = "af-countdown-fill"),
      (ft.innerHTML = "&mdash;");
  ($t.disabled = !d),
    (kt.disabled = l === "idle" || l === "expired"),
    (Ct.disabled = i === "");
  const g = c
      .slice()
      .reverse()
      .find((u) => u.event === "login-error"),
    p = g ? g.detail : "";
  if (
    (p !== je &&
      ((je = p),
      (De.textContent = p),
      De.classList.toggle("visible", p !== "")),
    (Or.textContent = `${e.tokenTTL}s`),
    (Dr.textContent = `${e.refreshBuffer}s`),
    (jr.textContent = `${e.loginFailRate}%`),
    (Mr.textContent = `${e.refreshFailRate}%`),
    c.length === 0)
  )
    Be.innerHTML =
    ...
    for (let u = c.length - 1; u >= 0; u--) {
      ...
      T.className = `af-timeline-entry ${E.event}`;
      ...
      (T.innerHTML = `
        <span class="af-timeline-time">${D}</span>
        <span class="af-timeline-event">${vt(E.event)}</span>
        <span class="af-timeline-detail">${vt(E.detail)}</span>
      `),
        Be.appendChild(T);
    }
  }
...
function We() {
  De.classList.remove("visible"), (je = ""), ie.events.requestLogin();
}
$t.addEventListener("click", We);
ge.addEventListener("keydown", (e) => {
...
ge.addEventListener("input", () => {
  ie.events.setEmail({ value: ge.value });
});
ve.addEventListener("input", () => {
  ie.events.setPassword({ value: ve.value });
});
kt.addEventListener("click", () => {
  De.classList.remove("visible"), (je = ""), ie.events.logout();
});
Ct.addEventListener("click", () => {
  ie.events.forceExpire();
});
document.addEventListener("click", (e) => {
  const t = e.target;
  t.classList.contains("af-chip") &&
    t.dataset.email &&
    ((ge.value = t.dataset.email),
    (ve.value = t.dataset.password || "password"),
    ie.events.setEmail({ value: ge.value }),
    ie.events.setPassword({ value: ve.value }));
});
pt.addEventListener("input", () => {
  ie.events.setTokenTTL({ value: Number(pt.value) });
});
mt.addEventListener("input", () => {
  ie.events.setRefreshBuffer({ value: Number(mt.value) });
});
ht.addEventListener("input", () => {
  ie.events.setLoginFailRate({ value: Number(ht.value) });
});
gt.addEventListener("input", () => {
  ie.events.setRefreshFailRate({ value: Number(gt.value) });
});
function vt(e) {
  const t = document.createElement("div");
