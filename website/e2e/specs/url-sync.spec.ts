...
test.describe("URL Sync example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-url-sync", {
        state: "attached",
        timeout: 30_000,
      });
    } catch {
  ...
  test("page loads with products", async ({ page }) => {
    ...
    await expect(tid(page, "us-total-items")).toHaveText("50 items", {
      timeout: 5_000,
    });
  ...
  test("search filters products", async ({ page }) => {
    ...
    await expect(tid(page, "us-total-items")).not.toHaveText("50 items", {
      timeout: 5_000,
    });
  ...
  test("category filter works", async ({ page }) => {
    ...
    await expect(tid(page, "us-total-items")).toHaveText("13 items", {
      timeout: 5_000,
    });
  ...
  test("sort by price ascending", async ({ page }) => {
    ...
    const firstPrice = tid(page, "us-product-list")
      .locator(".us-product-price")
      .first();
  ...
  test("empty search shows no results", async ({ page }) => {
    ...
    await expect(tid(page, "us-total-items")).toHaveText("0 items", {
      timeout: 5_000,
    });
    await expect(tid(page, "us-product-list")).toContainText(
      "No products found",
      { timeout: 5_000 },
    );
  });
