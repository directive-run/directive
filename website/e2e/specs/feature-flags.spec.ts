...
test.describe("Feature Flags example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => localStorage.removeItem("directive-feature-flags-example"));
    await page.reload();
    try {
      await page.waitForSelector("directive-feature-flags", { state: "attached", timeout: 30_000 });
    } catch {
      await page.reload();
      await page.waitForSelector("directive-feature-flags", { state: "attached", timeout: 30_000 });
    }
    await page.waitForSelector("[data-feature-flags-ready]", { timeout: 15_000 });
  });

  test("page loads with default flags", async ({ page }) => {
    // All 8 flags should be enabled by default
    await expect(tid(page, "ff-enabled-count")).toContainText("8/8", { timeout: 5_000 });
  ...
  test("toggle flag updates preview", async ({ page }) => {
    ...
    const chatToggle = tid(page, "ff-flag-chat").locator("input[type='checkbox']");
    await chatToggle.uncheck();

    // Enabled count should decrease
    await expect(tid(page, "ff-enabled-count")).toContainText("7/8", { timeout: 5_000 });
  });
  ...
  test("maintenance mode disables gated features", async ({ page }) => {
    ...
    const maintenanceToggle = tid(page, "ff-maintenance").locator("input[type='checkbox']");
    await maintenanceToggle.check();

    // Preview should show "Disabled (maintenance)" for gated features
    await expect(tid(page, "ff-preview")).toContainText("Disabled (maintenance)", { timeout: 5_000 });
  });
  ...
  test("onboarding toast auto-enables brand switcher", async ({ page }) => {
    ...
    const toastToggle = tid(page, "ff-flag-onboarding-toast").locator("input[type='checkbox']");
    await toastToggle.uncheck();
    await expect(tid(page, "ff-enabled-count")).toContainText("7/8", { timeout: 5_000 });

    // Now disable brand switcher (no constraint fires because toast is off)
    const brandToggle = tid(page, "ff-flag-brand-switcher").locator("input[type='checkbox']");
    await brandToggle.uncheck();
    await expect(tid(page, "ff-enabled-count")).toContainText("6/8", { timeout: 5_000 });
  ...
  test("enabled count updates correctly", async ({ page }) => {
    ...
    const themeToggle = tid(page, "ff-flag-theme-selector").locator("input[type='checkbox']");
