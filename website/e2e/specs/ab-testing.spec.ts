...
test.describe("A/B Testing example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-ab-testing", { state: "attached", timeout: 30_000 });
    } catch {
      await page.reload();
      await page.waitForSelector("directive-ab-testing", { state: "attached", timeout: 30_000 });
    }
  ...
  test("page loads with experiments", async ({ page }) => {
    await expect(tid(page, "ab-experiment-count")).toHaveText("2", { timeout: 5_000 });
  ...
  test("experiments show variant assignments", async ({ page }) => {
    ...
    await expect(tid(page, "ab-assigned-count")).toHaveText("2", { timeout: 5_000 });

    // Each experiment card should show "Assigned:" with a variant ID (not "–")
    const experiments = tid(page, "ab-experiments").locator(".experiment");
    const firstMeta = experiments.first().locator(".experiment-meta");
    await expect(firstMeta).not.toContainText("Assigned: –", { timeout: 5_000 });
  });

  test("exposure count matches assignments", async ({ page }) => {
    // Both should be tracked after constraint chain settles
    await expect(tid(page, "ab-assigned-count")).toHaveText("2", { timeout: 5_000 });
    await expect(tid(page, "ab-exposed-count")).toHaveText("2", { timeout: 5_000 });
  });

  test("pause button toggles to resume", async ({ page }) => {
    await expect(tid(page, "ab-btn-pause")).toHaveText("Pause All", { timeout: 5_000 });

    await tid(page, "ab-btn-pause").click();

    await expect(tid(page, "ab-btn-pause")).toHaveText("Resume All", { timeout: 5_000 });
  });
