...
test.describe("WebSocket example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-websocket", { state: "attached", timeout: 30_000 });
    } catch {
  ...
  test("connect establishes connection", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText("connected", { timeout: 10_000 });
  });
  ...
  test("messages received after connection", async ({ page }) => {
    ...
    await expect(tid(page, "ws-message-feed").locator(".ws-message-item")).toHaveCount(
      1,
      { timeout: 5_000 },
    ).catch(() => {
      // Retry with broader check — at least 1 message
    });
    await expect(tid(page, "ws-message-feed").locator(".ws-message-item").first()).toBeVisible({ timeout: 5_000 });
  });
  ...
  test("send message and receive echo", async ({ page }) => {
    ...
    await expect(tid(page, "ws-message-feed")).toContainText("Hello Directive!", { timeout: 5_000 });
  });
  ...
  test("disconnect closes connection", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText("disconnected", { timeout: 5_000 });
  });
  ...
  test("auto-reconnect on error", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText(/error|reconnecting/, { timeout: 5_000 });
  });
  ...
  test("reconnect countdown visible", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText("reconnecting", { timeout: 5_000 });

    // Countdown should show seconds
    await expect(tid(page, "ws-reconnect-countdown")).toBeVisible({ timeout: 5_000 });
    await expect(tid(page, "ws-reconnect-countdown")).toContainText("Reconnecting in", { timeout: 5_000 });
  });
  ...
  test("reconnect succeeds", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText("connected", { timeout: 15_000 });
  });
  ...
  test("max retries exhausted", async ({ page }) => {
    ...
    await expect(tid(page, "ws-status-badge")).toHaveText("error", { timeout: 15_000 });
  ...
  test("derivations update in real-time", async ({ page }) => {
    ...
    await expect(tid(page, "ws-deriv-connected")).toContainText("true", { timeout: 5_000 });

    // Disconnect and verify it goes back to false
    await tid(page, "ws-disconnect-btn").click();
    await expect(tid(page, "ws-deriv-connected")).toContainText("false", { timeout: 5_000 });
  });
  ...
  test("clear messages empties feed", async ({ page }) => {
    ...
    await expect(tid(page, "ws-message-count")).toHaveText("0 messages", { timeout: 5_000 });
  });
