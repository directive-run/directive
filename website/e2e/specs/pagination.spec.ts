...
test.describe("Pagination example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-pagination", {
        state: "attached",
        timeout: 30_000,
      });
    } catch {
  ...
  test("page loads and renders initial items", async ({ page }) => {
    ...
    await expect(itemList.locator(".pg-item").first()).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(tid(page, "pg-loading")).not.toHaveClass(/visible/, {
      timeout: 5_000,
    });
  });
  ...
  test("category filter changes results", async ({ page }) => {
    ...
    const categories = await itemList
      .locator(".pg-item-category")
      .allTextContents();
    ...
    const allCategories = await itemList
      .locator(".pg-item-category")
      .allTextContents();
    const uniqueCategories = [
      ...new Set(allCategories.map((c) => c.toLowerCase())),
    ];
  ...
  test("search filters items", async ({ page }) => {
    // Wait for initial load
    await expect(
      tid(page, "pg-item-list").locator(".pg-item").first(),
    ...
    const titles = await tid(page, "pg-item-list")
      .locator(".pg-item-title")
      .allTextContents();
  ...
  test("sort changes item order", async ({ page }) => {
    ...
    const firstTitleBefore = await tid(page, "pg-item-list")
      .locator(".pg-item-title")
      .first()
      .textContent();
    ...
    const firstTitleAfter = await tid(page, "pg-item-list")
      .locator(".pg-item-title")
      .first()
      .textContent();
