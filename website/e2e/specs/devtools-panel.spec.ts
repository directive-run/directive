...
test.describe("Core DevTools Panel", () => {
  ...
  test("devtoolsPlugin registers system on window.__DIRECTIVE__", async ({
    page,
  }) => {
  ...
  test("panel toggle button renders in dev mode", async ({ page }) => {
    ...
    if (!panelExists) {
      ...
      test.skip(
        true,
        "DevTools panel not rendered (production build — isDevMode() returns false)",
      );
    }
  ...
  test("panel dependency graph renders SVG", async ({ page }) => {
    ...
    const graphSummary = panel.locator("summary", {
      hasText: "Dependency Graph",
    });
    await expect(graphSummary).toBeVisible({ timeout: 5_000 });
    await graphSummary.click();

    const svg = panel.locator(
      "details:has(summary:has-text('Dependency Graph')) svg",
    );
  ...
  test("panel does not break host page functionality", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid='sc-subtotal']")).toHaveText(
      /\$\d+\.\d{2}/,
    );
  });
