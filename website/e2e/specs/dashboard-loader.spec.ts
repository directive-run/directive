...
test.describe("Dashboard Loader example", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector("directive-dashboard-loader", { state: "attached", timeout: 30_000 });
    await page.waitForSelector("[data-dashboard-loader-ready]", { timeout: 15_000 });
  });
  ...
  test("loads all 3 resources on start", async ({ page }) => {
    ...
    await expect(tid(page, "dashboard-loader-profile-status")).toHaveText("success", { timeout: 10_000 });
    await expect(tid(page, "dashboard-loader-prefs-status")).toHaveText("success", { timeout: 10_000 });
    await expect(tid(page, "dashboard-loader-perms-status")).toHaveText("success", { timeout: 10_000 });
  });
  ...
  test("shows loading state during fetch", async ({ page }) => {
    ...
    await expect(tid(page, "dashboard-loader-profile-status")).toHaveText("loading", { timeout: 5_000 });
    await expect(tid(page, "dashboard-loader-prefs-status")).toHaveText("loading", { timeout: 5_000 });
    await expect(tid(page, "dashboard-loader-perms-status")).toHaveText("loading", { timeout: 5_000 });
  });
  ...
  test("shows error state when resource fails", async ({ page }) => {
    ...
    await expect(tid(page, "dashboard-loader-profile-status")).toHaveText("error", { timeout: 30_000 });
  ...
  test("retry button re-fetches failed resource", async ({ page }) => {
    ...
    // Should recover
    await expect(tid(page, "dashboard-loader-profile-status")).toHaveText("success", { timeout: 15_000 });
  });
  ...
  test("combined status updates correctly", async ({ page }) => {
    ...
    await expect(tid(page, "dashboard-loader-combined-status")).toContainText("Not started", { timeout: 5_000 });

    await setInput(page, "dashboard-loader-user-input", "testuser");
    await tid(page, "dashboard-loader-start-btn").click();

    // Eventually "All loaded"
    await expect(tid(page, "dashboard-loader-combined-status")).toHaveText("All loaded", { timeout: 10_000 });
  });
  ...
  test("delay sliders affect fetch time", async ({ page }) => {
    ...
    await expect(tid(page, "dashboard-loader-profile-status")).toHaveText("success", { timeout: 5_000 });
    // Permissions should still be loading
    await expect(tid(page, "dashboard-loader-perms-status")).toHaveText("loading");
  });
  ...
  test("timeline log records events", async ({ page }) => {
    ...
    const entries = tid(page, "dashboard-loader-timeline").locator(".dl-timeline-entry");
