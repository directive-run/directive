...
test.describe("Dynamic Modules example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-dynamic-modules", {
        state: "attached",
        timeout: 30_000,
      });
    } catch {
    ...
    await page.waitForSelector("[data-dynamic-modules-ready]", {
      timeout: 15_000,
    });
  });

  test("page loads with empty dashboard", async ({ page }) => {
    await expect(tid(page, "dm-widgets-area")).toContainText(
  ...
  test("load counter module", async ({ page }) => {
    ...
    await expect(tid(page, "dm-widget-counter")).toBeVisible({
      timeout: 3_000,
    });
  ...
  test("counter increment and decrement work", async ({ page }) => {
    ...
    await expect(tid(page, "dm-counter-value")).toContainText("1", {
      timeout: 1_000,
    });

    await tid(page, "dm-counter-increment").click();
    await expect(tid(page, "dm-counter-value")).toContainText("2", {
      timeout: 1_000,
    });
  ...
  test("counter overflow constraint resets", async ({ page }) => {
    ...
    await expect(tid(page, "dm-counter-value")).toContainText("0", {
      timeout: 5_000,
    });
  });
  ...
  test("load weather module", async ({ page }) => {
    ...
    await expect(tid(page, "dm-widget-weather")).toBeVisible({
      timeout: 3_000,
    });
    await expect(tid(page, "dm-status-badge")).toContainText("1 / 3");
  });
  ...
  test("weather fetches on city input", async ({ page }) => {
    ...
    await expect(tid(page, "dm-weather-summary")).toBeVisible({
      timeout: 5_000,
    });
  ...
  test("dice roll updates values", async ({ page }) => {
    ...
    await expect(tid(page, "dm-dice-total")).toContainText("Total: 2", {
      timeout: 1_000,
    });
