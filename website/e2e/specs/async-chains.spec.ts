...
test.describe("Async Chains example", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    try {
      await page.waitForSelector("directive-async-chains", { state: "attached", timeout: 30_000 });
    } catch {
      await page.reload();
      await page.waitForSelector("directive-async-chains", { state: "attached", timeout: 30_000 });
    }
    await page.waitForSelector("[data-async-chains-ready]", { timeout: 15_000 });
  });
  ...
  test("start chain triggers auth validation", async ({ page }) => {
    ...
    await expect(tid(page, "ac-auth-status")).not.toHaveText("idle", { timeout: 5_000 });
  });

  test("full chain completes: auth → permissions → dashboard", async ({ page }) => {
    await tid(page, "ac-start-btn").click();

    // Step 1: auth must succeed first
    await expect(tid(page, "ac-auth-status")).toHaveText("success", { timeout: 30_000 });

    // Step 2: permissions fires after auth (after ordering)
    await expect(tid(page, "ac-perms-status")).not.toHaveText("idle", { timeout: 30_000 });
    await expect(tid(page, "ac-perms-status")).toHaveText("success", { timeout: 30_000 });

    // Step 3: dashboard fires after permissions
    await expect(tid(page, "ac-dash-status")).not.toHaveText("idle", { timeout: 30_000 });
    await expect(tid(page, "ac-dash-status")).toHaveText("success", { timeout: 30_000 });
  });
  ...
  test("reset clears all modules", async ({ page }) => {
    await tid(page, "ac-start-btn").click();
    await expect(tid(page, "ac-auth-status")).toHaveText("success", { timeout: 15_000 });

    await tid(page, "ac-reset-btn").click();

    await expect(tid(page, "ac-auth-status")).toHaveText("idle", { timeout: 5_000 });
    await expect(tid(page, "ac-perms-status")).toHaveText("idle", { timeout: 5_000 });
    await expect(tid(page, "ac-dash-status")).toHaveText("idle", { timeout: 5_000 });
  });

  test("fail rate sliders respond", async ({ page }) => {
    // Set auth fail rate via evaluate
    await page.evaluate(() => {
      const slider = document.querySelector('[data-testid="ac-auth-fail-rate"]') as HTMLInputElement;
      slider.value = "50";
