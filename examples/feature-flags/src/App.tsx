...
const system = createSystem({
  ...
  plugins: [
    ...
    persistencePlugin({
      ...
      key: "directive-feature-flags-example",
    }),
  ],
});
system.start();
...
export function App() {
  ...
  const [effectsDisabled, setEffectsDisabled] = useState<Set<string>>(
    new Set(),
  );
  const [constraintsDisabled, setConstraintsDisabled] = useState<Set<string>>(
    new Set(),
  );
  const [timelineEntries, setTimelineEntries] = useState<TimelineEntry[]>([]);

  const addTimelineEntry = useCallback(
    (event: string, detail: string, type: string) => {
      setTimelineEntries((prev) => [
        { time: Date.now(), event, detail, type },
        ...prev,
      ]);
    },
    [],
  ...
  const handleDisableEffect = useCallback(
    (effectId: string) => {
  ...
  const handleEnableEffect = useCallback(
    (effectId: string) => {
  ...
  const handleEnableConstraint = useCallback(
    (constraintId: string) => {
  ...
  return (
    <div className="ff-container">
      ...
      <div className="ff-how-it-works">
        ...
        <ul>
          <li>
            <strong>Facts</strong> &ndash; chatEnabled, searchEnabled,
            playgroundEnabled, brandSwitcherEnabled, themeSelectorEnabled,
            onboardingToastEnabled, versionSelectorEnabled, voteApiEnabled,
            maintenanceMode
          </li>
          <li>
            <strong>Derivations</strong> &ndash; canUseChat...canUseVoteApi (8
            boolean), enabledCount, allFeaturesEnabled (all auto-tracked)
          </li>
          <li>
            <strong>Constraints</strong> &ndash; onboardingRequiresBrandSwitcher
            (auto-enables Brand Switcher), maintenanceWarning
          </li>
          <li>
            <strong>Resolvers</strong> &ndash; enableBrandSwitcher,
            logMaintenanceWarning
          </li>
          <li>
            <strong>Effects</strong> &ndash; logChanges (logs every flag toggle
            to console)
          </li>
        </ul>
        <p className="directive-note">
          The <code>onboardingRequiresBrandSwitcher</code> constraint
          automatically re-enables Brand Switcher whenever Onboarding Toast is
          toggled on. Maintenance mode gates chat, search, playground, and vote
          API via derivations.
        </p>
      </div>
    </div>
